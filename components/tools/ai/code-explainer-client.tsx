...
export function CodeExplainerClient() {
 ...
 <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 bg-background p-6 rounded-3xl border border-border shadow-md shadow-slate-200/50">
 ...
 <h1 className="text-2xl sm:text-3xl font-black text-foreground tracking-tight">AI Code Explainer & Converter</h1>
 ...
 <p className="text-sm text-muted-foreground">Instantly decode complex algorithms, convert languages, generate unit tests, and audit Big-O complexity using top-tier AI models.</p>
 </div>
 </div>
 ...
 <span className="text-[11px] font-mono text-muted-foreground bg-slate-100 px-2 py-0.5 rounded">{lineCount} lines</span>
 </div>
 </CardHeader>
 <CardContent className="p-0">
 <textarea
 className="w-full bg-background p-4 text-sm outline-none resize-none min-h-[300px] font-mono leading-relaxed text-foreground focus:ring-0 border-0"
 ...
 <h3 className="text-sm font-bold text-foreground dark:text-slate-200">Configuration</h3>
 </div>
 
 <div className="space-y-3">
 <div>
 <Label className="text-xs mb-1.5 block text-slate-600 font-semibold">AI Model</Label>
 <select
 className="w-full rounded-xl border border-border bg-slate-50 px-3 py-2.5 text-sm text-foreground font-medium focus:ring-2 focus:ring-cyan-500 outline-none"
 value={aiModel}
 ...
 <div>
 <Label className="text-xs mb-1.5 block text-slate-600 font-semibold">Core Task</Label>
 <select
 className="w-full rounded-xl border border-border bg-slate-50 px-3 py-2.5 text-sm text-foreground font-medium focus:ring-2 focus:ring-cyan-500 outline-none"
 value={actionType}
 ...
 <Label className="text-xs mb-1.5 block text-slate-600 font-semibold">Target Language</Label>
 <select
 ...
 <div className="pt-2">
 <Label className="text-xs mb-2 block text-slate-600 font-semibold">Advanced Prompt Toggles</Label>
 <div className="space-y-2">
 <label className="flex items-center gap-2 cursor-pointer group">
 <input type="checkbox"checked={includeComments} onChange={(e) => setIncludeComments(e.target.checked)} className="rounded text-cyan-600 focus:ring-cyan-500 bg-slate-100 border-border cursor-pointer"/>
 ...
 <label className="flex items-center gap-2 cursor-pointer group">
 <input type="checkbox"checked={optimizePerformance} onChange={(e) => setOptimizePerformance(e.target.checked)} className="rounded text-cyan-600 focus:ring-cyan-500 bg-slate-100 border-border cursor-pointer"/>
 ...
 <label className="flex items-center gap-2 cursor-pointer group">
 <input type="checkbox"checked={edgeCases} onChange={(e) => setEdgeCases(e.target.checked)} className="rounded text-cyan-600 focus:ring-cyan-500 bg-slate-100 border-border cursor-pointer"/>
 ...
 <h3 className="text-sm font-bold text-foreground dark:text-slate-200">Recent Executions</h3>
 </div>
 ...
 <div className="space-y-2">
 {history.map((item) => (
 <div key={item.id} onClick={() => loadHistoryItem(item)} className="p-3 rounded-xl border border-border bg-slate-50 hover:bg-cyan-50 dark:hover:bg-cyan-950/30 cursor-pointer transition-colors group">
 <div className="flex justify-between items-center mb-1">
 <span className="text-xs font-bold text-foreground dark:text-slate-200 capitalize">{item.actionType}</span>
 ...
 <CardHeader className="border-b border-slate-100 bg-slate-50/50 p-4 shrink-0">
 <div className="flex justify-between items-center">
 <CardTitle className="text-sm font-semibold flex items-center gap-2 text-foreground">
 ...
 {result && (
 <Button variant="outline"size="sm"onClick={handleCopy} className="h-8 text-xs gap-1.5 border-border hover:bg-slate-100 dark:hover:bg-muted">
 ...
 <p className="text-sm font-bold text-slate-600 animate-pulse">Running advanced AI models...</p>
 <p className="text-xs text-muted-foreground font-mono text-center max-w-xs">Connecting to {aiModel} via API stream</p>
 </div>
 ) : result ? (
 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-5 sm:p-8">
 <div className="prose prose-sm sm:prose-base dark:prose-invert prose-pre:bg-[#0d1117] prose-pre:border prose-pre:border-border prose-pre:shadow-xl max-w-none prose-headings:text-foreground dark:prose-headings:text-slate-200 prose-a:text-cyan-600">
 ...
 <p className="text-sm max-w-md mt-1 text-muted-foreground leading-relaxed">
