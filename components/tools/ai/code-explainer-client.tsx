  ...
  return (
    <div className="w-full min-h-screen pb-20 relative">
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 space-y-8 relative z-10">
        <ToolPageHeader
          title="AI Code Explainer & Multi-Language Converter Studio"
          description="Decode complex algorithms, convert languages, generate unit tests, and audit Big-O complexity with high-speed AI models."
          icon={Code2}
        />
          ...
          <div className="lg:col-span-5 space-y-4">
            <GlassCard className="p-0 overflow-hidden bg-background border-border shadow-sm rounded-2xl">
              ...
              <CardContent className="p-0">
                <textarea
                  className="w-full bg-background p-4 text-xs sm:text-sm outline-none resize-none min-h-[300px] font-mono leading-relaxed text-foreground focus:ring-0 border-0"
              ...
              <div className="space-y-3">
                ...
                {actionType === "convert" && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}>
                    ...
                    <select
                      className="w-full rounded-lg border border-primary/40 bg-primary/5 px-3 py-2 text-xs text-foreground font-medium outline-none focus:ring-2 focus:ring-primary/50"
            ...
            {history.length > 0 && (
              <GlassCard className="p-4 bg-background border-border shadow-sm rounded-2xl">
                ...
                <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1">
                  {history.map((item) => (
                    <div key={item.id} onClick={() => loadHistoryItem(item)} className="p-2.5 rounded-xl border border-border bg-muted/30 hover:bg-accent cursor-pointer transition-colors group">
            ...
            <GlassCard className="h-full min-h-[600px] flex flex-col p-0 overflow-hidden bg-background border-border shadow-sm rounded-2xl">
              <CardHeader className="border-b border-border bg-muted/40 p-4 shrink-0">
                <div className="flex justify-between items-center">
                  ...
                  {result && (
                    <Button variant="outline" size="sm" onClick={handleCopy} className="h-8 text-xs gap-1.5 border-border font-semibold">
              ...
              <CardContent className="p-0 flex-1 overflow-auto bg-card/40">
                {isProcessing ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center p-8 text-muted-foreground min-h-[400px]">
        ...
        <ToolFaqAccordion
          faqs={[
            { question: "Which languages are supported?", answer: "We support Python, TypeScript, JavaScript, Rust, Go, C++, Java, C#, and SQL." },
            { question: "Are my code snippets saved or shared?", answer: "No, your code is processed in real time and stored only locally in your browser's history." },
          ]}
        />
