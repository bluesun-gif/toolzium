...
export function SocialBioClient() {
  ...
  const presets = [{
    ...
    skills: "Next.js, TypeScript, AI APIs",
    cta: "alexchen.dev"
  }, {
    label: "🚀 SaaS Founder",
    name: "Sarah Jenkins",
    role: "AI Startup Founder",
    skills: "Bootstrapping, Product, Growth",
    cta: "buildwithsarah.com"
  }, {
    label: "🎨 UI/UX Designer",
    name: "David Kim",
    role: "Lead Product Designer",
    skills: "Figma, Design Systems, Mobile UX",
    cta: "davidkim.design"
  }];
  useEffect(() => {
    try {
            ...
            <div className="space-y-4 flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input placeholder="e.g. Alex Morgan" value={name} onChange={e => setName(e.target.value)} className="bg-background border-border" />
                </div>

                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-muted-foreground">Profession / Primary Role</Label>
                  <Input placeholder="e.g. Senior Frontend Engineer" value={profession} onChange={e => setProfession(e.target.value)} className="bg-background border-border" />
                </div>
              </div>
              ...
              <div>
                ...
                <Input placeholder="e.g. Next.js, UI Design, Open Source, AI APIs" value={keySkills} onChange={e => setKeySkills(e.target.value)} className="bg-background border-border" />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-muted-foreground">Target Vibe & Tone</Label>
                  <select className="w-full rounded-lg border border-border bg-background px-3 py-2 text-xs text-foreground font-medium outline-none focus:ring-2 focus:ring-primary/50" value={vibe} onChange={e => setVibe(e.target.value as any)}>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input placeholder="e.g. alexmorgan.dev" value={callToAction} onChange={e => setCallToAction(e.target.value)} className="bg-background border-border" />
                </div>
              </div>
              ...
              y: 0
                ...
                <GlassCard className="p-4 space-y-2 border-l-4 border-l-sky-500 bg-card/70 backdrop-blur-md rounded-2xl">
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="outline" size="sm" onClick={() => handleCopy(bios.twitter, "Twitter bio")} className="h-7 text-xs gap-1 border-border font-semibold">
                ...
                <GlassCard className="p-4 space-y-2 border-l-4 border-l-primary bg-card/70 backdrop-blur-md rounded-2xl">
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="outline" size="sm" onClick={() => handleCopy(bios.linkedin, "LinkedIn bio")} className="h-7 text-xs gap-1 border-border font-semibold">
                      <Copy className="w-3.5 h-3.5" /> Copy
                    </Button>
                  </div>
                ...
                <GlassCard className="p-4 space-y-2 border-l-4 border-l-pink-500 bg-card/70 backdrop-blur-md rounded-2xl">
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="outline" size="sm" onClick={() => handleCopy(bios.instagram, "Instagram bio")} className="h-7 text-xs gap-1 border-border font-semibold">
                      <Copy className="w-3.5 h-3.5" /> Copy
                    </Button>
                  </div>
                ...
                {bios.github && <GlassCard className="p-4 space-y-2 border-l-4 border-l-slate-400 bg-card/70 backdrop-blur-md rounded-2xl">
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="outline" size="sm" onClick={() => handleCopy(bios.github, "GitHub bio")} className="h-7 text-xs gap-1 border-border font-semibold">
                        <Copy className="w-3.5 h-3.5" /> Copy
                      </Button>
                    </div>
