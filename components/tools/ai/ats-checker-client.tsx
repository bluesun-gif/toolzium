...
export function AtsCheckerClient() {
  ...
  const presets = [
    { label: "💻 Full-Stack Dev", role: "Senior Full-Stack Engineer", resume: "Senior Full-Stack Engineer with 6 years experience building React, Next.js, Node.js, PostgreSQL, Docker, AWS applications.", jd: "Looking for Senior Full-Stack Engineer with expertise in Next.js, TypeScript, PostgreSQL, CI/CD pipelines, and cloud architecture." },
    { label: "📊 Product Manager", role: "Technical Product Manager", resume: "Product Manager leading cross-functional agile engineering teams, roadmap strategy, A/B testing, SQL analytics, user research.", jd: "Seeking Technical Product Manager with background in SaaS, SQL, Agile sprint planning, roadmap execution, and product analytics." },
  ];
    ...
    try {
      ...
      if (!generatedResult || typeof generatedResult.score !== "number") {
        generatedResult = {
          ...
          recommendations: [
            "Incorporate missing cloud infrastructure keywords in your experience section.",
            "Quantify bullet points with measurable metrics (e.g. 'Improved speed by 35%').",
            "Use clear standard section headers (Work Experience, Education, Skills)."
          ],
  ...
  return (
    <div className="w-full min-h-screen pb-20 relative">
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 space-y-8 relative z-10">
        <ToolPageHeader
          title="AI ATS Resume Keyword & Format Scanner"
          description="Audit your resume's ATS match score against target job postings, discover missing keywords, and optimize for recruiter screening algorithms."
          icon={FileSearch}
        />
          ...
          <GlassCard className="p-5 flex flex-col bg-background border-border shadow-sm rounded-2xl">
            ...
            <div className="space-y-4 flex-1">
              ...
              <div>
                ...
                <textarea
                  className="w-full rounded-xl border border-border bg-background p-3 text-xs sm:text-sm outline-none focus:ring-2 focus:ring-primary/50 min-h-[140px] font-sans text-foreground"
            ...
            {result ? (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                <GlassCard className="p-6 flex flex-col items-center justify-center text-center space-y-3 bg-card/70 backdrop-blur-md border-border rounded-2xl">
                  ...
                  <div
                    className={`text-4xl font-extrabold px-6 py-3 rounded-2xl border font-mono ${
                      result.score >= 80
                ...
                <GlassCard className="p-4 space-y-2 bg-card/70 backdrop-blur-md rounded-2xl border-border">
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {result.matchedKeywords.map((kw, i) => (
                      <span key={i} className="text-xs bg-primary/10 text-primary border border-primary/20 px-2.5 py-1 rounded-lg font-mono font-semibold">
                        ✓ {kw}
                      </span>
                    ))}
                  </div>
                  ...
                  <GlassCard className="p-4 space-y-2 bg-card/70 backdrop-blur-md rounded-2xl border-border">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {result.missingKeywords.slice(0, 10).map((kw, i) => (
                        <span key={i} className="text-xs bg-amber-500/10 text-amber-500 border border-amber-500/20 px-2.5 py-1 rounded-lg font-mono font-semibold">
                          + {kw}
                        </span>
                      ))}
                    </div>
        ...
        <ToolFaqAccordion
          faqs={[
            { question: "Is my resume saved or stored anywhere?", answer: "No. Your resume text is processed strictly inside your web browser and AI execution endpoint." },
