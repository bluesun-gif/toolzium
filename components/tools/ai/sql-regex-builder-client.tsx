...
export function SqlRegexBuilderClient() {
 ...
 <h1 className="text-xl sm:text-2xl font-black text-foreground">AI SQL Regex Query Builder</h1>
 ...
 <p className="text-xs sm:text-sm text-muted-foreground mt-1">Convert plain-English pattern requirements into SQL regular expressions for PostgreSQL, MySQL, BigQuery, and SQLite.</p>
 </div>
 </div>
 ...
 <Button variant="outline"size="sm"onClick={() => handleCopy(result.sqlQuery,"SQL query")} className="h-7 text-xs gap-1 border-border">
 <Copy className="w-3 h-3"/> Copy
 </Button>
 </div>
 <pre className="text-xs font-mono bg-slate-50 p-3 rounded-xl border border-border/60 text-emerald-700 dark:text-emerald-400 font-bold whitespace-pre-wrap">{result.sqlQuery}</pre>
 </GlassCard>

 <GlassCard className="p-4 space-y-3">
 <div className="flex justify-between items-center border-b border-slate-100 pb-2">
 <span className="text-xs font-extrabold text-foreground dark:text-slate-200 uppercase tracking-wider font-mono">
 Raw Regex Pattern
 </span>
 <Button variant="outline"size="sm"onClick={() => handleCopy(result.regexPattern,"Regex pattern")} className="h-7 text-xs gap-1 border-border">
 <Copy className="w-3 h-3"/> Copy
 </Button>
 </div>
 <p className="text-xs font-mono bg-slate-50 p-2.5 rounded-lg border border-border/60 text-foreground dark:text-slate-200 truncate">{result.regexPattern}</p>
 </GlassCard>
 ...
 <p className="text-xs max-w-xs mt-1 text-muted-foreground">Describe your matching requirements on the left to construct database regular expressions.</p>
 </GlassCard>
 )}
 </div>
 </div>
