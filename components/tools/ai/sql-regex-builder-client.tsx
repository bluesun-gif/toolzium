...
export function SqlRegexBuilderClient() {
  ...
  const presets = [
    { label: "📧 Valid Email Addresses", text: "Match valid email addresses with domain extensions", col: "email", tbl: "users" },
    { label: "📱 E.164 Phone Numbers", text: "Match international phone numbers in E.164 format with country codes", col: "phone_number", tbl: "customers" },
    { label: "🌐 IPv4 Addresses", text: "Match standard IPv4 addresses in dot-decimal notation", col: "ip_address", tbl: "audit_logs" },
    { label: "🔑 UUID v4 Keys", text: "Match valid 36-character UUID v4 strings", col: "session_id", tbl: "user_sessions" },
  ];
  ...
  return (
    <div className="w-full min-h-screen pb-20 relative">
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 space-y-8 relative z-10">
        <ToolPageHeader
          title="AI SQL Regex Query & Pattern Builder"
          description="Convert plain-English pattern requirements into executable SQL regular expressions for PostgreSQL, MySQL, BigQuery, and SQLite."
          icon={Database}
        />
          ...
          <div className="flex flex-col space-y-4">
            {result ? (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                <GlassCard className="p-5 space-y-3 border-l-4 border-l-primary bg-card/70 backdrop-blur-md rounded-2xl">
                  <div className="flex justify-between items-center border-b border-border/60 pb-2">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleCopy(result.sqlQuery, "SQL query")}
                ...
                <GlassCard className="p-4 space-y-3 bg-card/70 backdrop-blur-md rounded-2xl border-border">
                  <div className="flex justify-between items-center border-b border-border/60 pb-2">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleCopy(result.regexPattern, "Regex pattern")}
                ...
                {result.testMatches && result.testMatches.length > 0 && (
                  <GlassCard className="p-4 space-y-2 bg-card/70 backdrop-blur-md rounded-2xl border-border">
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      {result.testMatches.map((m, idx) => (
                        <span
                          key={idx}
                          className="text-xs bg-primary/10 text-primary border border-primary/20 px-3 py-1 rounded-lg font-mono font-semibold"
                        >
                          ✓ {m}
                        </span>
                      ))}
