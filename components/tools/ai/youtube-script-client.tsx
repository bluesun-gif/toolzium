...
export function YoutubeScriptClient() {
 ...
 <h1 className="text-xl sm:text-2xl font-black text-foreground">AI YouTube Script & Teleprompter Generator</h1>
 ...
 <p className="text-xs sm:text-sm text-muted-foreground mt-1">Craft viral YouTube titles, 15-second opening retention hooks, timestamp outlines, and teleprompter-ready scripts.</p>
 </div>
 </div>
 ...
 {/* Presets */}
 <div>
 <Label className="text-[11px] mb-1.5 block text-muted-foreground font-semibold uppercase tracking-wider flex items-center gap-1">
 ...
 <textarea
 className="w-full rounded-xl border border-border bg-background p-3 text-sm outline-none focus:ring-2 focus:ring-red-500 min-h-[90px] font-sans text-foreground"
 ...
 <label className="flex items-center gap-2 p-2 rounded-xl bg-slate-50 border border-border/60 cursor-pointer font-medium text-foreground dark:text-slate-200">
 <input
 type="checkbox"
 ...
 <Button variant="outline"size="sm"onClick={downloadSrt} className="h-8 text-xs gap-1 border-border">
 ...
 <div className="flex gap-1 p-1 bg-slate-100 rounded-xl border border-border overflow-x-auto text-xs">
 <button
 onClick={() => setActiveTab("teleprompter")}
 className={`px-3 py-1.5 rounded-lg font-bold transition-all shrink-0 ${activeTab ==="teleprompter"?"bg-background text-red-600 shadow-sm":"text-slate-600"}`}
 >
 Teleprompter Script
 </button>
 <button
 onClick={() => setActiveTab("titles")}
 className={`px-3 py-1.5 rounded-lg font-bold transition-all shrink-0 ${activeTab ==="titles"?"bg-background text-red-600 shadow-sm":"text-slate-600"}`}
 >
 Viral Titles (5)
 </button>
 <button
 onClick={() => setActiveTab("hooks")}
 className={`px-3 py-1.5 rounded-lg font-bold transition-all shrink-0 ${activeTab ==="hooks"?"bg-background text-red-600 shadow-sm":"text-slate-600"}`}
 >
 Retention Hooks
 </button>
 <button
 onClick={() => setActiveTab("outline")}
 className={`px-3 py-1.5 rounded-lg font-bold transition-all shrink-0 ${activeTab ==="outline"?"bg-background text-red-600 shadow-sm":"text-slate-600"}`}
 >
 Timestamps
 </button>
 <button
 onClick={() => setActiveTab("seo")}
 className={`px-3 py-1.5 rounded-lg font-bold transition-all shrink-0 ${activeTab ==="seo"?"bg-background text-red-600 shadow-sm":"text-slate-600"}`}
 >
 YouTube SEO
 </button>
 </div>
 ...
 <Button variant="outline"size="sm"onClick={() => handleCopy(script.bodyScript,"Teleprompter Script")} className="h-7 text-xs gap-1 border-border">
 <Copy className="w-3.5 h-3.5"/> Copy
 </Button>
 </div>
 <pre className="text-xs font-sans bg-slate-50 p-4 rounded-xl border border-border/60 whitespace-pre-wrap leading-relaxed max-h-[320px] overflow-y-auto text-foreground dark:text-slate-200">{script.bodyScript}</pre>
 </GlassCard>
 )}
 ...
 <Button variant="outline"size="sm"onClick={() => handleCopy(script.titles.map(t => t.title).join("\n"),"Titles")} className="h-7 text-xs gap-1 border-border">
 <Copy className="w-3.5 h-3.5"/> Copy All
 </Button>
 </div>
 <div className="space-y-2">
 {script.titles.map((t, idx) => (
 <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-border/60 text-xs flex justify-between items-center">
 ...
 <Button variant="outline"size="sm"onClick={() => handleCopy(h.text,"Hook")} className="h-7 text-xs gap-1 border-border">
 <Copy className="w-3 h-3"/> Copy
 </Button>
 </div>
 <p className="text-xs text-foreground dark:text-slate-200 font-medium leading-relaxed">{h.text}</p>
 </GlassCard>
 ))}
 </div>
 )}
 ...
 <Button variant="outline"size="sm"onClick={() => handleCopy(script.outline,"Outline")} className="h-7 text-xs gap-1 border-border">
 <Copy className="w-3.5 h-3.5"/> Copy
 </Button>
 </div>
 <pre className="text-xs font-mono bg-slate-50 p-3 rounded-xl border border-border/60 whitespace-pre-wrap leading-relaxed text-foreground dark:text-slate-200">{script.outline}</pre>
 </GlassCard>
 )}
 ...
 <Button variant="outline"size="sm"onClick={() => handleCopy(`${script.descriptionSeo}\n\nTAGS:\n${script.tags.join(",")}`,"SEO Package")} className="h-7 text-xs gap-1 border-border">
 <Copy className="w-3.5 h-3.5"/> Copy SEO Package
 </Button>
 </div>
 <pre className="text-xs font-sans bg-slate-50 p-3 rounded-xl border border-border/60 whitespace-pre-wrap text-foreground dark:text-slate-200">{script.descriptionSeo}</pre>
 <div className="flex flex-wrap gap-1 pt-2">
 {script.tags.map((t, idx) => (
 <span key={idx} className="text-[10px] bg-slate-100 text-slate-700 dark:text-slate-300 px-2 py-0.5 rounded-full font-mono">
 #{t}
 </span>
 ))}
 </div>
 ...
 <div className="flex justify-between items-center border-b border-slate-100 pb-2">
 <span className="text-xs font-bold text-foreground dark:text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
 ...
 {history.map((item) => (
 <div key={item.id} className="p-3 bg-slate-50 rounded-xl border border-border/60 text-xs flex justify-between items-center">
 <div className="truncate max-w-[80%]">
 <span className="font-semibold text-foreground truncate block">{item.topic}</span>
 <span className="text-[10px] text-muted-foreground">{item.timestamp} · {item.niche.toUpperCase()} · {item.result.estimatedDuration}</span>
 </div>
 <Button variant="outline"size="sm"onClick={() => { setScript(item.result); setTopic(item.topic); setNiche(item.niche); }} className="h-7 text-xs px-2 border-border">
 Reload
 </Button>
 </div>
 ))}
