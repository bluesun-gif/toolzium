  ...
  return (
    <div className="w-full min-h-screen pb-20 relative">
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 space-y-8 relative z-10">
        <ToolPageHeader
          icon={Youtube}
          title="AI YouTube Script & Teleprompter Generator Studio"
          description="Craft viral YouTube titles, 15-second opening retention hooks, timestamp outlines, teleprompter-ready scripts, and YouTube SEO packages."
        />
          ...
          <GlassCard className="p-5 flex flex-col bg-background border-border shadow-sm rounded-2xl">
            ...
            <div className="space-y-4 flex-1">
              ...
              <div>
                ...
                <textarea
                  className="w-full rounded-xl border border-border bg-background p-3.5 text-xs sm:text-sm outline-none focus:ring-2 focus:ring-primary/50 min-h-[90px] font-sans text-foreground"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-xs text-foreground font-medium outline-none focus:ring-2 focus:ring-primary/50"
                    value={niche}
            ...
            {script ? (
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                ...
                <div className="flex gap-1 p-1 bg-muted/60 rounded-xl border border-border overflow-x-auto text-xs">
                  <button
                    ...
                    className={cn(
                      ...
                      activeTab === "teleprompter" ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:text-foreground"
                    )}
                  >
                    Teleprompter Script
                  </button>
                  <button
                    ...
                    className={cn(
                      "px-3 py-1.5 rounded-lg font-bold transition-all shrink-0 cursor-pointer",
                      activeTab === "titles" ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:text-foreground"
                    )}
                  >
                    Viral Titles (5)
                  </button>
                  <button
                    ...
                    className={cn(
                      "px-3 py-1.5 rounded-lg font-bold transition-all shrink-0 cursor-pointer",
                      activeTab === "hooks" ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:text-foreground"
                    )}
                  >
                    Retention Hooks
                  </button>
                  <button
                    ...
                    className={cn(
                      "px-3 py-1.5 rounded-lg font-bold transition-all shrink-0 cursor-pointer",
                      activeTab === "outline" ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:text-foreground"
                    )}
                  >
                    Timestamps
                  </button>
                ...
                {activeTab === "teleprompter" && (
                  <GlassCard className="p-4 space-y-3 border-l-4 border-l-primary bg-card/70 backdrop-blur-md rounded-2xl">
                  ...
                  <GlassCard className="p-4 space-y-3 border-l-4 border-l-primary bg-card/70 backdrop-blur-md rounded-2xl">
                    ...
                    <div className="space-y-2">
                      {script.titles.map((t, idx) => (
                        <div key={idx} className="p-3 bg-muted/30 rounded-xl border border-border text-xs flex justify-between items-center">
                ...
                {activeTab === "hooks" && (
                  <div className="space-y-3">
                    {script.hooks.map((h, idx) => (
                      <GlassCard key={idx} className="p-4 space-y-2 border-l-4 border-l-amber-500 bg-card/70 backdrop-blur-md rounded-2xl">
                ...
                {activeTab === "outline" && (
                  <GlassCard className="p-4 space-y-3 border-l-4 border-l-sky-500 bg-card/70 backdrop-blur-md rounded-2xl">
                    <div className="flex justify-between items-center border-b border-border pb-2">
                ...
                {activeTab === "seo" && (
                  <GlassCard className="p-4 space-y-3 border-l-4 border-l-emerald-500 bg-card/70 backdrop-blur-md rounded-2xl">
                    <div className="flex justify-between items-center border-b border-border pb-2">
                    ...
                    <div className="flex flex-wrap gap-1 pt-2">
                      {script.tags.map((t, idx) => (
                        <span key={idx} className="text-[10px] bg-muted/60 text-muted-foreground px-2 py-0.5 rounded-full font-mono font-semibold">
                          #{t}
                        </span>
                      ))}
                    </div>
        ...
        {history.length > 0 && (
          <GlassCard className="p-5 bg-background border-border shadow-sm rounded-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-[220px] overflow-y-auto pr-1">
              {history.map((item) => (
                <div key={item.id} className="p-3.5 bg-muted/30 rounded-xl border border-border text-xs flex justify-between items-center gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => { setScript(item.result); setTopic(item.topic); setNiche(item.niche); }} className="h-8 text-xs px-2.5 border-border font-semibold">
                    Reload
                  </Button>
                </div>
              ))}
