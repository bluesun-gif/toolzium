...
export function StartupNameClient() {
 ...
 <h1 className="text-xl sm:text-2xl font-black text-foreground">AI Startup Name & Brand Generator</h1>
 ...
 <p className="text-xs sm:text-sm text-muted-foreground mt-1">Generate catchy, brandable startup names, taglines, and domain suggestions based on your business keywords.</p>
 </div>
 </div>
 ...
 <Button variant="outline"size="sm"onClick={() => handleCopy(item.name,"Startup name")} className="h-8 text-xs gap-1 border-border">
 <Copy className="w-3.5 h-3.5"/> Copy
 </Button>
 </div>
 <p className="text-xs text-slate-600 font-medium">{item.tagline}</p>
 <div className="flex items-center justify-between pt-1">
 <span className="text-xs font-mono font-semibold text-muted-foreground">{item.domainSuggestion}</span>
 <a
 ...
 <p className="text-xs max-w-xs mt-1 text-muted-foreground">Enter your startup keywords on the left to brainstorm brandable business names and taglines.</p>
 </GlassCard>
 )}
 </div>
 </div>
