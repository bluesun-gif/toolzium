...
export function ElectricityMonitorClient() {
  ...
  useEffect(() => {
    localStorage.setItem("electricityMonitor", JSON.stringify({
      appliances,
      rate
    }));
  }, [appliances, rate]);
 ...
 {PRESETS.map(p => <Button key={p.name} variant="outline" size="sm" onClick={() => {
                setNewName(p.name);
 ...
 {appliances.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4">No appliances added yet</p> : appliances.map(a => <div key={a.id} className="flex justify-between items-center bg-muted/50 p-2 rounded-lg">
 <div>
 ...
 {appliances.some(a => a.name.toLowerCase().includes("ac")) && <li>Set your AC thermostat a few degrees higher to save on cooling costs.</li>}
 </ul>
 </CardContent>
 </GlassCard>}
 </div>
