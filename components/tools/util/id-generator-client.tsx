...
function fallbackUuidV4() {
  ...
  a[6] = a[6] & 0x0f | 0x40;
  a[8] = a[8] & 0x3f | 0x80;
  const s = Array.from(a).map(b => b.toString(16).padStart(2, "0")).join("");
...
export default function IdGeneratorClient() {
 ...
 <SelectField label="Type" value={mode} onValueChange={v => setMode(v as Mode ?? "uuid")} options={[{
            label: "UUID v4",
            value: "uuid"
          }, {
            label: "ULID",
 ...
 <SelectField label="Copy All Separator" value={separator} onValueChange={v => setSeparator(isSeparator(v) ? v : "newline")} options={[{
            label: "New line",
            value: "newline"
          }, {
            label: "Comma",
 ...
 {/* Per-mode options */}
 {mode === "uuid" && <div className="grid gap-2">
 ...
 {mode === "ulid" && <div className="grid gap-2">
 ...
 {mode === "hex" && <InputField label="Length (chars)" type="number" min={4} max={128} value={hexLen} onChange={e => setHexLen(Math.max(4, Math.min(128, Number(e.target.value) || 16)))} />}
 ...
 {processed.length === 0 ? <p className="text-sm text-muted-foreground">
 No IDs yet. Configure settings and click Generate.
 </p> : <div className="grid gap-3 md:grid-cols-2">
 {processed.map(id => <div key={id} className="flex items-center justify-between rounded-md border p-3">
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: Copy
      }]} badges={["UUID v4 & v1", "ULID & NanoID", "Bulk generation"]} />
 ...
 <tbody>
 {[["UUID v4", "36 chars", "No", "No (has hyphens)", "Database PKs, most use cases"], ["UUID v1", "36 chars", "Yes (time-based)", "No", "Time-ordered records"], ["ULID", "26 chars", "Yes (monotonic)", "Yes", "DB indexes, event logs"], ["NanoID", "21 chars", "No", "Yes", "URL slugs, user-facing IDs"], ["CUID", "25+ chars", "Yes (time-prefix)", "Yes", "Distributed systems"], ["CUID2", "24 chars", "No", "Yes", "Secure distributed IDs"], ["ObjectID (MongoDB)", "24 hex chars", "Yes", "Yes", "MongoDB primary keys"]].map(([fmt, len, sort, url, best]) => <tr key={fmt} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["Uniqueness", "Unique within one table", "Globally unique across systems"], ["Sortability", "Naturally sortable", "Not sortable by insertion time"], ["Security", "Enumerable (predictable)", "Non-enumerable (unpredictable)"], ["Distributed systems", "Requires central counter", "Generated client-side"], ["Database size", "4 bytes (int)", "16 bytes"], ["URL exposure", "Reveals record count", "No information exposed"]].map(([prop, auto, uuid]) => <tr key={prop} className="odd:bg-muted/20">
