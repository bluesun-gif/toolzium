...
export function DecisionMakerClient() {
 ...
 <Label>Simple</Label>
 <Switch checked={mode === "weighted"} onCheckedChange={c => setMode(c ? "weighted" : "simple")} />
 <Label>Weighted</Label>
 </div>
 </div>
 </CardHeader>
 <CardContent>
 {mode === "simple" ? <div className="space-y-4">
 {simpleOptions.map((opt, i) => <div key={i} className="flex items-center gap-2">
 <Input value={opt} onChange={e => {
                const newOpts = [...simpleOptions];
                newOpts[i] = e.target.value;
                setSimpleOptions(newOpts);
              }} placeholder={`Option ${i + 1}`} />
 {simpleOptions.length > 2 && <Button variant="ghost" size="icon" onClick={() => setSimpleOptions(simpleOptions.filter((_, idx) => idx !== i))}>
 ...
 <ActionButton onClick={handleSimpleDecide} icon={Shuffle} label="Decide for me" />
 
 {simpleResult && <div className={cn("p-6 bg-primary/10 rounded-lg text-center transition-all", isSpinning ? 'animate-pulse scale-105' : 'scale-100')}>
 ...
 {weightedOptions.map((opt, i) => <div key={opt.id} className="grid grid-cols-1 md:grid-cols-4 gap-2 items-center">
 <div className="md:col-span-2 flex items-center gap-2">
 {weightedOptions.length > 2 && <Button variant="ghost" size="icon" onClick={() => setWeightedOptions(weightedOptions.filter(o => o.id !== opt.id))}>
 <Trash className="w-4 h-4" />
 </Button>}
 ...
 <Input type="number" min={1} max={10} value={opt.prosScore} onChange={e => {
                  const newOpts = [...weightedOptions];
 ...
 <Input type="number" min={1} max={10} value={opt.consScore} onChange={e => {
                  const newOpts = [...weightedOptions];
 ...
 <Button variant="outline" onClick={() => setWeightedOptions([...weightedOptions, {
 ...
 <div className="pt-4 flex flex-col items-center gap-4">
 <ActionButton onClick={handleWeightedDecide} icon={Scale} label="Calculate Best Option" />
 
 {weightedResult && <div className="p-6 bg-primary/10 rounded-lg text-center">
 ...
 {history.map((h, i) => <li key={i} className="flex items-center gap-2">
