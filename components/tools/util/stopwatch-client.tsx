...
export default function StopwatchClient() {
  ...
  const copyLaps = () => {
    ...
    const text = laps.map(lap => `Lap ${lap.id}: ${formatTime(lap.time)} (Total: ${formatTime(lap.overall)})`).join("\n");
 ...
 <Button size="lg" variant={isRunning ? "destructive" : "default"} className="flex-1 min-w-32 h-16 text-lg" onClick={handleStartStop}>
 {isRunning ? <><Square className="mr-2 h-5 w-5 fill-current" /> Stop</> : <><Play className="mr-2 h-5 w-5 fill-current" /> Start</>}
 </Button>
 
 <Button size="lg" variant="secondary" className="flex-1 min-w-32 h-16 text-lg" onClick={handleLap} disabled={!isRunning}>
 <Flag className="mr-2 h-5 w-5" /> Lap
 </Button>
 
 <Button size="lg" variant="outline" className="flex-1 min-w-32 h-16 text-lg" onClick={handleReset} disabled={time === 0}>
 ...
 {laps.length === 0 ? <div className="absolute inset-0 flex items-center justify-center text-muted-foreground p-6 text-center">
 No laps recorded yet. Start the timer and click Lap to record.
 </div> : <ScrollArea className="h-full absolute inset-0">
 <div className="space-y-4 px-6 pb-6 pt-2">
 {laps.map(lap => <div key={lap.id} className="flex justify-between items-center py-2 border-b last:border-0">
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: RotateCcw
      }]} badges={["Millisecond precision", "Lap recording", "Keyboard shortcuts"]} />
 ...
 <tbody>
 {[["Lap 1", "1:23.45", "1:23.45"], ["Lap 2", "1:21.87", "2:45.32"], ["Lap 3", "1:24.10", "4:09.42"], ["Lap 4", "1:19.95", "5:29.37"]].map(([lap, lapTime, split]) => <tr key={lap} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["Running / Athletics", "Sprint times, lap splits", "0.01 seconds"], ["Swimming", "50m / 100m lap times", "0.01 seconds"], ["Cooking", "Cooking intervals, resting time", "Seconds"], ["Presentations", "Talk rehearsal timing", "Seconds"], ["Software dev", "Function benchmark, test runtime", "Milliseconds"], ["Science lab", "Reaction timing, experiment duration", "Milliseconds"], ["Language learning", "Timed reading or speaking drills", "Seconds"], ["Game speed runs", "Route segment times", "Milliseconds"]].map(([field, what, precision]) => <tr key={field} className="odd:bg-muted/20">
