...
export default function CoinFlipClient() {
 ...
 {/* The Coin */}
 <div className="relative w-56 h-56 cursor-pointer" onClick={flipCoin} style={{
              perspective: "1000px"
            }}>
 <div className="w-full h-full rounded-full transition-transform duration-1000 ease-out" style={{
                transform: `rotateY(${rotation}deg)`,
                transformStyle: "preserve-3d"
              }}>
 {/* Heads Side */}
 <div className="absolute inset-0 flex items-center justify-center rounded-full bg-gradient-to-br from-yellow-300 to-yellow-600 border-8 border-yellow-700 shadow-[inset_0_0_20px_rgba(0,0,0,0.4),_0_10px_20px_rgba(0,0,0,0.3)]" style={{
 ...
 {/* Tails Side */}
 <div className="absolute inset-0 flex items-center justify-center rounded-full bg-gradient-to-br from-slate-200 to-slate-400 border-8 border-slate-500 shadow-[inset_0_0_20px_rgba(0,0,0,0.4),_0_10px_20px_rgba(0,0,0,0.3)]" style={{
 ...
 <Button size="lg" className="w-full mt-8 font-bold text-lg h-14" onClick={flipCoin} disabled={isFlipping}>
 ...
 {history.length > 0 ? history.map((result, i) => <Badge key={i} className={cn("text-sm px-3 py-1", result === "Heads" ? "bg-yellow-500 hover:bg-yellow-600 text-primary-foreground border-none shadow-sm" : "bg-slate-500 hover:bg-slate-600 text-primary-foreground border-none shadow-sm")}>
 {result === "Heads" ? "H" : "T"}
 </Badge>) : <div className="w-full text-center py-8 text-sm text-muted-foreground border-2 border-dashed rounded-lg">
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: Shuffle
      }]} badges={["Cryptographically random", "Streak tracking", "Bulk flip mode"]} />
 ...
 <tbody>
 {[["2 in a row", "25%", "1 in 4"], ["3 in a row", "12.5%", "1 in 8"], ["4 in a row", "6.25%", "1 in 16"], ["5 in a row", "3.125%", "1 in 32"], ["6 in a row", "1.56%", "1 in 64"], ["7 in a row", "0.78%", "1 in 128"], ["10 in a row", "0.098%", "1 in 1,024"], ["20 in a row", "~0.0001%", "1 in 1,048,576"]].map(([streak, prob, odds]) => <tr key={streak} className="odd:bg-muted/20">
