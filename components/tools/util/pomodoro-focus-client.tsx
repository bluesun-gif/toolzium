...
export default function PomodoroFocusClient() {
  ...
  const ping = React.useCallback((title: string, body: string) => {
  ...
  /* Session Complete */
  const onCompleteSession = React.useCallback((skipped = false) => {
    setRunning(false);
    setStartedAt(null);
    setHistory(prev => [{
  ...
  }, [startedAt, targetMs, remainingMs, mode, cycleCount, sessionsUntilLong, longMin, shortMin, workMin, autoStartBreaks, autoStartWork, ping]);
  ...
  const toggle = React.useCallback(() => {
    if (running) {
      ...
      if (!askedNotifRef.current && typeof window !== "undefined" && "Notification" in window && Notification.permission === "default") {
  ...
  const resetTimer = React.useCallback((nextMode?: Mode) => {
    setRunning(false);
 ...
 <ActionButton variant="default" icon={running ? Pause : Play} label={running ? "Pause" : "Start"} onClick={toggle} />
 </>} />
 ...
 <svg className="h-full w-full" role="img" aria-label="Pomodoro progress ring" viewBox="0 0 100 100">
 <circle cx="50" cy="50" r="44" stroke="hsl(var(--muted))" strokeWidth="8" fill="none" />
 <circle cx="50" cy="50" r="44" stroke="hsl(var(--primary))" strokeWidth="8" fill="none" strokeDasharray={2 * Math.PI * 44} strokeDashoffset={(100 - progress) / 100 * 2 * Math.PI * 44} strokeLinecap="round" transform="rotate(-90 50 50)" />
 </svg>
 ...
 <ActionButton variant="default" size="sm" icon={running ? Pause : Play} label={running ? "Pause" : "Start"} onClick={toggle} />
 </div>
 </div>
 ...
 <InputField label="Work (min)" type="number" min={1} max={180} value={workMin} onChange={e => setWorkMin(clamp(parseInt(e.target.value || "0", 10), 1, 180))} />
 <InputField label="Short (min)" type="number" min={1} max={60} value={shortMin} onChange={e => setShortMin(clamp(parseInt(e.target.value || "0", 10), 1, 60))} />
 <InputField label="Long (min)" type="number" min={5} max={90} value={longMin} onChange={e => setLongMin(clamp(parseInt(e.target.value || "0", 10), 5, 90))} />
 </div>
 <div className="grid grid-cols-2 gap-3">
 <div className="space-y-2">
 <Label>Mode</Label>
 <Select value={mode} onValueChange={(v: Mode) => {
                    setMode(v);
                    setRunning(false);
 ...
 <InputField label="Until Long Break" type="number" min={2} max={12} value={sessionsUntilLong} onChange={e => setSessionsUntilLong(clamp(parseInt(e.target.value || "0", 10), 2, 12))} />
 </div>
 <div>
 ...
 <SwitchRow label="Auto-start breaks" checked={autoStartBreaks} onCheckedChange={setAutoStartBreaks} />
 <SwitchRow label="Auto-start work" checked={autoStartWork} onCheckedChange={setAutoStartWork} />
 </div>
 </div>
 ...
 <ActionButton size="icon" onClick={() => setSoundOn(s => !s)} aria-label="Toggle sound" icon={soundOn ? Volume2 : VolumeX} />
 </div>
 </div>
 </div>
 </div>
 ...
 </p> : <ul className="grid gap-2">
 {history.slice(0, 20).map(h => <li key={h.id} className={cn("flex items-center justify-between rounded-md border p-3", h.mode === "work" ? "bg-primary/5" : "bg-muted/50")}>
 <div className="flex items-center gap-2">
 {h.mode === "work" ? <Zap className="h-4 w-4" /> : <Coffee className="h-4 w-4" />}
 <span className="text-sm font-medium">
 {h.mode === "work" ? "Focus" : h.mode === "short" ? "Short Break" : "Long Break"}
 </span>
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: BarChart3
      }]} badges={["25/5 min intervals", "Long break after 4 rounds", "Browser tab timer"]} />
 ...
 <tbody>
 {[["1", "Choose a task to work on", "Before starting"], ["2", "Set timer and work with full focus", "25 minutes"], ["3", "Stop when timer rings; mark one Pomodoro", "0 minutes"], ["4", "Take a short break (rest, stretch, hydrate)", "5 minutes"], ["5", "Every 4 Pomodoros: take a long break", "15-30 minutes"]].map(([step, action, duration]) => <tr key={step} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["Classic Pomodoro", "25 min", "5 min", "Most knowledge workers"], ["52/17 Rule", "52 min", "17 min", "Deep focus tasks"], ["90-min Ultradian", "90 min", "20 min", "Creative work, writing"], ["Short Burst", "15 min", "3 min", "ADHD, low-energy days"], ["Extended Focus", "50 min", "10 min", "Complex problem-solving"]].map(([style, work, brk, best]) => <tr key={style} className="odd:bg-muted/20">
