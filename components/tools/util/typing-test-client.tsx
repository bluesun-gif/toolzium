...
const SAMPLE_TEXTS = {
  easy: ["the quick brown fox jumps over the lazy dog", "hello world this is a simple typing test for beginners", "practice makes perfect keep typing to improve your speed", "a short sentence is good for warming up your fingers", "water is essential for life on earth and human health", "reading a good book can improve your vocabulary immensely", "never give up on your dreams no matter how hard it gets", "the sun shines bright in the beautiful blue summer sky", "typing games are a fun way to practice your keyboard skills", "always remember to take breaks when working on a computer"],
  medium: ["Typing speed tests are a great way to measure your progress and improve your keyboard skills over time.", "The ability to type quickly and accurately can save you hours of work each week, increasing your productivity.", "When learning to touch type, it is more important to focus on accuracy rather than speed in the beginning.", "A mechanical keyboard provides tactile feedback which many typists find satisfying and helpful for their speed.", "Remember to maintain good posture while typing to prevent back pain and reduce strain on your wrists.", "Consistent practice for just fifteen minutes a day can dramatically increase your overall words per minute.", "Look at the screen instead of your hands while typing to train your muscle memory more effectively.", "Different keyboard layouts like Dvorak and Colemak were designed to minimize finger movement and increase efficiency.", "In the modern digital age, being proficient at typing is almost as essential as being able to write by hand.", "There are many free online resources available to help you master touch typing from the comfort of your home."],
  hard: ["function calculateSpeed(chars, timeInSeconds) { return Math.round((chars / 5) / (timeInSeconds / 60)); }", "const fetchData = async (url) => { try { const res = await fetch(url); return await res.json(); } catch(e) { console.error(e); } };", "import React, { useState, useEffect } from 'react'; export default function App() { const [count, setCount] = useState(0); return <div/>; }", "SELECT users.name, orders.total FROM users INNER JOIN orders ON users.id = orders.user_id WHERE orders.status = 'completed';", "def bubble_sort(arr): n = len(arr); for i in range(n-1): for j in range(0, n-i-1): if arr[j] > arr[j+1]: arr[j], arr[j+1] = arr[j+1], arr[j]", "document.addEventListener('DOMContentLoaded', () => { const elements = document.querySelectorAll('.item'); elements.forEach(el => el.classList.add('active')); });", "type User = { id: string; username: string; email: string; createdAt: Date; isActive: boolean; };", "public static void main(String[] args) { System.out.println(\"Hello, World!\"); ArrayList<String> list = new ArrayList<>(); }", "interface DatabaseResponse<T> { data: T[]; totalCount: number; hasNextPage: boolean; error: Error | null; }", "sudo apt-get update && sudo apt-get upgrade -y && sudo systemctl restart nginx.service"]
};
...
export default function TypingTestClient() {
  ...
  const renderText = () => {
    return targetText.split("").map((char, index) => {
      ...
      return <span key={index} className={cn("text-2xl md:text-3xl font-mono transition-colors", colorClass)}>
 {char === "" ? "" : char}
 </span>;
    });
  };
 ...
 <Select value={difficulty} onValueChange={(val: Difficulty) => {
                  startNewTest(val);
 ...
 <div className="relative p-6 bg-card border rounded-lg min-h-[200px] cursor-text" onClick={() => inputRef.current?.focus()}>
 <div className="absolute inset-0 p-6 pointer-events-none select-none break-words whitespace-pre-wrap leading-relaxed">
 {renderText()}
 </div>
 <textarea ref={inputRef} value={userInput} onChange={handleInputChange} className="absolute inset-0 w-full h-full opacity-0 resize-none z-10 cursor-text" disabled={status === "finished"} autoFocus spellCheck={false} />
 {status === "idle" && userInput.length === 0 && <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-muted-foreground text-sm flex items-center gap-2 animate-pulse">
 ...
 {status === "finished" && <Card className="border-primary/50 bg-primary/5 shadow-sm animate-in fade-in slide-in-from-bottom-4">
 <CardHeader className="pb-2">
 <CardTitle className="text-lg flex items-center gap-2">
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: BarChart3
      }]} badges={["WPM measurement", "Accuracy tracking", "Multiple difficulty levels"]} />
 ...
 <tbody>
 {[["Beginner", "10-30 WPM", "< 90%", "Two-finger hunt & peck"], ["Below average", "30-40 WPM", "90%+", "Developing typist"], ["Average adult", "40-55 WPM", "92%+", "General computer user"], ["Good typist", "55-80 WPM", "95%+", "Office worker"], ["Fast typist", "80-100 WPM", "97%+", "Professional, programmer"], ["Expert typist", "100-130 WPM", "98%+", "Secretary, transcriptionist"], ["World-class", "130-200+ WPM", "99%+", "Competitive typist"]].map(([level, wpm, acc, context]) => <tr key={level} className="odd:bg-muted/20">
