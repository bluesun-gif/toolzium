...
export default function PdfToImageClient() {
  ...
  useEffect(() => {
    ...
    } else if (typeof window !== "undefined" && (window as unknown as {
      pdfjsLib?: unknown;
    }).pdfjsLib) {
      setPdfjsLoaded(true);
    }
  ...
  const handleDownloadZip = async () => {
    ...
    try {
      ...
      pages.forEach(page => {
        ...
        zip.file(`page-${page.pageNumber}.${ext}`, base64Data, {
          base64: true
        });
      });
 ...
 <Button variant={format === "image/jpeg" ? "default" : "outline"} className="flex-1" onClick={() => setFormat("image/jpeg")}>
 JPG
 </Button>
 <Button variant={format === "image/png" ? "default" : "outline"} className="flex-1" onClick={() => setFormat("image/png")}>
 PNG
 </Button>
 </div>
 </div>
 ...
 <div className="flex gap-2">
 {[72, 150, 300].map(val => <Button key={val} variant={dpi === val ? "default" : "outline"} className="flex-1 text-xs" onClick={() => setDpi(val)}>
 {val}
 </Button>)}
 </div>
 ...
 <div className="flex gap-2">
 <Button className="flex-1" onClick={handleConvert} disabled={isConverting || !pdfjsLoaded}>
 ...
 {error && <div className="p-4 bg-destructive/10 text-destructive rounded-lg flex items-start gap-3 border border-destructive/20">
 <AlertCircle className="h-5 w-5 shrink-0 mt-0.5" />
 <div className="text-sm">{error}</div>
 </div>}

 {!file ? <div {...getRootProps()} className={cn("border-2 border-dashed rounded-xl p-12 text-center transition-colors cursor-pointer flex flex-col items-center justify-center min-h-[400px]", isDragActive ? "border-primary bg-primary/5" : "border-muted-foreground/25 hover:border-primary/50")}>
 <input {...getInputProps()} />
 ...
 </div> : <GlassCard className="min-h-[400px]">
 ...
 {pages.length > 0 && <ActionButton onClick={handleDownloadZip} icon={Download} label="Download ZIP" />}
 </CardHeader>
 <CardContent>
 {pages.length > 0 ? <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
 {pages.map(page => <div key={page.pageNumber} className="group relative border rounded-lg overflow-hidden bg-muted/30">
 <div className="aspect-[1/1.4] relative flex items-center justify-center p-2">
 {/* eslint-disable-next-line @next/next/no-img-element */}
 <img src={page.dataUrl} alt={`Page ${page.pageNumber}`} className="max-w-full max-h-full object-contain shadow-sm bg-background" />
 </div>
 <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
 <Button size="sm" variant="secondary" onClick={() => handleDownloadPage(page)} className="gap-2">
 ...
 </div> : <div className="h-[300px] flex flex-col items-center justify-center text-muted-foreground space-y-4">
 <ImageIcon className="h-16 w-16 opacity-20" />
 {isConverting ? <div className="space-y-4 w-full max-w-xs">
 ...
 </div> : <p>Ready to extract images from your PDF</p>}
 </div>}
 </CardContent>
 </GlassCard>}
 </div>
      ...
      <ToolFeatureGuides features={[
        ...
        <div className="prose dark:prose-invert max-w-none">
          ...
          <p>
            ...
            Whether you're a developer, designer, student, or professional, our file.name provides
