...
"No data uploads",
 ...
 <tbody>
 {[
 ["UPPERCASE keywords","SELECT id FROM users WHERE active = 1","Most SQL style guides (Google, GitLab, dbt)"],
 ["lowercase keywords","select id from users where active = 1","Some modern ORMs, quick scripts"],
 ["Mixed (Title Case)","Select Id From Users Where Active = 1","Not recommended — inconsistent"],
 ].map(([style, ex, pref]) => (
 ...
 <tbody>
 {[
 ["String quotes","Single quotes only","Single or double quotes","Single quotes"],
 ["Identifier quoting", '\"double quotes\"',"\`backticks\`","[brackets]"],
 ["LIMIT syntax","LIMIT n","LIMIT n","TOP n (before SELECT)"],
 ["Auto-increment","SERIAL / GENERATED","AUTO_INCREMENT","IDENTITY(1,1)"],
 ["JSON support","Native JSONB type","JSON type (5.7+)","OPENJSON / FOR JSON"],
 ].map(([feat, pg, my, ms]) => (
