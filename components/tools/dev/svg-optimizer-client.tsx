...
export default function SvgOptimizerClient() {
 ...
 <textarea value={inputSvg} onChange={e => setInputSvg(e.target.value)} rows={12} className={textareaClass} placeholder="Paste your SVG code here..." />
 ...
 {(["optimized", "react", "uri"] as const).map(tab => <Button key={tab} variant={activeTab === tab ? "default" : "outline"} size="sm" onClick={() => setActiveTab(tab)} className="capitalize">
 {tab === "uri" ? "Data URI" : tab === "react" ? "React JSX" : "Optimized SVG"}
 </Button>)}
 </div>
 <Button onClick={handleCopy} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
