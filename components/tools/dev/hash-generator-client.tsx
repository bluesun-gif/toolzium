...
export default function HashGeneratorClient() {
 ...
 <ExportTextButton variant="outline" label="Export JSON" filename="hash-results.json" icon={DownloadCloud} getText={() => JSON.stringify({
          mode,
          fileName,
 ...
 <Stat label="Input bytes" value={mode === "text" ? toBytes(text).length : fileBytes?.length ?? 0} hint={mode === "file" ? fileName || "No file" : "UTF‑8 length"} />
 <Stat label="Algorithms" value={selectedAlgoList.length} hint={selectedAlgoList.join(",") || "None"} />
 <Stat label="Last run" value={perf ? `${perf.toFixed(2)} ms` : "—"} hint={autoRun ? "Auto‑run on" : "Manual"} Icon={Timer} />
 </div>
 ...
 <ActionButton label="Text" variant={mode === "text" ? "default" : "outline"} onClick={() => setMode("text")} />
 <ActionButton label="File" variant={mode === "file" ? "default" : "outline"} onClick={() => setMode("file")} />
 </div>
 ...
 {ALL_ALGOS.map(name => <ActionButton key={name} size="sm" variant={algos[name] ? "default" : "outline"} label={name} onClick={() => setAlgos(p => ({
                  ...p,
                  [name]: !p[name]
                }))} />)}
 </div>
 ...
 <ActionButton label="Hex" variant={encoding === "hex" ? "default" : "outline"} onClick={() => setEncoding("hex")} />
 <ActionButton label="Base64" variant={encoding === "base64" ? "default" : "outline"} onClick={() => setEncoding("base64")} />
 </div>

 <SwitchRow label="Uppercase" hint="Applied for hex output" checked={uppercase} onCheckedChange={v => setUppercase(Boolean(v))} />

 <Separator />

 {/* HMAC */}
 <SwitchRow label={<span className="inline-flex items-center gap-2">
 <Lock className="h-4 w-4" />
 HMAC
 </span> as unknown as string} hint={`Compute HMAC over the input using ${selectedAlgoList.length ? selectedAlgoList.join(",") : "selected algos"}.`} checked={useHmac} onCheckedChange={v => setUseHmac(Boolean(v))} />
 <InputField label="Secret key (UTF‑8)" value={hmacKey} onChange={e => setHmacKey(e.target.value)} disabled={!useHmac} placeholder="Enter HMAC key" />

 {/* Salt */}
 <InputField label="Salt (optional)" value={salt} onChange={e => setSalt(e.target.value)} placeholder="Salt string" />
 <SwitchRow label="Prefix salt" hint="Off = suffix" checked={saltBefore} onCheckedChange={v => setSaltBefore(Boolean(v))} />

 {/* Perf & autorun */}
 <SwitchRow label="Auto‑run" hint="Re-run on every change" checked={autoRun} onCheckedChange={v => setAutoRun(Boolean(v))} />

 {error && <div className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-destructive">
 <p className="text-sm font-medium">{error}</p>
 </div>}
 </CardContent>
 <CardFooter className="flex gap-2">
 <ActionButton label="Hash" onClick={() => {
              void run();
 ...
 <CardContent className="space-y-4">
 {mode === "text" ? <TextareaField label="Text" value={text} onValueChange={setText} textareaClassName="min-h-[140px] font-mono" placeholder="Type or paste text here..." /> : <div className="space-y-2">
 ...
 <ActionButton variant="outline" label="Choose file" icon={Upload} className="pointer-events-none" />
 </div>
 ...
 {results.length === 0 && <div className="rounded-md border p-3 text-sm text-muted-foreground">
 No results yet. Click <em>Hash</em> or enable Auto‑run.
 </div>}
 {results.map(r => <div key={r.name} className="flex flex-col gap-2 rounded-md border p-3">
 <div className="flex items-center justify-between">
 <Badge variant="secondary">
 {useHmac ? `HMAC-${r.name}` : r.name}
 </Badge>
 <CopyButton variant="outline" size="sm" label={copiedKey === r.name ? "Copied" : "Copy"} icon={copiedKey === r.name ? Check : undefined} getText={() => r.value} onCopied={() => {
                      setCopiedKey(r.name);
                      setTimeout(() => setCopiedKey(null), 1200);
                    }} />
 </div>
 <TextareaField rows={1} readOnly value={r.value} autoResize textareaClassName="h-full" />
 </div>)}
 </div>
 </div>
 </CardContent>
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: BookOpen
      }]} badges={["MD5, SHA-256, SHA-512", "Client-side only", "Instant hashing"]} />
 ...
 <tbody>
 {[["MD5", "128 bits (32 hex)", "⚠️ Broken", "Legacy checksums only"], ["SHA-1", "160 bits (40 hex)", "⚠️ Deprecated", "Legacy Git commits, avoid for security"], ["SHA-256", "256 bits (64 hex)", "✅ Secure", "TLS, Bitcoin, code signing, passwords"], ["SHA-384", "384 bits (96 hex)", "✅ Secure", "High-security TLS certificates"], ["SHA-512", "512 bits (128 hex)", "✅ Secure", "Maximum security, slower than SHA-256"], ["SHA-3-256", "256 bits (64 hex)", "✅ Secure", "Future-proof, quantum-resistant design"]].map(([algo, output, status, use]) => <tr key={algo} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["File integrity check", "SHA-256", "Publisher shares SHA-256; you verify download matches"], ["Password storage", "bcrypt / Argon2", "Hash + salt stored; never store plain passwords"], ["Digital signatures", "SHA-256 + RSA/ECDSA", "Hash the document, sign the hash"], ["Git commits", "SHA-1 (legacy) / SHA-256", "Each commit is identified by hash of content"], ["API request signing", "HMAC-SHA256", "Hash message + secret key to prove authenticity"], ["Blockchain", "SHA-256 (Bitcoin)", "Each block contains hash of previous block"]].map(([use, algo, how]) => <tr key={use} className="odd:bg-muted/20">
