...
const syntaxHighlight = (json: string): React.ReactNode => {
  ...
  return <pre className="text-xs font-mono whitespace-pre-wrap" dangerouslySetInnerHTML={{
    __html: html
  }} />;
};
export function ApiTesterClient() {
 ...
 {activeTab === "params" && <div className="space-y-2">
 {params.map((p, i) => <div key={i} className="flex gap-2 items-center">
 ...
 {activeTab === "headers" && <div className="space-y-2">
 ...
 {activeTab === "body" && <div className="space-y-3">
 ...
 {activeTab === "auth" && <div className="space-y-3">
 ...
 {Object.entries(response.headers).map(([k, v]) => <div key={k}><span className="text-cyan-400">{k}:</span> <span className="text-muted-foreground">{v as string}</span></div>)}
 </div>
 </details>
