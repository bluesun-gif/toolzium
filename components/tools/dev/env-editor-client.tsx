...
type EnvLine = {
  type: "var" | "comment" | "empty" | "invalid";
  key?: string;
  value?: string;
  comment?: string;
  raw?: string;
...
export function EnvEditorClient() {
  ...
  const exportedCode = useMemo(() => {
    ...
    if (exportFormat === "json") {
      ...
      vars.forEach(v => {
        obj[v.key as string] = v.value as string;
      });
 ...
 {viewMode === "raw" ? <textarea value={rawEnv} onChange={e => setRawEnv(e.target.value)} className={textareaClass} rows={12} /> : <div className="border border-border/50 rounded-lg overflow-hidden">
 ...
 <tbody>
 {parsedEnv.map(line => <tr key={line.index} className="border-b border-border/30 hover:bg-muted/10">
 <td className="p-2">
 ...
 <td className="p-2">
 {(line.type === "var" || line.type === "comment" || line.type === "empty") && <Button variant="ghost" size="icon" onClick={() => deleteLine(line.index)}><Trash2 className="w-4 h-4 text-red-500" /></Button>}
 </td>
 </tr>)}
 </tbody>
 </table>
 ...
 {validations.length > 0 && <Card className="border border-yellow-500/50 shadow-lg bg-yellow-500/5 rounded-2xl overflow-hidden">
 ...
 <div className="flex gap-2">
 {(["env", "json", "yaml", "docker"] as const).map(fmt => <Button key={fmt} variant={exportFormat === fmt ? "default" : "outline"} size="sm" onClick={() => setExportFormat(fmt)} className="uppercase">
 {fmt}
 </Button>)}
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Import your raw environment variables file into the editor.",
        icon: FileCode
      }, {
        step: "02",
        title: "Validate & Clean",
        description: "Automatically detect duplicate keys, invalid formats, and mask sensitive secrets.",
        icon: AlertTriangle
      }, {
        step: "03",
        title: "Export for Deployment",
        description: "Convert your variables into JSON, YAML, or Docker CLI formats instantly.",
        icon: Copy
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Automatically hides values for keys containing 'password', 'secret', 'token', or 'key'."
      }, {
        icon: AlertTriangle,
        title: "Smart Validation",
        description: "Detects duplicate keys, empty values, and enforces SCREAMING_SNAKE_CASE naming conventions."
      }, {
        icon: FileCode,
        title: "Multi-Format Export",
        description: "Export your config as standard .env, JSON, YAML, or Docker CLI arguments."
      }, {
        icon: ArrowDownAZ,
        title: "Table Management",
        description: "Sort alphabetically, remove duplicates, and edit keys/values in a structured grid view."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Are my environment variables uploaded to a server?",
        answer: "No. This tool runs 100% locally in your browser. Your .env data is never transmitted over the internet, ensuring maximum security for your credentials."
      }, {
        question: "Why are my values being ignored by my application?",
        answer: "Many parsers require values containing spaces or special characters to be wrapped in quotes (e.g., `MY_VAR=\"hello world\"`). Ensure your values are properly escaped."
      }, {
