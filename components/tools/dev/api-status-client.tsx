...
export default function ApiStatusClient() {
  ...
  const filteredApis = useMemo(() => {
    ...
    return all.filter(api => (category === "All" || api.category === category) && (api.name.toLowerCase().includes(search.toLowerCase()) || api.url.toLowerCase().includes(search.toLowerCase())));
  }, [search, category, customApis]);
  const steps = [{
    step: "01",
    title: "Browse Services",
    description: "Filter through popular cloud, payment, and developer APIs by category.",
    icon: Search
  }, {
    step: "02",
    title: "Quick Ping",
    description: "Test endpoint responsiveness directly from your browser using beacon requests.",
    icon: Activity
  }, {
    step: "03",
    title: "Track Custom URLs",
    description: "Add your own private API endpoints to monitor their status page links.",
    icon: Plus
  }];
  const features = [{
    icon: Cloud,
    title: "Cloud Providers",
    description: "Track the uptime and SLAs of major infrastructure providers like AWS and GCP."
  }, {
    icon: CreditCard,
    title: "Payment Gateways",
    description: "Monitor critical financial infrastructure like Stripe and PayPal for transaction failures."
  }, {
    icon: Code,
    title: "Developer Tools",
    description: "Stay ahead of outages in CI/CD pipelines, version control, and hosting platforms."
  }, {
    icon: Shield,
    title: "Custom Endpoints",
    description: "Save and monitor your own internal microservices and private API endpoints."
  }];
  const faqs = [{
    question: "Why does the Quick Ping show an 'Opaque response'?",
    answer: "Modern browsers enforce Cross-Origin Resource Sharing (CORS) policies. When pinging a third-party API that hasn't explicitly allowed your domain, the browser blocks the response body for security, but the successful network round-trip still confirms the server is online and responding."
  }, {
    question: "How are SLA percentages calculated?",
    answer: "Service Level Agreements (SLAs) represent the guaranteed uptime a provider commits to. For example, 99.9% (three nines) allows for roughly 8.7 hours of downtime per year, while 99.99% (four nines) allows for only 52 minutes of downtime annually."
  }, {
 ...
 {filteredApis.map(api => <Card key={api.url} className="border border-border/50 shadow-sm">
 ...
 {api.category === "Custom" && <Button size="sm" variant="destructive" onClick={() => removeCustomApi(api.url)}>X</Button>}
 </div>
 </CardContent>
 </Card>)}
 </div>
