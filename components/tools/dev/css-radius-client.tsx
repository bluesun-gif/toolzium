...
export default function CssRadiusClient() {
  const [tlH, setTlH] = useState(10);
  const [tlV, setTlV] = useState(10);
  const [trH, setTrH] = useState(10);
  const [trV, setTrV] = useState(10);
  const [brH, setBrH] = useState(10);
  const [brV, setBrV] = useState(10);
  const [blH, setBlH] = useState(10);
 ...
 <div className="w-full h-full transition-all duration-100 shadow-2xl" style={{
 ...
 <Button variant={linked ? "default" : "outline"} size="sm" className="text-xs font-semibold" onClick={() => setLinked(!linked)}>
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Adjust the width and height of your preview element to define the bounding box.",
        icon: Maximize
      }, {
        step: "02",
        title: "Shape Corners",
        description: "Use linked sliders for uniform curves or unlink to design complex elliptical corners.",
        icon: Circle
      }, {
        step: "03",
        title: "Export Code",
        description: "Copy the shorthand and longhand CSS border-radius properties instantly.",
        icon: Copy
      }]} badges={["Elliptical Support", "Visual Preview", "One-Click Copy"]} />

 <ToolFeatureGuides features={[{
        icon: Circle,
        title: "Elliptical Corner Control",
        description: "Unlike basic generators, this tool supports independent horizontal and vertical radii for true elliptical curves."
      }, {
        icon: Link,
        title: "Smart Linking System",
        description: "Toggle linked mode to adjust all four corners simultaneously, or unlink them to craft asymmetric, organic blob shapes."
      }, {
        icon: Palette,
        title: "Live Color & Sizing",
        description: "Dynamically resize the preview canvas and test your border radius against any background color in real-time."
      }, {
        icon: Maximize,
        title: "Intelligent Presets",
        description: "Instantly apply common shapes like Pills, Squircles, and Cards, with automatic mathematical adjustments to element dimensions."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "What does the slash (/) mean in the generated CSS?",
        answer: "The slash separates the horizontal radius values from the vertical radius values. For example, '10px / 20px' means the corners curve 10px inward horizontally, but 20px inward vertically, creating an elliptical rather than circular curve."
      }, {
        question: "Can I use percentages instead of pixels?",
        answer: "Yes, CSS border-radius accepts percentages. However, percentages are calculated relative to the element's own width and height, which can lead to unpredictable elliptical shapes. This tool uses pixels for precise, predictable control."
      }, {
