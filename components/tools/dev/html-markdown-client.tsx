...
export function HtmlMarkdownClient() {
 ...
 <textarea className={textareaClass} rows={15} value={input} onChange={e => setInput(e.target.value)} placeholder={direction === "html2md" ? "<h1>Header</h1>" : "# Header"} />
 </div>
 <div className="space-y-2">
 <div className="flex justify-between items-center">
 ...
 <div className="prose dark:prose-invert max-w-none min-h-[150px] p-4 rounded-lg border border-border/50 bg-muted/20" dangerouslySetInnerHTML={{
            __html: direction === "md2html" ? output : markdownToHtml(output)
          }} />
 </CardContent>
 </GlassCard>
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Choose whether to convert from HTML to Markdown or vice-versa using the toggle.",
        icon: ArrowRightLeft
      }, {
        step: "02",
        title: "Paste Your Code",
        description: "Drop your markup or markdown into the input textarea. Conversion happens instantly on every keystroke.",
        icon: FileCode
      }, {
        step: "03",
        title: "Copy or Preview",
        description: "Grab the converted output or check the live rendered preview to verify formatting accuracy.",
        icon: Eye
      }]} badges={["100% Free", "Client-Side Privacy", "No Signup"]} />

 <ToolFeatureGuides features={[{
        icon: FileCode,
        title: "Robust Tag Mapping",
        description: "Handles complex nested HTML including blockquotes, lists, inline code, and multi-line code blocks."
      }, {
        icon: FileText,
        title: "Markdown Standard Support",
        description: "Supports CommonMark standards for headings, bold/italic emphasis, links, images, and horizontal rules."
      }, {
        icon: Eye,
        title: "Instant Live Preview",
        description: "Visually renders the output HTML in real-time so you can verify typography and layout instantly."
      }, {
        icon: ArrowRightLeft,
        title: "One-Click Swap",
        description: "Quickly reverse the conversion pipeline by swapping input and output buffers with a single click."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "Yes, it supports core GFM features including tables, strikethrough (via standard HTML mapping), and task lists when converting back to HTML."
      }, {
        question: "Will inline styles in HTML be converted?",
        answer: "Markdown does not support inline CSS styling. Inline styles will be stripped during conversion to maintain clean Markdown syntax."
      }, {
        question: "Can it convert HTML tables to Markdown?",
        answer: "Basic table structures are supported, though highly complex tables with merged cells (colspan/rowspan) may require manual adjustment after conversion."
      }, {
