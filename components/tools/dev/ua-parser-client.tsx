...
const commonUAs = [
 { name:"Chrome (Windows 11)", ua:"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"},
 { name:"Safari (iPhone)", ua:"Mozilla/5.0 (iPhone; CPU iPhone OS 17_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.1 Mobile/15E148 Safari/604.1"},
 { name:"Firefox (Ubuntu)", ua:"Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/119.0"},
 { name:"Googlebot", ua:"Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)"},
 { name:"Edge (macOS)", ua:"Mozilla/5.0 (Macintosh; Intel Mac OS X 13_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36 Edg/119.0.0.0"}
];
...
export default function UaParserClient() {
 ...
 return { browser, browserVersion, os, osVersion, device, engine, isBot, raw: ua };
 }, []);
 ...
 <select 
 onChange={(e) => { setUaString(e.target.value); setParsedData(parseUA(e.target.value)); }} 
 className="p-2 rounded-md border border-border bg-background text-sm flex-1 sm:flex-none outline-none"
 defaultValue=""
 >
 <option value=""disabled>Load Common UA...</option>
 {commonUAs.map((item, i) => (
 <option key={i} value={item.ua}>{item.name}</option>
 ))}
 </select>
 </div>
 </CardContent>
 ...
 <ToolHowItWorks
 steps={[
 { step:"01", title:"Input UA String", description:"Paste any raw User-Agent header string or load a common preset from the dropdown.", icon: Globe },
 { step:"02", title:"Parse & Analyze", description:"Our client-side regex engine instantly dissects the string into structured components.", icon: Search },
 { step:"03", title:"View Insights", description:"Identify the browser, OS, device type, rendering engine, and detect automated bots.", icon: Monitor }
 ]}
 ...
 <ToolFeatureGuides
 features={[
 { icon: Globe, title:"Comprehensive Browser Detection", description:"Accurately identifies modern browsers including Chrome, Firefox, Safari, and Edge with precise version numbers."},
 { icon: Monitor, title:"OS & Device Profiling", description:"Distinguishes between Windows, macOS, Linux, iOS, and Android, while detecting mobile vs. tablet form factors."},
 { icon: Bot, title:"Automated Bot Identification", description:"Flags known crawlers and bots like Googlebot, Bingbot, and scraper scripts to help analyze traffic sources."},
 { icon: Cpu, title:"Engine Recognition", description:"Identifies the underlying rendering engine (Blink, WebKit, Gecko, Trident) powering the user's browser."}
 ]}
 >
 ...
 <ToolFaqAccordion
 faqs={[
 { question:"Is my User-Agent string sent to a server?", answer:"No. This tool is 100% client-side. Your User-Agent string is parsed locally in your browser using JavaScript and is never transmitted over the internet, ensuring complete privacy."},
 { question:"Why do browsers include other browser names in their UA string?", answer:"This is a historical artifact called 'User-Agent sniffing'. Early websites only served content to specific browsers like Netscape or IE. To get the same content, new browsers started including the older browser names in their strings to pass compatibility checks."},
 { question:"Can this tool detect all types of bots?", answer:"It detects common crawlers and bots (like Googlebot, Bingbot, and generic 'spider' keywords) via regex patterns. However, highly sophisticated bots that spoof standard browser UA strings may bypass basic detection."}
 ]}
 />
