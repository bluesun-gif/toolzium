...
const PRESETS = [
  { name: "Dreamy", points: [{ color: "#ff9a9e", x: 20, y: 20 }, { color: "#fecfef", x: 80, y: 30 }, { color: "#fdfbfb", x: 50, y: 80 }, { color: "#a18cd1", x: 10, y: 90 }] },
  { name: "Sunset Mesh", points: [{ color: "#ff0844", x: 10, y: 10 }, { color: "#ffb199", x: 80, y: 20 }, { color: "#ffd200", x: 40, y: 90 }, { color: "#f6d365", x: 90, y: 80 }] },
  { name: "Ocean Depth", points: [{ color: "#2E3192", x: 0, y: 0 }, { color: "#1BFFFF", x: 100, y: 100 }, { color: "#004FF9", x: 50, y: 50 }, { color: "#FFF94C", x: 80, y: 20 }] },
  { name: "Northern Lights", points: [{ color: "#43cea2", x: 10, y: 80 }, { color: "#185a9d", x: 90, y: 20 }, { color: "#00c6ff", x: 50, y: 50 }, { color: "#764ba2", x: 20, y: 10 }] },
  { name: "Cotton Candy", points: [{ color: "#fbc2eb", x: 30, y: 30 }, { color: "#a6c1ee", x: 70, y: 70 }, { color: "#f6d365", x: 80, y: 20 }, { color: "#ffecd2", x: 20, y: 80 }] },
  { name: "Emerald Forest", points: [{ color: "#134E5E", x: 10, y: 10 }, { color: "#71B280", x: 80, y: 80 }, { color: "#0f9b0f", x: 40, y: 60 }, { color: "#e6dada", x: 90, y: 10 }] },
  { name: "Midnight", points: [{ color: "#232526", x: 20, y: 20 }, { color: "#414345", x: 80, y: 80 }, { color: "#000046", x: 50, y: 50 }, { color: "#1CB5E0", x: 90, y: 10 }] },
  { name: "Peach", points: [{ color: "#ED4264", x: 10, y: 90 }, { color: "#FFEDBC", x: 90, y: 10 }, { color: "#ff9966", x: 50, y: 50 }, { color: "#ff5e62", x: 20, y: 20 }] },
];
...
const SIZES = [
  ...
  { label: "Wide", w: 1680, h: 720 },
];

export function MeshGradientClient() {
  const [points, setPoints] = useState<MeshPoint[]>(PRESETS[0].points.map((p, i) => ({ ...p, id: i })));
  ...
  const randomize = () => {
    ...
    const newPoints: MeshPoint[] = Array.from({ length: 4 }, (_, i) => ({
      id: i,
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <ToolPageHeader
        ...
        description="Design beautiful multi-point color blending mesh gradients for backgrounds, exportable as CSS, SVG, or PNG."
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className={`${cardClass} lg:col-span-1`}>
          ...
          <CardContent className="p-4 space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {SIZES.map((s, i) => (
                  <Button key={s.label} variant={i === sizeIdx ? "default" : "outline"} size="sm" onClick={() => setSizeIdx(i)} className="text-xs">
                    {s.label}
                  </Button>
                ))}
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-4 gap-2">
                {PRESETS.map((p, i) => (
                  <button
                    key={p.name}
                    onClick={() => setPoints(p.points.map((pt, idx) => ({ ...pt, id: idx })))}
                    className="h-8 rounded border border-border/50 hover:scale-105 transition-transform"
                    style={{ background: `linear-gradient(45deg, ${p.points[0].color}, ${p.points[1].color})` }}
                    title={p.name}
                  />
                ))}
              </div>
        ...
        <Card className={`${cardClass} lg:col-span-2 flex flex-col`}>
          ...
          <CardContent className="p-4 flex-1 flex flex-col gap-4">
            <div
              className="w-full rounded-xl border border-border/50 shadow-inner"
              style={{
            ...
            <div className="flex border-b border-border/50">
              <button className={`px-4 py-2 text-sm font-medium ${activeTab === "css" ? "border-b-2 border-primary text-primary" : "text-muted-foreground"}`} onClick={() => setActiveTab("css")}>
                CSS Output
              </button>
              <button className={`px-4 py-2 text-sm font-medium ${activeTab === "svg" ? "border-b-2 border-primary text-primary" : "text-muted-foreground"}`} onClick={() => setActiveTab("svg")}>
                SVG Output
              </button>
            </div>
      ...
      <ToolHowItWorks
        steps={[
          { step: "01", title: "Define Color Points", description: "Add up to 6 color points and position them across the X/Y axis using the interactive sliders.", icon: Layers },
          { step: "02", title: "Adjust Blur & Size", description: "Control the mesh intensity with the blur slider and select your target canvas aspect ratio.", icon: Palette },
          { step: "03", title: "Export Code or PNG", description: "Copy the generated CSS/SVG directly to your project, or download a high-resolution PNG asset.", icon: Download },
        ]}
      ...
      <ToolFeatureGuides
        features={[
          { icon: Layers, title: "Multi-Point Radial Blending", description: "Layer multiple radial gradients to create complex, organic mesh transitions impossible with standard linear gradients." },
          { icon: Code, title: "Production-Ready CSS", description: "Outputs clean, cross-browser compatible CSS utilizing standard radial-gradient syntax and backdrop filters." },
          { icon: Palette, title: "Curated Presets", description: "Jumpstart your design with 8 professionally curated color palettes inspired by modern UI trends." },
          { icon: Download, title: "Vector & Raster Export", description: "Export infinitely scalable SVG code for web, or high-DPI PNG files via client-side canvas rendering." },
        ]}
      >
      ...
      <ToolFaqAccordion
        faqs={[
          { question: "Is the generated CSS compatible with all browsers?", answer: "Yes. The output uses standard radial-gradient syntax which is supported by all modern browsers (Chrome, Firefox, Safari, Edge). The blur effect uses the standard CSS filter property." },
          { question: "How does the PNG export work without a server?", answer: "We utilize the HTML5 Canvas API. The tool generates an SVG string, loads it into an invisible Image object, draws it onto a canvas at your selected resolution, and extracts the PNG data URL locally." },
          { question: "Can I use these gradients for commercial projects?", answer: "Absolutely. All gradients generated using this tool are entirely yours and can be used in personal, commercial, or client projects without attribution." },
          { question: "Why does my CSS look sharp instead of blended?", answer: "Ensure you have included the 'filter: blur(Xpx)' property in your CSS. The blur is what softens the hard edges of the radial gradients into a smooth mesh." },
        ]}
      />
