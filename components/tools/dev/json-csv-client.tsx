...
export default function JsonCsvClient() {
  ...
  useEffect(() => {
    ...
    try {
      if (direction === "json-csv") {
        ...
        const rows = flatData.map(row => headers.map(h => {
          let val = row[h];
        ...
        const json = lines.slice(1).map(line => {
          ...
          headers.forEach((h, i) => {
            let v: any = vals[i] || "";
            if (!isNaN(Number(v)) && v.trim() !== "") v = Number(v);else if (v === "null") v = null;else if (v === "true") v = true;else if (v === "false") v = false;
            obj[h] = v;
          });
          return obj;
        });
  ...
  const steps = [{
    step: "01",
    title: "Paste Data",
    description: "Input your raw JSON array or CSV text into the respective editor pane.",
    icon: FileJson
  }, {
    step: "02",
    title: "Configure Options",
    description: "Set delimiters, toggle nested object flattening, and enable type detection.",
    icon: Settings
  }, {
    step: "03",
    title: "Convert & Export",
    description: "Instantly transform your data and download the resulting file or copy to clipboard.",
    icon: Download
  }];
  const features = [{
    icon: FileJson,
    title: "Nested Flattening",
    description: "Automatically converts deeply nested JSON objects into dot-notation CSV headers."
  }, {
    icon: FileSpreadsheet,
    title: "Smart Delimiters",
    description: "Support for commas, semicolons, tabs, and pipes with proper quote escaping."
  }, {
    icon: ArrowRightLeft,
    title: "Bi-Directional",
    description: "Seamlessly convert JSON to CSV and CSV back to typed JSON objects."
  }, {
    icon: Download,
    title: "Instant Export",
    description: "Download your transformed data as clean .json or .csv files instantly."
  }];
  const faqs = [{
    question: "How does the tool handle nested JSON objects?",
    answer: "When the 'Flatten nested' option is enabled, the tool recursively traverses your JSON objects and converts nested structures into dot-notation headers (e.g., `user.address.city`). This ensures that complex hierarchical data can be accurately represented in a flat, two-dimensional CSV table."
  }, {
    question: "Will my CSV data with commas break the conversion?",
    answer: "No. The converter strictly follows RFC 4180 standards. If a cell value contains the selected delimiter, double quotes, or newline characters, it is automatically wrapped in double quotes, and any internal quotes are properly escaped to prevent parsing errors."
  }, {
 ...
 {error && <div className="flex items-center gap-2 text-destructive text-xs bg-destructive/10 p-2 rounded">
