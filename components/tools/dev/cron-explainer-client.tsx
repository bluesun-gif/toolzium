...
const explainCron = (expression: string) => {
  ...
  if (parts.length !== 5) return {
    valid: false,
    text: "Invalid cron format. Please use 5 fields."
  };
  try {
...
export function CronExplainerClient() {
 ...
 <Input value={expression} onChange={e => setExpression(e.target.value)} className="text-center text-2xl font-mono py-8 tracking-widest" placeholder="* * * * *" />
 </div>
 ...
 <CardContent>
 {explanation.valid ? <ul className="space-y-2">
 {nextRuns.map((run, i) => <li key={i} className="p-3 bg-muted/50 rounded-md text-sm font-mono flex items-center gap-3">
 <Zap className="w-4 h-4 text-primary" />
 {run}
 </li>)}
 </ul> : <p className="text-muted-foreground text-sm">Please enter a valid expression to see next runs.</p>}
 </CardContent>
 </GlassCard>
 </div>
 ...
 {PRESETS.map((p, i) => <Button key={i} variant="outline" className="w-full justify-start text-left h-auto py-3" onClick={() => setExpression(p.expr)}>
 <div>
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: Calendar
      }]} badges={["Plain English translation", "Next run preview", "Syntax validation"]} />
 ...
 <tbody>
 {[["1st", "Minute", "0–59", "* , - /"], ["2nd", "Hour", "0–23", "* , - /"], ["3rd", "Day of Month", "1–31", "* , - / ? L W"], ["4th", "Month", "1–12 or JAN–DEC", "* , - /"], ["5th", "Day of Week", "0–6 or SUN–SAT", "* , - / ? L #"]].map(([pos, field, vals, chars]) => <tr key={pos} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["* * * * *", "Every minute"], ["0 * * * *", "Every hour (at :00)"], ["0 0 * * *", "Every day at midnight"], ["0 9 * * 1-5", "Every weekday at 9 AM"], ["0 0 * * 0", "Every Sunday at midnight"], ["*/15 * * * *", "Every 15 minutes"], ["0 9,17 * * *", "At 9 AM and 5 PM daily"], ["0 0 1 * *", "First day of every month"], ["0 0 1 1 *", "Once a year (Jan 1st)"], ["30 6 * * 1", "Every Monday at 6:30 AM"]].map(([expr, meaning]) => <tr key={expr} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["*", "Wildcard", "Any value", "* in hour = every hour"], [",", "List", "Multiple values", "1,15 in minute = at :01 and :15"], ["-", "Range", "Range of values", "9-17 in hour = 9 AM to 5 PM"], ["/", "Step", "Every N units", "*/5 in minute = every 5 minutes"], ["L", "Last", "Last day of month/week", "L in DOM = last day of month"], ["#", "Nth weekday", "Nth occurrence in month", "1#2 = 2nd Monday"]].map(([char, name, meaning, ex]) => <tr key={char} className="odd:bg-muted/20">
