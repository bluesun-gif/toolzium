...
import { Square, Plus, Trash2, Sparkles, Shield, Zap, Copy } from "lucide-react";
...
export function BoxShadowClient() {
  ...
  const updateActiveLayer = (updates: Partial<ShadowLayer>) => {
    setLayers(layers.map(l => l.id === activeLayerId ? {
      ...l,
      ...updates
    } : l));
  };
  ...
  const loadPreset = (presetLayers: ShadowLayer[]) => {
    const newLayers = presetLayers.map(l => ({
      ...l,
      id: Math.random().toString(36).substring(7)
    }));
    setLayers(newLayers);
 ...
 {layers.map((l, i) => <div key={l.id} className={cn("flex items-center border rounded-md overflow-hidden", activeLayerId === l.id ? "border-primary ring-1 ring-primary" : "border-input")}>
 <Button className="px-3 py-1.5 text-sm hover:bg-muted" onClick={() => setActiveLayerId(l.id)}>
 Layer {i + 1}
 </Button>
 {layers.length > 1 && <Button className="px-2 py-1.5 text-destructive hover:bg-destructive/10" onClick={() => removeLayer(l.id)}>
 <Trash2 className="h-3.5 w-3.5" />
 </Button>}
 </div>)}
 </div>

 {activeLayer && <div className="space-y-4 pt-4 border-t">
 <div className="flex items-center justify-between">
 ...
 {presets.map(p => <Button key={p.name} variant="outline" onClick={() => loadPreset(p.layers as any)}>
 {p.name}
 </Button>)}
 </div>
 </CardContent>
      ...
      <ToolFeatureGuides features={[{
        ...
        <div className="prose dark:prose-invert max-w-none">
          ...
          <p>
            ...
            Whether you're a developer, designer, student, or professional, our CSS Box Shadow Generator provides
