...
export default function ColorPickerClient() {
 ...
 <ToolPageHeader title="Color Picker" description="Pick any color and get HEX, RGB, HSL codes instantly. Free online color picker with color name detection, complementary colors, and recent color history." icon={Pipette} />
 ...
 {/* SV Area */}
 <div ref={svAreaRef} className="w-full h-64 rounded-xl relative cursor-crosshair touch-none border border-border" style={{
 ...
 <div className="absolute w-5 h-5 -ml-2.5 -mt-2.5 rounded-full border-2 border-white shadow-[0_0_2px_rgba(0,0,0,0.5)] pointer-events-none" style={{
                    left: hsv.s + "%",
                    top: 100 - hsv.v + "%",
 ...
 {/* Hue Bar */}
 <div ref={hueBarRef} className="w-full h-6 rounded-full relative cursor-pointer touch-none" style={{
 ...
 <div className="absolute w-6 h-6 -ml-3 rounded-full border-2 border-white bg-background shadow-sm pointer-events-none" style={{
                    left: hsv.h / 360 * 100 + "%"
                  }} />
 </div>
 </div>
 ...
 <input type="color" value={activeHex} onChange={handleNativePicker} onBlur={handleNativePickerBlur} className="absolute -inset-4 w-24 h-24 cursor-pointer" title="Native color picker" />
 </div>
 <div>
 ...
 <div className="flex-1 flex items-center gap-4 p-4 rounded-xl bg-muted/30">
 <div className="w-12 h-12 rounded-lg border border-border shrink-0 shadow-sm cursor-pointer hover:scale-105 transition-transform" style={{
                    backgroundColor: compHex
                  }} onClick={() => updateColorFromHex(compHex, true)} title="Click to apply" />
 <div>
 ...
 <InputField label="HEX" value={hex} onChange={e => updateColorFromHex(e.target.value)} onBlur={() => updateColorFromHex(hex, true)} />
 </div>
 ...
 <InputField type="number" min={0} max={255} value={rgb.r.toString()} onChange={e => updateColorFromRgb(Number(e.target.value) || 0, rgb.g, rgb.b)} />
 <InputField type="number" min={0} max={255} value={rgb.g.toString()} onChange={e => updateColorFromRgb(rgb.r, Number(e.target.value) || 0, rgb.b)} />
 <InputField type="number" min={0} max={255} value={rgb.b.toString()} onChange={e => updateColorFromRgb(rgb.r, rgb.g, Number(e.target.value) || 0)} />
 </div>
 ...
 <div className="flex items-center gap-2">
 <div className="grid grid-cols-3 gap-2 flex-1">
 <InputField type="number" min={0} max={360} value={hsl.h.toString()} onChange={e => updateColorFromHsl(Number(e.target.value) || 0, hsl.s, hsl.l)} />
 <InputField type="number" min={0} max={100} value={hsl.s.toString()} onChange={e => updateColorFromHsl(hsl.h, Number(e.target.value) || 0, hsl.l)} />
 <InputField type="number" min={0} max={100} value={hsl.l.toString()} onChange={e => updateColorFromHsl(hsl.h, hsl.s, Number(e.target.value) || 0)} />
 </div>
 ...
 <div className="flex items-center gap-2">
 <div className="grid grid-cols-3 gap-2 flex-1">
 <InputField type="number" value={hsv.h.toString()} readOnly />
 <InputField type="number" value={hsv.s.toString()} readOnly />
 <InputField type="number" value={hsv.v.toString()} readOnly />
 </div>
 ...
 {history.length === 0 ? <p className="text-sm text-muted-foreground">No recent colors yet.</p> : <div className="flex flex-wrap gap-2">
 {history.map((h, i) => <div key={h + "-" + i} className="w-8 h-8 rounded-md border border-border shadow-sm cursor-pointer hover:scale-110 transition-transform" style={{
                    backgroundColor: h
                  }} onClick={() => updateColorFromHex(h, false)} title={h} />)}
 </div>}
 </div>
 </div>
 </div>
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: Copy
      }]} badges={["HEX, RGB, HSL support", "Instant conversion", "CSS-ready output"]} />
 ...
 <tbody>
 {[["HEX", "#3B82F6", "CSS, design tools, copy-paste", "#RRGGBBAA (8-digit)"], ["RGB", "rgb(59, 130, 246)", "CSS, dynamic color with JS", "rgba(r, g, b, a)"], ["HSL", "hsl(217, 91%, 60%)", "Theming, color manipulation", "hsla(h, s%, l%, a)"], ["HSV/HSB", "hsv(217, 76%, 96%)", "Photoshop, design apps", "Not in CSS"], ["Named", "cornflowerblue", "Quick prototyping only", "No"], ["OKLCH", "oklch(64% 0.18 259)", "CSS Color Level 4, modern", "oklch(l c h / a)"]].map(([fmt, ex, best, alpha]) => <tr key={fmt} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["WCAG AA", "4.5:1 ratio", "3:1 ratio", "Required for most websites"], ["WCAG AAA", "7:1 ratio", "4.5:1 ratio", "Enhanced accessibility"], ["Fail", "< 4.5:1", "< 3:1", "Not accessible"]].map(([level, normal, large, use]) => <tr key={level} className="odd:bg-muted/20">
