...
export function CssTransform3dClient() {
  ...
  const howItWorksSteps = [{
    step: "01",
    title: "Define Parent Perspective",
    description: "Set the perspective distance on the parent container to establish the 3D space and depth perception.",
    icon: Eye
  }, {
    step: "02",
    title: "Apply 3D Transforms",
    description: "Rotate the element along the X, Y, and Z axes, and push it forward or backward using translateZ.",
    icon: Box
  }, {
    step: "03",
    title: "Configure Rendering",
    description: "Toggle transform-style preserve-3d for nested elements and manage backface visibility for flipping cards.",
    icon: Sliders
  }];
  const features = [{
    icon: Eye,
    title: "Perspective Control",
    description: "Adjust the viewer's distance from the Z=0 plane, controlling the intensity of the 3D foreshortening effect."
  }, {
    icon: Box,
    title: "Tri-Axis Rotation",
    description: "Freely rotate elements around the X, Y, and Z axes to build complex 3D structures like cubes and carousels."
  }, {
    icon: Sliders,
    title: "Z-Axis Translation",
    description: "Move elements closer to or further from the viewer using translateZ, creating true depth layering."
  }, {
    icon: RotateCcw,
    title: "Backface Management",
    description: "Hide the rear side of elements when rotated past 90 degrees, essential for double-sided card flip animations."
  }];
  const faqs = [{
    question: "Why does my 3D transform look flat?",
    answer: "You likely need to set 'transform-style: preserve-3d' on the parent element, otherwise child elements are flattened into a single 2D plane before rendering."
  }, {
    question: "What does perspective do?",
    answer: "Perspective defines how far the Z=0 plane is from the user. A lower value creates an exaggerated, extreme 3D effect, while a higher value results in a subtle, orthographic-like projection."
  }, {
 ...
 <CardContent className="p-8 flex-1 flex items-center justify-center relative bg-background overflow-hidden" style={{
            perspective: `${perspective}px`
          }}>
 <div className="w-32 h-32 bg-primary/80 rounded-xl shadow-2xl border border-white/20 flex items-center justify-center text-primary-foreground font-bold transition-transform duration-500" style={{
