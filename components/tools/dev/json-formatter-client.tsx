...
export default function JsonFormatterClient() {
  ...
  function sortObjectDeep<T>(value: T): T {
    ...
    if (value && typeof value === "object") {
      ...
      entries.sort(([a], [b]) => a.localeCompare(b, undefined, {
        numeric: true
      }));
  ...
  return <TooltipProvider><div className="relative z-10">
 {/* SECTION 1: HEADER */}
 <ToolPageHeader icon={FileJson} title="Free Online JSON Formatter, Prettifier & Validator" description="Pretty print, validate, and minify JSON data online. Sort object keys, detect syntax errors, and audit schemas with live AI inference." actions={<>
 <Button variant="outline" size="sm" onClick={loadSample} className="gap-1.5 text-xs">
 <Braces className="h-3.5 w-3.5" />
 Try Sample
 </Button>
 <ResetButton onClick={clearAll} />
 <CopyButton variant="default" getText={() => {
          toast.success("Formatted JSON copied to clipboard!");
          return output || "";
        }} disabled={!output} />
 <ExportTextButton filename="data.json" getText={() => output || input || ""} disabled={!output && !input} />
 </>} />
 ...
 <SelectField label="Indentation Level" options={[{
            value: "2",
            label: "2 Spaces (Standard)"
          }, {
            value: "4",
            label: "4 Spaces (Compact)"
          }, {
            value: "tab",
            label: "Tabs"
          }]} value={indent} onValueChange={v => setIndent(v as IndentOpt)} />
 <SwitchRow label="Sort Keys Alphabetically" hint="Sort object keys recursively for clean diffs." checked={sortKeys} onCheckedChange={v => setSortKeys(Boolean(v))} />
 <SwitchRow label="Auto-format on Paste" checked={autoOnPaste} onCheckedChange={v => setAutoOnPaste(Boolean(v))} />
 </div>
 </GlassCard>
 ...
 <PasteButton variant="outline" size="sm" className="gap-1.5 text-xs" label="Paste" pastedLabel="Pasted" getExisting={() => input} setValue={val => {
                setInput(val);
 ...
 <InputField accept=".json,.txt,application/json" type="file" onFilesChange={async files => {
                const f = files?.[0];
                if (!f) return;
 ...
 <TextareaField ref={inputRef} value={input} onValueChange={setInput} placeholder='{"name":"Toolzium","type":"utility","status":"active"}' textareaClassName="min-h-[320px] font-mono text-xs leading-relaxed" />

 {error && <div className="rounded-lg bg-red-500/10 border border-red-500/30 p-3 text-xs text-red-500 font-mono">
 ...
 <Button size="sm" variant="outline" onClick={explainWithAi} disabled={aiLoading || !input} className="gap-1.5 font-semibold ml-auto">
 ...
 <TextareaField readOnly value={output} onValueChange={() => {}} placeholder="Your formatted, validated JSON output will appear here..." textareaClassName="min-h-[340px] font-mono text-xs bg-background text-emerald-400 leading-relaxed" />
 ...
 {/* AI Analysis Display */}
 {aiAnalysis.length > 0 && <div className="mt-6">
 <AiOutputDisplay title="AI JSON Structure & Security Audit" subtitle="Real-time LLM schema explanation, data types, and interface recommendations" content={aiAnalysis} loading={aiLoading} onRegenerate={explainWithAi} variant="prose" />
 </div>}

 {/* SECTION 3: HOW IT WORKS */}
 <ToolHowItWorks title="How to Format & Validate JSON" subtitle="Format, validate, and minify JSON strings in 3 simple steps." steps={steps} />
 ...
 <th className="px-4 py-3 border border-border font-semibold">Supports Nesting</th>
 <th className="px-4 py-3 border border-border font-semibold">File Size</th>
 <th className="px-4 py-3 border border-border font-semibold">Common Use</th>
 </tr>
 </thead>
 <tbody>
 <tr>
 <td className="px-4 py-2 border border-border font-medium">JSON</td>
 <td className="px-4 py-2 border border-border">High</td>
 <td className="px-4 py-2 border border-border">Yes</td>
 <td className="px-4 py-2 border border-border">Medium</td>
 <td className="px-4 py-2 border border-border">REST APIs, Configs</td>
 </tr>
 <tr>
 <td className="px-4 py-2 border border-border font-medium">XML</td>
 <td className="px-4 py-2 border border-border">Medium</td>
 <td className="px-4 py-2 border border-border">Yes</td>
 <td className="px-4 py-2 border border-border">Large</td>
 <td className="px-4 py-2 border border-border">SOAP APIs, RSS, Enterprise</td>
 </tr>
 <tr>
 <td className="px-4 py-2 border border-border font-medium">YAML</td>
 <td className="px-4 py-2 border border-border">Very High</td>
 <td className="px-4 py-2 border border-border">Yes</td>
 <td className="px-4 py-2 border border-border">Small to Medium</td>
 <td className="px-4 py-2 border border-border">CI/CD, Kubernetes, Configs</td>
 </tr>
 <tr>
 <td className="px-4 py-2 border border-border font-medium">CSV</td>
 <td className="px-4 py-2 border border-border">High (for tabular)</td>
 <td className="px-4 py-2 border border-border">No</td>
 <td className="px-4 py-2 border border-border">Small</td>
 <td className="px-4 py-2 border border-border">Spreadsheets, Data Export</td>
 </tr>
 </tbody>
 </table>
 </div>
