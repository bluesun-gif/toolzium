...
import { Scissors, Copy, RotateCcw, Plus, Star, Heart } from "lucide-react";
...
export default function CssClippathClient() {
 ...
 {presetShapes.map((shape, idx) => <Button key={idx} onClick={() => handlePresetClick(idx)} className={cn(`aspect-square rounded-lg border-2 transition-all hover:scale-105 ${selectedShape === idx ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/50'}`)} title={shape.name}>
 <svg viewBox="0 0 100 100" className="w-full h-full p-2">
 <polygon points={shape.points.map(p => `${p.x},${p.y}`).join(' ')} fill={selectedShape === idx ? 'hsl(var(--primary))' : 'currentColor'} className={selectedShape === idx ? '' : 'text-muted-foreground/60'} opacity="0.8" />
 </svg>
 </Button>)}
 </div>
 ...
 <svg ref={svgRef} width={previewSize.width} height={previewSize.height} className="overflow-visible select-none touch-none" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
 <polygon points={points.map(p => `${p.x * previewSize.width / 100},${p.y * previewSize.height / 100}`).join(' ')} fill={bgColor} opacity="0.85" />
 {points.map((point, idx) => <g key={idx}>
 <circle cx={point.x * previewSize.width / 100} cy={point.y * previewSize.height / 100} r="10" fill="white" stroke="hsl(var(--primary))" strokeWidth="3" className="cursor-move drop-shadow-md hover:scale-110 transition-transform" onMouseDown={() => handleMouseDown(idx)} />
 <text x={point.x * previewSize.width / 100} y={point.y * previewSize.height / 100} textAnchor="middle" dy="0.3em" className="text-[9px] font-black pointer-events-none select-none" fill="hsl(var(--primary))">
 {idx + 1}
 </text>
 </g>)}
 </svg>
 ...
 <Button onClick={copyCSS} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
 ...
 <span className="text-primary font-semibold">clip-path</span>: <span className="text-primary">{clipPathValue}</span>;{'\n'}
 <span className="text-primary font-semibold">-webkit-clip-path</span>: <span className="text-primary">{clipPathValue}</span>;
 </code>
 </pre>
 </CardContent>
 </GlassCard>
 ...
 {points.map((point, idx) => <div key={idx} className="flex items-center gap-2 p-2 bg-muted/20 rounded border border-border/40">
 <span className="text-xs font-mono font-bold w-6 text-primary">{idx + 1}</span>
 <span className="text-xs flex-1 font-mono">
 x: {point.x.toFixed(1)}%, y: {point.y.toFixed(1)}%
 </span>
 <Button onClick={() => removePoint(idx)} className="text-xs text-red-500 hover:text-red-600 font-semibold px-2">
 Remove
 </Button>
 </div>)}
 </CardContent>
