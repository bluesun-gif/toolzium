...
export default function ColorBlindnessClient() {
  ...
  const steps = [{
    step: "01",
    title: "Input Colors",
    description: "Enter a single hex code or paste an entire comma-separated color palette.",
    icon: Droplet
  }, {
    step: "02",
    title: "Apply Matrices",
    description: "View instant simulations across 8 different color vision deficiency types.",
    icon: Eye
  }, {
    step: "03",
    title: "Verify Contrast",
    description: "Ensure your text remains readable and meets WCAG guidelines for all users.",
    icon: Contrast
  }];
  const features = [{
    icon: Eye,
    title: "8 CVD Simulations",
    description: "Accurately simulate Protanopia, Deuteranopia, Tritanopia, and various anomalies."
  }, {
    icon: Droplet,
    title: "Palette Mode",
    description: "Paste multiple comma-separated colors to test your entire UI palette simultaneously."
  }, {
    icon: Contrast,
    title: "Contrast Auditing",
    description: "Verify if text contrast ratios hold up under different color blindness conditions."
  }, {
    icon: AlertTriangle,
    title: "Inclusive Design",
    description: "Ensure your error states and success indicators don't rely solely on red/green hues."
  }];
  const faqs = [{
    question: "Why shouldn't I use only red and green for error/success states?",
    answer: "Red-green color blindness (Protanopia and Deuteranopia) affects roughly 8% of the male population. To these users, red and green appear as indistinguishable muddy yellows or browns. Always pair color cues with icons (like ⚠️ or ✓) or text labels to ensure universal comprehension."
  }, {
    question: "How accurate are these simulation matrices?",
    answer: "This tool uses established Brettel and Viénot transformation matrices, which are the industry standard for simulating color vision deficiency. While individual biological variations exist, these matrices provide a highly accurate representation of how the majority of CVD users perceive color."
  }, {
 ...
 {colors.length > 0 && <div className="space-y-8">
 {CVD_TYPES.map(type => <div key={type.name} className="space-y-3">
 ...
 <div className="flex-1 flex items-center justify-center text-[10px] font-bold" style={{
                        backgroundColor: color,
                        color: contrastOrig > 3 ? "#fff" : "#000"
                      }}>Orig</div>
 <div className="flex-1 flex items-center justify-center text-[10px] font-bold" style={{
                        backgroundColor: sim,
                        color: contrastSim > 3 ? "#fff" : "#000"
                      }}>Sim</div>
 </div>
