...
export function CssFiltersClient() {
  ...
  const applyPreset = (preset: any) => {
    ...
    setDropShadow(preset.values.dropShadow);
  };
 ...
 <Button onClick={() => setDropShadow(!dropShadow)} className={cn(`w-10 h-5 rounded-full transition-colors ${dropShadow ? "bg-primary" : "bg-muted"}`)}>
 ...
 {dropShadow && <div className="space-y-3 pl-2 border-l-2 border-primary/30">
 ...
 {presets.map(p => <Button key={p.name} variant="outline" size="sm" onClick={() => applyPreset(p)}>
 {p.name}
 </Button>)}
 </div>
