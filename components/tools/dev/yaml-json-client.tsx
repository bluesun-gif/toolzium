...
export default function YamlJsonClient() {
  ...
  const normalizeKeys = React.useCallback((val: unknown): unknown => {
    if (!jsonSortKeys && !yamlSortKeys) return val;
    const sort = (x: unknown): unknown => {
      if (Array.isArray(x)) return x.map(sort);
      if (x && typeof x === "object") {
        const obj = x as Record<string, unknown>;
        return Object.fromEntries(Object.keys(obj).sort((a, b) => a.localeCompare(b)).map(k => [k, sort(obj[k])]));
      }
      return x;
    };
    return sort(val);
  }, [jsonSortKeys, yamlSortKeys]);
  const toYaml = React.useCallback((obj: unknown): string => {
    if (!yamlLib) return "";
  ...
  const toJson = React.useCallback((obj: unknown): string => JSON.stringify(normalizeKeys(obj), null, Math.max(0, jsonSpaces)), [normalizeKeys, jsonSpaces]);
...
"profile": {"email":"bob@example.com","age": 25 }
  ...
  }), [input, direction, jsonSpaces, jsonSortKeys, yamlFlow, yamlLineWidth, yamlSortKeys, yamlMultiDocs, output]);
  const resetAll = () => {
    setInput("");
    setOutput("");
    setDirection("auto");
 ...
 <ExportTextButton variant="default" icon={Download} label="Export Session" filename="yaml-json-session.json" getText={() => JSON.stringify(exportPayload, null, 2)} />
 </>} />
 ...
 <TextareaField id="input" placeholder="Paste YAML or JSON here…" value={input} onChange={e => setInput(e.target.value)} textareaClassName="min-h-[320px]" />

 <div className="flex items-center gap-2 flex-wrap">
 <InputField fileButtonSize="sm" type="file" fileButtonLabel="Import JSON" accept="application/json" onFilesChange={async files => {
                const f = files?.[0];
                if (!f) return;
 ...
 <InputField fileButtonSize="sm" type="file" fileButtonLabel="Import YAML" accept=".yaml,.yml,text/yaml,text/plain" onFilesChange={async files => {
                const f = files?.[0];
                if (!f) return;
                const txt = await f.text();
 ...
 <ActionButton size="sm" label="Sample YAML" icon={Braces} onClick={() => {
                setInput(sampleYaml);
                setDirection("yaml-to-json");
              }} />
 <ActionButton size="sm" label="Sample JSON" icon={Braces} onClick={() => {
                setInput(sampleJSON);
                setDirection("json-to-yaml");
              }} />
 </div>
 ...
 <SelectField id="direction" label="Direction" value={direction} onValueChange={v => setDirection(v as Direction)} options={[{
                value: "auto",
                label: "Auto-detect"
              }, {
                value: "yaml-to-json",
 ...
 <SwitchRow className="h-fit" label="Auto-run" checked={autoRun} onCheckedChange={setAutoRun} />
 </div>

 {error && <div className="mt-2 flex items-start gap-2 rounded-md border border-destructive/40 bg-destructive/10 p-2 text-destructive">
 ...
 <InputField id="jsonSpaces" type="number" label="Spaces" min={0} max={8} value={String(jsonSpaces)} onChange={e => setJsonSpaces(Math.max(0, Number(e.target.value) || 0))} />
 <SwitchRow label="Sort keys (JSON)" checked={jsonSortKeys} onCheckedChange={setJsonSortKeys} />
 </div>
 ...
 <InputField id="yamlWidth" type="number" label="Line width" min={40} max={200} value={String(yamlLineWidth)} onChange={e => setYamlLineWidth(Math.min(200, Math.max(40, Number(e.target.value) || 80)))} />
 <SwitchRow label="Sort keys (YAML)" checked={yamlSortKeys} onCheckedChange={setYamlSortKeys} />
 <SwitchRow label="Allow multi-doc YAML (---)" checked={yamlMultiDocs} onCheckedChange={setYamlMultiDocs} />
 </div>
 </div>
 </CardContent>
 </GlassCard>
 ...
 <ExportTextButton variant="default" icon={Download} label="Download" filename="converted.txt" getText={() => output} disabled={!output} />
 </div>
 </CardHeader>
 <CardContent>
 <TextareaField id="output" value={output} readOnly autoResize textareaClassName="min-h-[220px]" />
 </CardContent>
 </GlassCard>
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: BookOpen
      }]} badges={["Bidirectional conversion", "Nested structure support", "Works offline"]} />
 ...
 <tbody>
 {[["Comments", "Yes (# syntax)", "No"], ["Readability", "Very high — indentation-based", "Medium — bracket-based"], ["Verbosity", "Less verbose", "More verbose (quotes required)"], ["Multi-line strings", "Yes (block scalars | and >)", "Requires \\n escaping"], ["Data types", "Auto-detected (123, true, null)", "Explicit (\"true\"vs true)"], ["Parsing speed", "Slower (complex grammar)", "Faster (simpler grammar)"], ["Browser support", "Requires library", "Built-in (JSON.parse)"], ["Error-prone", "Yes (indentation errors)", "Less (brackets delimit)"]].map(([feat, yaml, json]) => <tr key={feat} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["GitHub Actions, GitLab CI", "YAML", "Human-written, comments needed"], ["Kubernetes manifests", "YAML", "Industry standard, readable"], ["Docker Compose", "YAML", "Official format"], ["REST API responses", "JSON", "Browser-native parsing"], ["Package.json / config files", "JSON", "No comments needed, strict"], ["Ansible playbooks", "YAML", "Official format"], ["Swagger/OpenAPI spec", "YAML or JSON", "Both supported equally"], ["Database storage", "JSON", "Native JSONB support in PostgreSQL"]].map(([useCase, prefer, reason]) => <tr key={useCase} className="odd:bg-muted/20">
