...
export function GradientGeneratorClient() {
 ...
 {(["linear", "radial", "conic"] as GradientType[]).map(t => <Button key={t} variant={type === t ? "default" : "outline"} size="sm" className="flex-1 capitalize" onClick={() => setType(t)}>
 {t}
 </Button>)}
 </div>

 {type === "linear" && <div className="space-y-1.5">
 ...
 <input type="range" min={0} max={360} value={angle} onChange={e => setAngle(Number(e.target.value))} className="w-full h-2 bg-muted rounded-lg appearance-none cursor-pointer accent-primary" />
 </div>}
 ...
 {stops.map(stop => <div key={stop.id} className="flex items-center gap-2 p-2 bg-muted/30 rounded-lg">
 <input type="color" value={stop.color} onChange={e => updateStop(stop.id, "color", e.target.value)} className="w-8 h-8 rounded cursor-pointer bg-transparent border-0" />
 <input type="number" min={0} max={100} value={stop.position} onChange={e => updateStop(stop.id, "position", Number(e.target.value))} className="w-16 text-xs bg-background border border-border rounded px-2 py-1" />
 <input type="range" min={0} max={100} value={stop.position} onChange={e => updateStop(stop.id, "position", Number(e.target.value))} className="flex-1 h-1.5 bg-muted rounded-lg appearance-none cursor-pointer accent-primary" />
