...
"Soft": [{ id:"1", x: 0, y: 4, blur: 6, spread: -1, color:"#000000", opacity: 0.1, inset: false }, { id:"2", x: 0, y: 2, blur: 4, spread: -2, color:"#000000", opacity: 0.1, inset: false }],
"Hard": [{ id:"1", x: 8, y: 8, blur: 0, spread: 0, color:"#000000", opacity: 1, inset: false }],
"Neon Glow": [{ id:"1", x: 0, y: 0, blur: 15, spread: 5, color:"#0ea5e9", opacity: 0.8, inset: false }, { id:"2", x: 0, y: 0, blur: 30, spread: 10, color:"#0ea5e9", opacity: 0.4, inset: false }],
"Neumorphism": [{ id:"1", x: -8, y: -8, blur: 16, spread: 0, color:"#ffffff", opacity: 0.8, inset: false }, { id:"2", x: 8, y: 8, blur: 16, spread: 0, color:"#000000", opacity: 0.15, inset: false }],
"Layered": [{ id:"1", x: 0, y: 1, blur: 2, spread: 0, color:"#000000", opacity: 0.05, inset: false }, { id:"2", x: 0, y: 4, blur: 8, spread: -2, color:"#000000", opacity: 0.1, inset: false }, { id:"3", x: 0, y: 16, blur: 24, spread: -8, color:"#000000", opacity: 0.15, inset: false }],
"Inner Shadow": [{ id:"1", x: 0, y: 4, blur: 8, spread: 0, color:"#000000", opacity: 0.2, inset: true }],
};
...
export default function CssShadowClient() {
 ...
 const addLayer = () => {
 setLayers(prev => [...prev, { id: Date.now().toString(), x: 0, y: 4, blur: 8, spread: 0, color:"#000000", opacity: 0.1, inset: false }]);
 };
 ...
 const steps = [
 { step:"01", title:"Add Layers", description:"Stack multiple shadow layers to create complex, realistic UI depth effects.", icon: Layers },
 { step:"02", title:"Tune Parameters", description:"Adjust X/Y offsets, blur, spread, and opacity for each individual layer.", icon: Sliders },
 { step:"03", title:"Copy CSS", description:"Grab the generated box-shadow code and paste it directly into your stylesheet.", icon: Code },
 ];

 const features = [
 { icon: Layers, title:"Multi-Layer Stacking", description:"Combine unlimited shadow layers to achieve complex material and neumorphic designs."},
 { icon: Sliders, title:"Precision Controls", description:"Fine-tune X/Y offsets, blur radius, spread, and opacity with pixel-perfect accuracy."},
 { icon: Box, title:"Live Preview", description:"Instantly visualize your shadows against custom backgrounds and border radii."},
 { icon: Code, title:"One-Click Export", description:"Copy clean, minified CSS code directly to your clipboard for immediate use."},
 ];

 const faqs = [
 { question:"How do I create a realistic Material Design shadow?", answer:"Material design relies on layered shadows to simulate physical elevation. Use the 'Layered' preset, which combines a tight, dark shadow for immediate contact depth with a wider, highly transparent shadow to simulate ambient light occlusion."},
 { question:"What is the difference between blur and spread?", answer:"Blur determines how soft or diffused the shadow edge appears, while spread expands or contracts the overall size of the shadow before the blur is applied. Negative spread values are excellent for creating tight, subtle inset shadows."},
 { question:"Can I use inset shadows for pressed button states?", answer:"Absolutely. Inset shadows simulate light being blocked inside an element, creating the illusion that the surface has been pushed down. Combine a subtle dark inset shadow with a slight downward translation for a highly tactile active state."},
 ];
 ...
 {layers.map((layer, i) => (
 <Card key={layer.id} className="border border-border/50">
 ...
 <CardContent className="p-6 space-y-6 flex flex-col items-center justify-center min-h-[400px]"style={{ backgroundColor: bgColor }}>
 <div className="w-48 h-48 transition-all duration-300"style={{ backgroundColor: boxColor, borderRadius: `${radius}px`, boxShadow: cssString }} />
