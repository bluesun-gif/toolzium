...
export default function CssGridClient() {
  ...
  const updateCellSpan = (cellIdx: number, type: 'colSpan' | 'rowSpan', value: number) => {
    ...
    setCellSpans(prev => ({
      ...
      [key]: {
        ...prev[key],
        [type]: value,
        colSpan: prev[key]?.colSpan || 1,
        rowSpan: prev[key]?.rowSpan || 1
      }
    }));
  };
  const resetGrid = () => {
 ...
 {expandedSections.has('presets') && <CardContent className="p-4 space-y-2">
 {Object.keys(presets).map(name => <Button key={name} variant="outline" className="w-full justify-start text-xs font-semibold" onClick={() => applyPreset(name as keyof typeof presets)}>
 {name}
 </Button>)}
 </CardContent>}
 </GlassCard>
 ...
 {expandedSections.has('layout') && <CardContent className="p-4 space-y-4">
 ...
 {expandedSections.has('spacing') && <CardContent className="p-4 space-y-4">
 <div className="space-y-1">
 ...
 {expandedSections.has('areas') && <CardContent className="p-4">
 <textarea className={textareaClass} rows={5} value={config.templateAreas} onChange={e => updateConfig('templateAreas', e.target.value)} placeholder={'"header header"\n"main sidebar"\n"footer footer"'} />
 </CardContent>}
 </GlassCard>
 ...
 {expandedSections.has('alignment') && <CardContent className="p-4 space-y-3">
 <div className="space-y-1">
