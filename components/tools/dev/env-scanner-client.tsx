...
export function EnvScannerClient() {
 ...
 if (!input.trim()) return { vars: [] as EnvVar[], risks: [] as Risk[], stats: { total: 0, risks: 0, critical: 0 } };
 ...
 vars.push({ key, value, line: idx + 1 });

 if (keysSeen.has(key)) {
 risks.push({ key, level:"WARNING", issue:"Duplicate key definition", recommendation:"Remove the duplicate variable to avoid unexpected overrides."});
 }
 keysSeen.add(key);

 if (!/^[A-Z0-9_]+$/.test(key) && !key.startsWith("NEXT_PUBLIC_")) {
 risks.push({ key, level:"INFO", issue:"Non-standard naming convention", recommendation:"Use SCREAMING_SNAKE_CASE for environment variables."});
 }

 if (value ==="") {
 risks.push({ key, level:"WARNING", issue:"Empty value detected", recommendation:"Assign a valid value or remove the variable if unused."});
 }

 if (value.includes("") && !trimmed.substring(eqIdx + 1).trim().startsWith('"') && !trimmed.substring(eqIdx + 1).trim().startsWith("'")) {
 risks.push({ key, level:"WARNING", issue:"Unquoted value with spaces", recommendation: 'Wrap values containing spaces in quotes (e.g.,"my value").' });
 }

 if (value ==="*") {
 risks.push({ key, level:"CRITICAL", issue:"Overly permissive wildcard (*)", recommendation:"Restrict CORS or access control to specific domains/IPs."});
 }

 const lowerVal = value.toLowerCase();
 if (placeholders.some((p) => lowerVal.includes(p))) {
 risks.push({ key, level:"WARNING", issue:"Placeholder/default value detected", recommendation:"Replace with a secure, production-ready value."});
 }

 if (value.startsWith("sk_live_") || value.startsWith("AKIA") || value.startsWith("ghp_") || (value.startsWith("sk-") && value.length > 20)) {
 risks.push({ key, level:"CRITICAL", issue:"Potential live API key exposed", recommendation:"REVOKE THIS KEY IMMEDIATELY and move to a secure vault."});
 }

 if (key.startsWith("NEXT_PUBLIC_") && sensitiveWords.some((w) => key.toLowerCase().includes(w))) {
 risks.push({ key, level:"CRITICAL", issue:"Sensitive variable exposed to browser", recommendation:"Remove NEXT_PUBLIC_ prefix. Secrets must remain server-side only."});
 }
 });

 requiredVars.forEach((req) => {
 if (!keysSeen.has(req)) {
 risks.push({ key: req, level:"INFO", issue:"Common required variable missing", recommendation: `Consider adding ${req} for standard environment configuration.` });
 }
 });
 ...
 return (
 <React.Fragment key={`${risk.key}-${idx}`}>
 ...
 <ToolHowItWorks
 steps={[
 { step:"01", title:"Paste Your Env File", description:"Copy the raw contents of your .env or .env.local file and paste it into the scanner textarea.", icon: FileWarning },
 { step:"02", title:"Instant Analysis", description:"Our engine immediately parses variables, checking against known leak patterns, weak defaults, and naming standards.", icon: ShieldAlert },
 { step:"03", title:"Review & Export", description:"Review the severity-ranked findings, expand for remediation steps, and export the full report as JSON.", icon: Download },
 ]}
 ...
 <ToolFeatureGuides
 features={[
 { icon: KeyRound, title:"API Key Leak Detection", description:"Instantly identifies exposed Stripe, AWS, GitHub, and OpenAI live keys using pattern matching."},
 { icon: AlertTriangle, title:"Next.js Public Exposure", description:"Flags dangerous NEXT_PUBLIC_ prefixes applied to sensitive variables that leak secrets to the browser."},
 { icon: FileWarning, title:"Weak Defaults & Placeholders", description:"Catches insecure defaults like 'changeme', empty strings, and overly permissive wildcards."},
 { icon: ShieldAlert, title:"Zero Data Transmission", description:"All parsing and analysis occurs locally in your browser. Your secrets never touch our servers."},
 ]}
 >
 ...
 <ToolFaqAccordion
 faqs={[
 { question:"Are my .env variables sent to a server for analysis?", answer:"No. This tool operates 100% client-side using JavaScript in your browser. Your environment variables, secrets, and API keys never leave your local machine."},
 { question:"Does this tool support .env formatting with quotes?", answer:"Yes. The parser intelligently strips both single and double quotes from values to accurately analyze the underlying string data."},
 { question:"What frameworks does this scanner support?", answer:"While it works for any standard key-value format, it includes specialized rules for Next.js (NEXT_PUBLIC_), React, Node.js, and standard Docker environment configurations."},
 { question:"Can I export the audit results for my compliance team?", answer:"Absolutely. Use the 'Export JSON Report' button to download a structured file containing all scanned variables, identified risks, and remediation recommendations."},
 ]}
 />
