...
"city":"Anytown"
 ...
 schema.items = { type:"any"}; // Simplified for tuple mixed types
 }
 ...
 (schema as any)._stats = { props: propCount, nested: nestedCount, arrays: arrayCount };
 return schema;
 }
 ...
 const options = { includeDesc, includeExamples, requireAll, additionalProps };
...
"7":"http://json-schema.org/draft-07/schema#",
 ...
 return { schema, error: null, stats: (schema as any)._stats || { props: 0, nested: 0, arrays: 0 } };
 } catch (e: any) {
 return { schema: null, error: e.message, stats: { props: 0, nested: 0, arrays: 0 } };
 }
 ...
 setValidationResult({ valid: isValid, error: isValid ?"":"Data structure does not match the generated schema."});
 if (isValid) toast.success("Validation passed!");
 else toast.error("Validation failed");
 } catch (e: any) {
 setValidationResult({ valid: false, error:"Invalid JSON in test payload."});
 ...
 description="Infer strict JSON Schema (Draft 4/6/7) from sample payloads with automated format detection and validation."
 />
 ...
 {["4","6","7"].map((d) => (
 <Button key={d} variant={draft === d ?"default":"outline"} size="sm"onClick={() => setDraft(d as any)} className="flex-1">
 v{d}
 </Button>
 ))}
 </div>
 ...
 <Button size="icon"variant="ghost"className="absolute top-2 right-2"onClick={() => handleCopy(JSON.stringify(generatedSchema.schema, null, 2))}>
 ...
 placeholder='Paste a second JSON object here to test against the schema...'
 />
 <div className="flex items-center gap-4">
 <Button onClick={runValidation}>Validate Payload</Button>
 {validationResult && (
 <div className={`flex items-center gap-2 text-sm font-medium ${validationResult.valid ?"text-green-500":"text-red-500"}`}>
 ...
 <ToolHowItWorks
 steps={[
 { step:"01", title:"Provide Sample Data", description:"Paste a representative JSON object. The engine will recursively analyze nested structures and arrays.", icon: FileJson },
 { step:"02", title:"Configure Rules", description:"Select your target Draft version, enforce required fields, and enable automatic descriptions and examples.", icon: Settings },
 { step:"03", title:"Validate & Export", description:"Copy the generated schema for your API, or use the built-in validator to test edge-case payloads instantly.", icon: CheckCircle },
 ]}
 ...
 <ToolFeatureGuides
 features={[
 { icon: BarChart3, title:"Smart Format Detection", description:"Automatically identifies emails, URIs, UUIDs, and IPv4 addresses to apply strict format constraints."},
 { icon: FileJson, title:"Recursive Inference", description:"Deeply traverses nested objects and arrays, calculating tuple types and generating nested property definitions."},
 { icon: Settings, title:"Draft 4/6/7 Support", description:"Outputs compliant schema URIs for legacy systems (Draft 4) or modern validators (Draft 7)."},
 { icon: CheckCircle, title:"Instant Validation", description:"Test edge-case payloads against your generated schema locally without needing an external API or CLI tool."},
 ]}
 >
 ...
 <ToolFaqAccordion
 faqs={[
 { question:"What is the difference between JSON Schema Draft 4 and Draft 7?", answer:"Draft 7 introduces several new keywords like 'if/then/else', 'const', and 'readOnly'. Draft 4 is older and used by legacy systems, while Draft 7 is widely supported by modern validators like Ajv."},
 { question:"Will this tool detect custom formats like phone numbers?", answer:"Currently, it detects standard IETF formats (email, uri, uuid, ipv4, date-time). Custom regex formats must be added manually to the generated output."},
 { question:"Can it handle deeply nested JSON?", answer:"Yes. The inference engine is fully recursive and will map objects nested to any depth, generating the corresponding nested 'properties' blocks."},
 { question:"Is my JSON data sent to a server for parsing?", answer:"No. All parsing, inference, and validation logic executes entirely in your browser via JavaScript. Your data remains private and local."},
 ]}
 />
