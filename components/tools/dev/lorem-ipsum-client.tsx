...
export default function LoremIpsumClient() {
  ...
  const outputText = useMemo(() => separatorBlankLine ? output.join("\n\n") : output.join("\n"), [output, separatorBlankLine]);
  const stats = useMemo(() => {
 ...
 <ExportTextButton variant="default" filename="lorem-ipsum.txt" getText={() => outputText || ""} disabled={!outputText} />
 </>} />
 ...
 <InputField label="Paragraphs" type="number" min={1} max={50} value={String(paragraphs)} onChange={e => setParagraphs(Math.max(1, Number(e.target.value) || 1))} />
 <InputField label="Words / paragraph" type="number" min={5} max={400} value={String(words)} onChange={e => setWords(Math.max(5, Number(e.target.value) || 5))} />
 </div>

 <SwitchRow label="Start with ‘Lorem ipsum…’" checked={startWithClassic} onCheckedChange={v => setStartWithClassic(Boolean(v))} />
 <SwitchRow label="Add punctuation" hint="Sprinkle commas and end with a period" checked={punctuation} onCheckedChange={v => setPunctuation(Boolean(v))} />
 <SwitchRow label="Blank line between paragraphs" checked={separatorBlankLine} onCheckedChange={v => setSeparatorBlankLine(Boolean(v))} />

 <Separator />

 <SwitchRow label="Deterministic" hint="Enable seeded generation for reproducible output" checked={deterministic} onCheckedChange={v => setDeterministic(Boolean(v))} />
 <InputField label="Seed" type="number" value={String(seed)} onChange={e => setSeed(Number(e.target.value) || 0)} disabled={!deterministic} />
 </CardContent>
 <CardFooter className="flex gap-2 flex-wrap">
 <ActionButton label="Generate" icon={AlignLeft} onClick={run} />
 <ActionButton label="1 para" variant="outline" onClick={() => {
              setParagraphs(1);
              setWords(80);
              if (!autoRun) run();
            }} />
 <ActionButton label="3 para" variant="outline" onClick={() => {
              setParagraphs(3);
              setWords(60);
              if (!autoRun) run();
            }} />
 <ActionButton label="5 para" variant="outline" onClick={() => {
              setParagraphs(5);
              setWords(50);
              if (!autoRun) run();
            }} />
 <SwitchRow label="Auto‑generate" checked={autoRun} onCheckedChange={v => setAutoRun(Boolean(v))} />
 </CardFooter>
 </GlassCard>
 ...
 <CardContent className="space-y-4">
 {output.length === 0 ? <div className="rounded-md border p-3 text-sm text-muted-foreground">
 No text yet. Click <em>Generate</em> or enable Auto‑generate.
 </div> : <TextareaField readOnly value={outputText} onValueChange={() => {}} textareaClassName="min-h-[530px] font-mono" />}
 </CardContent>
 </GlassCard>
 </div>
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: BookOpen
      }]} badges={["Paragraphs, words & chars", "HTML output mode", "Instant generation"]} />
 ...
 <tbody>
 {[["Figma / Sketch mockups", "Plain text paragraphs", "1–3 paragraphs"], ["HTML/CSS templates", "HTML output (<p> tags)", "3–5 paragraphs"], ["Email templates", "Plain text or HTML", "2–4 paragraphs"], ["CMS editors (WordPress)", "Plain text paragraphs", "3–5 paragraphs"], ["Button/label copy", "Words mode: 2–6 words", "N/A"], ["Card/tooltip copy", "Sentences mode: 1–2 sentences", "N/A"], ["Performance testing", "Character mode", "Exact byte count needed"]].map(([tool, format, amount]) => <tr key={tool} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["Lorem Ipsum", "Lorem ipsum dolor sit amet...", "UI prototypes, design mockups — neutral, internationally recognized"], ["Real content", "Actual article or product copy", "Final design reviews, client presentations, usability testing"], ["Blind text", "Foo bar baz qux...", "Technical testing where content is irrelevant"], ["Repeated text", "Content content content...", "Quick layout tests; avoid in final mockups"]].map(([type, ex, when]) => <tr key={type} className="odd:bg-muted/20">
