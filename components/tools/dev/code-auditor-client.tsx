...
const patterns = [
  { regex: /eval\s*\(/g, severity: 'critical' as const, category: 'Security', description: 'eval() usage detected', suggestion: 'Avoid eval() - use safer alternatives like Function constructor or JSON.parse' },
  { regex: /innerHTML\s*=|dangerouslySetInnerHTML/g, severity: 'critical' as const, category: 'XSS', description: 'Potential XSS vulnerability', suggestion: 'Use textContent or sanitize HTML with DOMPurify' },
  { regex: /var\s+\w+/g, severity: 'warning' as const, category: 'Best Practice', description: 'var keyword used', suggestion: 'Use const or let for block-scoped variables' },
  { regex: /==(?!=)/g, severity: 'warning' as const, category: 'Best Practice', description: 'Loose equality operator', suggestion: 'Use === for strict equality comparison' },
  { regex: /console\.(log|error|warn|info)/g, severity: 'info' as const, category: 'Production', description: 'Console statement found', suggestion: 'Remove console statements before production deployment' },
  { regex: /password\s*[:=]\s*['"][^'"]+['"]/gi, severity: 'critical' as const, category: 'Security', description: 'Hardcoded password detected', suggestion: 'Move sensitive data to environment variables' },
  { regex: /api[_-]?key\s*[:=]\s*['"][^'"]+['"]/gi, severity: 'critical' as const, category: 'Security', description: 'Hardcoded API key found', suggestion: 'Use environment variables for API keys' },
  { regex: /try\s*{[^}]*}/g, severity: 'info' as const, category: 'Error Handling', description: 'Verify error handling coverage', suggestion: 'Ensure all async operations have proper error handling' },
  { regex: /\b\d{2,}\b/g, severity: 'info' as const, category: 'Code Quality', description: 'Magic number detected', suggestion: 'Extract magic numbers to named constants' },
  { regex: /\.split\(|\.substring\(|\.slice\(/g, severity: 'info' as const, category: 'SQL Injection', description: 'String manipulation in potential query', suggestion: 'Use parameterized queries instead of string concatenation' }
];
...
export default function CodeAuditorClient() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 px-2 sm:px-4 py-4 sm:py-6">
      ...
      <Card className={cardClass}>
        ...
        <CardContent className="p-4 space-y-4">
          <textarea
            ...
            placeholder="Paste your code here for security and quality analysis..."
          />
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => { setCode(''); setFindings([]); }}>
              Clear
            </Button>
          </div>
        </CardContent>
              ...
              <div className="flex flex-wrap gap-3 items-center">
                ...
                {stats.info > 0 && (
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-500/10 border border-blue-500/30">
                    ...
                    <span className="text-sm font-bold text-blue-600">{stats.info} Info</span>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          <Card className={cardClass}>
            ...
            <CardContent className="p-4 space-y-3">
              {findings.map((finding, idx) => (
                <div
                  key={idx}
                  className={`p-4 rounded-lg border ${severityColor(finding.severity)} animate-in fade-in slide-in-from-top-2`}
      ...
      <ToolHowItWorks
        steps={[
          { step: "01", title: "Paste Your Code", description: "Copy and paste any code snippet into the editor", icon: Braces },
          { step: "02", title: "Run Analysis", description: "Click audit to scan for vulnerabilities and issues", icon: Shield },
          { step: "03", title: "Review Findings", description: "Get detailed reports with line numbers and fixes", icon: CheckCircle2 }
        ]}
      ...
      <ToolFeatureGuides
        features={[
          { icon: Shield, title: "Security Scanning", description: "Detect SQL injection, XSS, and hardcoded secrets" },
          { icon: AlertTriangle, title: "Best Practices", description: "Identify code quality issues and anti-patterns" },
          { icon: Info, title: "Multi-Language", description: "Auto-detect JavaScript, Python, SQL, and more" },
          { icon: CheckCircle2, title: "Actionable Fixes", description: "Get specific recommendations for each issue" }
        ]}
      >
      ...
      <ToolFaqAccordion
        faqs={[
          { question: "Is my code uploaded to a server?", answer: "No. All analysis happens 100% client-side in your browser. Your code never leaves your machine, ensuring complete privacy and security." },
          { question: "What languages are supported?", answer: "The auditor auto-detects JavaScript, TypeScript, Python, SQL, and Java. It uses pattern matching to identify language-specific constructs and applies appropriate rules." },
          { question: "Can I use this for production code?", answer: "Yes! This tool is designed for production code review. However, it's a supplement to professional security audits, not a replacement for comprehensive penetration testing." }
        ]}
      />
