...
export default function RegexTesterClient() {
  ...
  const [activeTab, setActiveTab] = useState<"match" | "replace" | "ops" | "lines" | "snippet" | "help">("match");
  ...
  const activeFlags = useMemo(() => (Object.keys(FLAG_META) as Flag[]).filter(k => flags[k]).join(""), [flags]);
...
for (const m of matches) {
 console.log(m[0], m.index, m.groups);
  ...
  const CSVRows: (string | number)[][] = [["#", "index", "length", "text"], ...matches.map((m, i) => [i + 1, m.index, m.length, JSON.stringify(m.text)])];
  const payload = {
    p: pattern,
    t: testText,
    r: replaceWith,
 ...
 <ToolPageHeader icon={Regex} title="Regex Tester" description="Test & debug regular expressions online." actions={<>
 <ResetButton onClick={resetAll} />
 <CopyButton getText={() => shareLink} />
 <ExportTextButton label="Export JSON" filename="regex-matches.json" getText={() => {
          const payload = {
            pattern,
            flags: activeFlags,
            matches,
 ...
 <ExportCSVButton variant="default" filename="regex-matches.csv" getRows={() => CSVRows} />
 </>} />
 ...
 <InputField label="Pattern" value={pattern} onChange={e => setPattern(e.target.value)} placeholder={String.raw`e.g. \b[A-Za-z]+\b`} className="font-mono" />

 <div className="space-y-2">
 <div className="text-sm font-medium">Flags</div>
 <div className="flex flex-wrap gap-2">
 {(Object.keys(FLAG_META) as Flag[]).map(k => <ActionButton key={k} size="sm" variant={flags[k] ? "default" : "outline"} label={FLAG_META[k].label} onClick={() => toggleFlag(k)} className="h-8 w-8 p-0 font-mono" />)}
 </div>
 ...
 <div className="flex flex-wrap gap-2">
 {PRESETS.concat(localPresets).map(p => <ActionButton key={`${p.name}-${p.pattern}`} variant="secondary" size="sm" label={p.name} onClick={() => loadPreset(p.pattern)} />)}
 <ActionButton size="sm" onClick={savePresetLocal} label="Save" icon={Save} />
 </div>
 </div>

 <div className="grid grid-cols-2 gap-3">
 <InputField label="Match limit" type="number" min={1} max={1000000} value={String(limit)} onChange={e => setLimit(Number(e.target.value) || 1)} />
 <InputField label="Timeout (ms)" type="number" min={50} max={10000} value={String(timeoutMs)} onChange={e => setTimeoutMs(Math.max(50, Number(e.target.value) || 50))} />
 </div>

 <SwitchRow label="Auto-run" hint="Re-run on every change" checked={autoRun} onCheckedChange={v => setAutoRun(Boolean(v))} />

 {error && <div className="flex items-start gap-2 rounded-md border border-destructive/40 bg-destructive/10 p-3 text-destructive">
 ...
 {perf && <span className="inline-flex items-center gap-1">
 <Timer className="h-3.5 w-3.5" /> {perf.took.toFixed(2)}ms{""}
 {perf.limited && "(limited)"}
 </span>}

 <ExportTextButton size="sm" className="ml-auto" filename="regex-state.json" label="State" getText={() => JSON.stringify({
              p: pattern,
              t: testText,
              r: replaceWith,
              f: flags,
 ...
 <InputField fileButtonSize="sm" type="file" accept="application/json" onFilesChange={async files => {
              const f = files?.[0];
              if (!f) return;
 ...
 <TextareaField label="Test Text" value={testText} onValueChange={setTestText} textareaClassName="min-h-[180px] font-mono" />
 ...
 <div className="prose prose-sm max-w-none whitespace-pre-wrap font-mono leading-relaxed dark:prose-invert" dangerouslySetInnerHTML={{
                    __html: highlighted
                  }} />
 </div>
 ...
 {matches.map((m, i) => <div key={`${m.index}-${i}`} className="rounded-md bg-muted/50 p-2 text-xs">
 ...
 {Object.keys(m.groups || {}).length > 0 && <div className="mt-1 grid grid-cols-1 gap-1 md:grid-cols-2">
 {Object.entries(m.groups).map(([k, v]) => <div key={k} className="rounded border bg-background p-1 font-mono text-[11px]">
 ...
 <InputField label="Replace With" value={replaceWith} onChange={e => setReplaceWith(e.target.value)} placeholder="$$&" className="font-mono" hint="Use tokens like $$&, $1, $<name>" />
 <div className="space-y-2">
 ...
 <SwitchRow label="Keep matching lines" hint="Toggle to switch between keep / remove." checked={lineFilterKeep} onCheckedChange={v => setLineFilterKeep(Boolean(v))} />
 <div className="rounded-md border bg-muted/30 p-3">
 <pre className="max-h-64 overflow-auto whitespace-pre-wrap break-words text-sm">
 {(() => {
                      const rx = buildRegex();
 ...
 <CopyButton label="Copy Current View" icon={Copy} getText={() => {
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: BookOpen
      }]} badges={["Privacy First — runs locally", "Real-time matching", "No server uploads"]} />
 ...
 <tbody>
 {[["/g", "Global", "Find all matches (not just the first)", "grep-style search, string.replaceAll"], ["/i", "Case-insensitive", "Match [a-z] and [A-Z] equally", "Username, email parsing"], ["/m", "Multiline", "^ and $ match line start/end", "Log parsing, line-by-line extraction"], ["/s", "DotAll", ". matches newlines too", "Multi-line HTML tag matching"], ["/u", "Unicode", "Full Unicode code point matching", "Emoji, CJK, Bengali text matching"], ["/y", "Sticky", "Match only at lastIndex", "Tokenizers, streaming parsers"]].map(([flag, name, effect, use]) => <tr key={flag} className="odd:bg-muted/20">
 ...
 {[[".", "Any character except newline"], ["\\d / \\D", "Digit / non-digit"], ["\\w / \\W", "Word char [a-zA-Z0-9_] / non-word"], ["\\s / \\S", "Whitespace / non-whitespace"], ["^ / $", "Start / end of string (or line with /m)"], ["* / + / ?", "0+ / 1+ / 0-or-1 of previous token"], ["{n,m}", "Between n and m repetitions"], ["(a|b)", "Alternation — match a or b"], ["(group)", "Capture group — referenced as $1 in replace"], ["(?:group)", "Non-capturing group — for grouping without capture"], ["(?=...)", "Positive lookahead — match if followed by..."], ["(?<!...)", "Negative lookbehind — match if NOT preceded by..."]].map(([syntax, desc]) => <div key={syntax} className="flex gap-2 rounded-md border bg-muted/30 p-2">
 ...
 <tbody>
 {[["Named groups", "(?<name>...) — ES2018+", "(?P<name>...)"], ["Lookbehind", "Fixed-length only (ES2018+)", "Variable-length OK"], ["Unicode props", "\\p{L} with /u (ES2018+)", "\\p{L} natively"], ["Atomic groups", "Not supported", "(?>...) supported"], ["Flags", "g i m s u y d v", "i m s x (different set)"]].map(([feat, js, pcre]) => <tr key={feat} className="odd:bg-muted/20">
