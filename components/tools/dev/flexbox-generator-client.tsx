...
export function FlexboxGeneratorClient() {
 ...
 <CardContent>
 <div className="bg-accent/20 rounded-md border border-dashed border-accent p-2 min-h-[300px]" style={{
                display: "flex",
 ...
 {items.map((item, index) => <div key={item.id} onClick={() => setSelectedItemId(item.id)} className={cn("flex items-center justify-center p-4 rounded bg-primary text-primary-foreground font-bold cursor-pointer transition-all border-2", selectedItemId === item.id ? "border-foreground" : "border-transparent")} style={{
