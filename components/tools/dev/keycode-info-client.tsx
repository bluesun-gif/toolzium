...
export default function KeycodeInfoClient() {
 ...
 {!lastEvent ? <div className="space-y-4 animate-pulse">
 ...
 </div> : <div className="space-y-6 w-full">
 ...
 {commonKeys.map((k, i) => <tr key={i} className="hover:bg-muted/20 transition-colors">
 ...
 {history.length === 0 ? <p className="text-sm text-muted-foreground text-center py-8">No events recorded yet.</p> : history.map((h, i) => <div key={i} className="p-2 rounded bg-muted/20 border border-border/50 flex justify-between items-center text-xs font-mono">
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Focus Window",
        description: "Ensure your browser window is in focus and the 'Listener Active' toggle is enabled.",
        icon: Keyboard
      }, {
        step: "02",
        title: "Press Any Key",
        description: "Hit any key, combination, or modifier to instantly capture the native KeyboardEvent.",
        icon: Command
      }, {
        step: "03",
        title: "Analyze & Copy",
        description: "Review the key, code, and deprecated keyCode values, then copy the full JSON payload.",
        icon: Copy
      }]} badges={["Real-Time Capture", "Modifier Detection", "JSON Export"]} />

 <ToolFeatureGuides features={[{
        icon: Keyboard,
        title: "Modern vs Legacy Properties",
        description: "Clearly distinguishes between the modern `key` and `code` properties and the deprecated `keyCode` and `which` integers."
      }, {
        icon: Command,
        title: "Modifier State Tracking",
        description: "Instantly visualizes the state of Ctrl, Shift, Alt, and Meta (Cmd/Win) keys during complex chorded shortcuts."
      }, {
        icon: Terminal,
        title: "Comprehensive Reference",
        description: "Includes a built-in lookup table for common navigation, function, and control keys to speed up development."
      }, {
        icon: History,
        title: "Sequential Event Logging",
        description: "Maintains a rolling buffer of the last 10 keystrokes, allowing you to analyze sequences and rapid inputs."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Why is my keyCode showing as undefined or deprecated?",
        answer: "The `keyCode` and `which` properties were officially deprecated by the W3C because they were unreliable across different keyboard layouts and platforms. Modern browsers still support them for backward compatibility, but you should use `event.key` or `event.code` for new development."
      }, {
        question: "What is the difference between event.key and event.code?",
        answer: "`event.key` returns the character or action produced by the key (e.g., 'A' or 'Enter'), which changes based on Shift or keyboard layout. `event.code` returns the physical key identifier (e.g., 'KeyA' or 'Enter'), which remains the same regardless of layout or modifiers."
      }, {
