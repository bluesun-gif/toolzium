...
export default function StringEscapeClient() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 px-2 sm:px-4 py-4 sm:py-6">
      ...
      <ToolHowItWorks
        steps={[
          { step: "01", title: "Select Format", description: "Choose the target context like HTML, URL, JSON, or Base64 from the quick-select tabs.", icon: FileText },
          { step: "02", title: "Choose Direction", description: "Toggle between escaping (encoding) or unescaping (decoding) your input string.", icon: ArrowRightLeft },
          { step: "03", title: "Real-Time Output", description: "Watch the output update instantly on every keystroke. Copy the result with one click.", icon: Copy }
        ]}
      ...
      <ToolFeatureGuides
        features={[
          { icon: Shield, title: "XSS Prevention (HTML)", description: "Safely encode user-generated content to prevent Cross-Site Scripting vulnerabilities by escaping dangerous characters." },
          { icon: Globe, title: "URL Safety", description: "Encode special characters for safe transmission in query parameters and URI paths using standard percent-encoding." },
          { icon: Code, title: "Developer Serialization", description: "Perfect for generating safe JSON payloads, JavaScript string literals, and CSV exports directly in the browser." },
          { icon: Replace, title: "Bidirectional Swap", description: "Instantly flip input and output and reverse the operation direction for rapid decoding and testing workflows." }
        ]}
      >
      ...
      <ToolFaqAccordion
        faqs={[
          { question: "What is the difference between HTML escaping and URL encoding?", answer: "HTML escaping converts characters that have special meaning in HTML (like <, >, &) into entities to prevent code execution. URL encoding converts characters that are invalid in URLs (like spaces, ?, #) into percent-encoded hex values (like %20) to ensure the URL is parsed correctly by servers." },
          { question: "Is my data safe when using this tool?", answer: "Yes, absolutely. This tool operates 100% client-side. Your strings are processed locally by your browser's JavaScript engine and are never transmitted over the internet to any external server." },
          { question: "How does the CSV escaping work?", answer: "CSV escaping ensures that strings containing commas, newlines, or double quotes do not break the CSV structure. It wraps the string in double quotes and escapes any internal double quotes by doubling them (e.g., \" becomes \"\"), which is the standard RFC 4180 CSV format." }
        ]}
      />
