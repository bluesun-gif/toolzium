...
export function JsonToTypescriptClient() {
  ...
  const generateTs = useCallback((jsonStr: string) => {
    try {
      ...
      if (typeof data === "object" && data !== null && !Array.isArray(data)) {
        ...
        return {
          code: `export type ${rootName || "RootObject"} = ${inferPrimitive(data)};`,
          interfaces: 1,
          props: 0
        };
      }
      return {
        code: interfaces.reverse().join("\n\n"),
        interfaces: interfaceCount,
        props: propCount
      };
    } catch (e) {
      return {
        code: "// Invalid JSON format. Please check your input.",
        interfaces: 0,
        props: 0
      };
    }
  ...
  const howItWorksSteps = [{
    ...
    description: "Input your raw JSON object or array into the editor. The parser automatically validates the structure.",
    icon: FileJson
  }, {
    step: "02",
    title: "Configure Options",
    description: "Choose between interfaces or type aliases, toggle readonly modifiers, and decide whether to inline nested objects.",
    icon: Settings
  }, {
    step: "03",
    title: "Generate & Copy",
    description: "Instantly receive strictly-typed TypeScript definitions ready to be copied directly into your codebase.",
    icon: Code2
  }];
  const features = [{
    icon: FileType,
    title: "Interface vs Type Aliases",
    description: "Seamlessly switch between TypeScript 'interface' and 'type' keyword generation based on your project conventions."
  }, {
    icon: CheckSquare,
    title: "Deep Nested Parsing",
    description: "Automatically extracts deeply nested objects into separate, reusable interfaces or inline type definitions."
  }, {
    icon: Settings,
    title: "Strict Typing Controls",
    description: "Apply readonly modifiers, make all properties optional, and automatically export generated definitions."
  }, {
    icon: Code2,
    title: "Array Type Inference",
    description: "Intelligently detects array element types, supporting primitive arrays, object arrays, and mixed union types."
  }];
  const faqs = [{
    question: "Does this tool support deeply nested JSON objects?",
    answer: "Yes, the parser recursively traverses the entire JSON structure, generating separate interfaces for nested objects or inlining them based on your preference."
  }, {
    question: "Can I generate type aliases instead of interfaces?",
    answer: "Absolutely. Use the 'Output Style' toggle to switch between 'interface' and 'type' alias generation."
  }, {
    question: "How are arrays handled?",
    answer: "The tool inspects the first element of an array to determine its type. If it's an object, it generates a corresponding interface and appends '[]' to the type."
  }, {
 ...
 <textarea className={textareaClass} rows={12} value={jsonInput} onChange={e => setJsonInput(e.target.value)} placeholder="Paste your JSON here..." />
 ...
 <select className="w-full rounded-lg border border-border/70 bg-background/80 p-2 text-sm outline-none focus:ring-2 focus:ring-primary/50" value={outputStyle} onChange={e => setOutputStyle(e.target.value as any)}>
