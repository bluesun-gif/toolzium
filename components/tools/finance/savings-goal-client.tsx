...
export default function SavingsGoalClient() {
 ...
 <InputField label="Goal Amount" id="goal" inputMode="numeric" value={num(goal)} onChange={e => setGoal(safeNum(e.target.value))} />
 ...
 <div className="space-y-2">
 <InputField label="Current Savings" id="current" inputMode="numeric" value={num(current)} onChange={e => setCurrent(safeNum(e.target.value))} />

 <p className="text-xs text-muted-foreground">What you already have saved.</p>
 </div>

 <div className="space-y-2">
 <InputField label="Expected Annual Return (%)" id="rate" type="number" min={0} step="0.1" value={rate} onChange={e => setRate(Number(e.target.value) || 0)} />
 ...
 {useDate ? <InputField id="targetDate" type="date" value={targetDate} onChange={e => setTargetDate(e.target.value)} /> : <InputField id="months" type="number" min={1} value={months} onChange={e => setMonths(Number(e.target.value) || 1)} />}
 <p className="text-xs text-muted-foreground">
 {useDate ? `${n} month${n === 1 ? "" : "s"} until target.` : "Enter how many months you want to save."}
 </p>
 </div>

 <div className="col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
 <SwitchRow label="Deposit at beginning of month" hint="Turn on if you plan to deposit at the start of each month." checked={begin} onCheckedChange={setBegin} />
 ...
 <ExportCSVButton variant="default" filename="savings-schedule.csv" getRows={() => CSVRows} />
 </CardHeader>
 ...
 <tbody>
 {schedule.slice(0, 12).map(r => <tr key={r.month} className="border-t">
 ...
 <ToolHowItWorks steps={[{
      ...
      icon: BarChart3
    }]} badges={["Goal timeline calculator", "Compound interest included", "Instant results"]} />
 ...
 <tbody>
 {[["Emergency Fund", "3-6 months expenses", "6-18 months", "High-yield savings"], ["Vacation", "$2,000-$10,000", "6-24 months", "High-yield savings"], ["New Car (down payment)", "$5,000-$15,000", "1-3 years", "High-yield savings"], ["Home Down Payment", "$20,000-$100,000+", "3-7 years", "HYSA + bonds"], ["Wedding", "$15,000-$35,000", "2-4 years", "High-yield savings"], ["Retirement", "25x annual expenses", "20-40 years", "401k, IRA, index funds"], ["Child's Education", "$50,000-$200,000", "18 years", "529 plan, index funds"]].map(([goal, amount, timeline, account]) => <tr key={goal} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["$100/month", "$7,159", "$17,308", "$52,397"], ["$250/month", "$17,898", "$43,270", "$130,993"], ["$500/month", "$35,796", "$86,540", "$261,985"], ["$1,000/month", "$71,592", "$173,080", "$523,971"]].map(([monthly, y5, y10, y20]) => <tr key={monthly} className="odd:bg-muted/20">
...
function Stat({
  ...
  return <div className={cn("rounded-lg border p-3", highlight && "bg-primary/5 border-primary/30")}>
...
function monthsBetween(a: Date, b: Date) {
  ...
  if (b.getDate() >= a.getDate()) months += 1;else months = Math.max(1, months);
