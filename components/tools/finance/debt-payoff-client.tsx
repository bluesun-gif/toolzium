...
import { CreditCard, Calculator, TrendingDown, Calendar, AlertCircle, Sparkles, Shield, Zap, Copy } from "lucide-react";
...
export function DebtPayoffClient() {
 ...
 <ToolPageHeader icon={CreditCard} title="Debt Payoff Calculator" description="Calculate how long it takes to pay off your debt and how extra payments can save you money." actions={<ResetButton onClick={handleReset} label="Reset" />} />
 ...
 <Input type="number" value={balance} onChange={e => setBalance(e.target.value)} min="0" step="100" />
 </div>
 <div className="space-y-2">
 ...
 <Input type="number" value={monthlyPayment} onChange={e => setMonthlyPayment(e.target.value)} min="0" step="10" />
 </div>
 </CardContent>
 </GlassCard>
 ...
 <CardContent>
 {results?.error ? <Alert variant="destructive">
 <AlertCircle className="h-4 w-4" />
 <AlertTitle>Warning</AlertTitle>
 <AlertDescription>{results.error}</AlertDescription>
 </Alert> : results ? <div className="space-y-6">
 ...
 <tbody>
 {results.amortization.map(row => <tr key={row.month} className="border-b last:border-0 hover:bg-muted/20">
 ...
 <td className="px-4 py-3">{formatCurrency(row.balance)}</td>
      ...
      <ToolHowItWorks steps={[{
        ...
        description: "List each debt with balance and rate in the fields above — everything calculates live as you type.",
        icon: Sparkles
      }, {
        step: "02",
        title: "Review the Result",
        description: "Instantly see your payoff order, time, and interest saved, with breakdowns and visual cues.",
        icon: Zap
      }, {
        step: "03",
        ...
        icon: Sparkles,
        title: "Snowball & avalanche ranked",
        description: "Snowball & avalanche ranked"
      }, {
        icon: Shield,
        ...
        <div className="prose dark:prose-invert max-w-none">
          <h3>Why Use the Debt Payoff Calculator?</h3>
          <p>
            Anyone with multiple debts uses this to rank payoffs and see exactly how much interest the right order saves.
          </p>
          <p>
