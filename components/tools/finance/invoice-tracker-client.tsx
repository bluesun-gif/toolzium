...
export function InvoiceTrackerClient() {
 ...
 [i.clientName, i.invoiceNumber, i.amount, i.issueDate, i.dueDate, i.status].join(",")
 ).join("\n");
 ...
 else if (status ==="Overdue") classes +="bg-red-100 text-red-800";
 else if (status ==="Cancelled") classes +="bg-gray-100 text-gray-800";
 else classes +="bg-yellow-100 text-yellow-800"; // Pending
 ...
 description="Track pending invoices, payments, and outstanding balances locally."
 actions={
 <React.Fragment>
 ...
 <div key={inv.id} className={cn("flex flex-col sm:flex-row sm:items-center justify-between p-4 border rounded-lg transition-colors hover:bg-muted/30", (inv.status ==="Overdue"?"border-red-200 bg-red-50/30":""))}>
 ...
 {inv.status !=="Paid"&& (
 <Button size="icon"variant="ghost"className="h-8 w-8 text-green-600 hover:text-green-700 hover:bg-green-100"onClick={() => markStatus(inv.id,"Paid")} title="Mark as Paid">
