...
export default function VatCalculatorClient() {
  ...
  const calc = React.useMemo(() => computeVat({
    price,
    r,
    mode
  }), [price, r, mode]);
 ...
 <div className="space-y-2">
 <InputField label={mode === "add" ? "Net Price" : "Gross Price"} id="price" inputMode="decimal" value={numDisplay(price)} onChange={e => setPrice(safeNum(e.target.value))} />
 ...
 <div className="space-y-2">
 <InputField label="Tax Rate (%)" id="rate" type="number" min={0} step="0.1" value={rate} onChange={e => setRate(Number(e.target.value) || 0)} />
 <div className="flex flex-wrap gap-2 pt-1">
 {[5, 7.5, 10, 12, 15].map(v => <QuickChip key={v} onClick={() => setRate(v)}>
 {v}%
 </QuickChip>)}
 </div>
 </div>
 ...
 <ResultBox label="Net" value={formatMoney(calc.net, currency, round2)} onCopyText={formatMoney(calc.net, currency, round2)} />
 <ResultBox label="Tax" value={formatMoney(calc.tax, currency, round2)} onCopyText={formatMoney(calc.tax, currency, round2)} />
 <ResultBox label="Gross" value={formatMoney(calc.gross, currency, round2)} onCopyText={formatMoney(calc.gross, currency, round2)} />
 </div>
 ...
 <div className="pt-2">
 <ExportCSVButton variant="default" filename="vat-calculation.csv" getRows={() => CSVRows} />
 </div>
 </CardContent>
 </GlassCard>
 ...
 <ToolHowItWorks steps={[{
      ...
      icon: Receipt
    }]} badges={["Add or remove VAT", "Global VAT rates", "Instant calculation"]} />
 ...
 <tbody>
 {[["United Kingdom", "VAT", "20%", "5% (energy, children's car seats)"], ["Germany", "MwSt (VAT)", "19%", "7% (food, books, transport)"], ["France", "TVA (VAT)", "20%", "10% / 5.5% / 2.1%"], ["European Union avg.", "VAT", "21%", "Varies by country"], ["Australia", "GST", "10%", "0% (fresh food, health)"], ["Canada", "HST/GST", "5–15%", "Varies by province"], ["India", "GST", "18%", "5% / 12% (tiered)"], ["Bangladesh", "VAT", "15%", "5% / 10% (selected goods)"], ["USA", "Sales Tax", "0–13%", "No federal VAT (state only)"], ["Japan", "Consumption Tax", "10%", "8% (food & non-alcoholic drinks)"]].map(([country, name, standard, reduced]) => <tr key={country} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["Collection point", "At each production stage", "Only at final retail sale"], ["Who collects", "Every business in chain", "Only the retailer"], ["B2B transactions", "Charged, then reclaimed", "Often exempt"], ["Invoice requirement", "VAT invoice required", "Receipt sufficient"], ["Used by", "170+ countries", "USA (state level)"]].map(([feat, vat, sales]) => <tr key={feat} className="odd:bg-muted/20">
