...
export function SopBuilderClient() {
  ...
  const templates = {
    ...
    it: {
      ...
      dep: "IT",
      ver: "1.0",
      pur: "To ensure new employees receive necessary hardware and access.",
      scp: "All new hires.",
      steps: [{
        id: "1",
        description: "Create user accounts",
        responsible: "IT Admin",
        substeps: ["Email account", "Active Directory"]
      }]
    },
    safety: {
      ...
      dep: "Facilities",
      ver: "1.1",
      pur: "Safe evacuation during a fire alarm.",
      scp: "All building occupants.",
      steps: [{
        id: "1",
        description: "Sound the alarm",
        responsible: "Anyone",
        warning: "Do not use elevators",
        substeps: ["Pull nearest fire alarm"]
      }]
    },
    equipment: {
      ...
      ver: "2.0",
      pur: "Daily cleaning of the espresso machine.",
      scp: "Kitchen staff.",
      steps: [{
        id: "1",
        description: "Empty drip tray",
        responsible: "Cleaner",
        substeps: ["Remove tray carefully", "Wash with warm water"]
      }]
    }
  };
...
`;
 ...
 {steps.map((step, idx) => <div key={step.id} className="p-4 border rounded-lg space-y-4 bg-muted/20 relative group">
 ...
 <textarea className="w-full min-h-[80px] p-2 rounded-md border bg-background text-sm" value={step.substeps?.join('\n') || ""} onChange={e => updateStep(idx, "substeps", e.target.value.split('\n').filter(s => s.trim()))} />
 </div>
 </div>)}
 ...
 {steps.map((step, idx) => <div key={step.id}>
 ...
 {step.warning && <div className="ml-7 mb-2 p-2 bg-amber-500/10 border-l-2 border-amber-500 text-amber-600 dark:text-amber-400 text-sm flex gap-2">
 <AlertTriangle className="w-4 h-4 shrink-0" /> {step.warning}
 </div>}
 {step.substeps && step.substeps.length > 0 && <ul className="list-disc ml-11 text-sm">
 {step.substeps.map((sub, sidx) => <li key={sidx}>{sub}</li>)}
 </ul>}
 </div>)}
 </div>
 </div>
