...
export function BloodPressureClient() {
 ...
 if (sys > 180 || dia > 120)
 return { label:"Crisis", color:"bg-red-600 text-white"};
 if (sys >= 140 || dia >= 90)
 return { label:"High Stage 2", color:"bg-orange-600 text-white"};
 if ((sys >= 130 && sys <= 139) || (dia >= 80 && dia <= 89))
 return { label:"High Stage 1", color:"bg-orange-400 text-black"};
 if (sys >= 120 && sys <= 129 && dia < 80)
 return { label:"Elevated", color:"bg-yellow-400 text-black"};
 if (sys < 120 && dia < 80)
 ...
 items.reduce((a, b) => a + b.sys, 0) / items.length,
 );
 const avgDia = Math.round(
 items.reduce((a, b) => a + b.dia, 0) / items.length,
 );
 ...
 icon={Heart}
 actions={<ResetButton onClick={handleClear} label="Clear Data"/>}
 />
 ...
 <div
 className={cn("px-2 py-1 text-xs font-semibold rounded-md", (cat.color), "w-24 text-center")}
 >
 {cat.label}
 </div>
 <div>
