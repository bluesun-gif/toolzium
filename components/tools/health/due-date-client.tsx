...
export function DueDateClient() {
  ...
  const {
    ...
    if (diffDays >= 0 && diffDays <= 294) {
      ...
      if (weeks < 13) trim = 1;else if (weeks < 27) trim = 2;else trim = 3;
    }
    const ms = [{
      name: "First Heartbeat (approx)",
      date: new Date(lmp.getTime() + 6 * 7 * 24 * 60 * 60 * 1000),
      week: 6
    }, {
      name: "End of First Trimester",
      date: new Date(lmp.getTime() + 13 * 7 * 24 * 60 * 60 * 1000),
      week: 13
    }, {
      name: "First Kick (approx)",
      date: new Date(lmp.getTime() + 20 * 7 * 24 * 60 * 60 * 1000),
      week: 20
    }, {
      name: "Viability",
      date: new Date(lmp.getTime() + 24 * 7 * 24 * 60 * 60 * 1000),
      week: 24
    }, {
      name: "Full Term",
      date: new Date(lmp.getTime() + 37 * 7 * 24 * 60 * 60 * 1000),
      week: 37
    }, {
 ...
 <ToolPageHeader icon={HeartPulse} title="Pregnancy Due Date Calculator" description="Estimate your due date and track key milestones based on your last menstrual period." actions={<>
 ...
 {currentWeek > 0 && currentWeek <= 42 ? <div className="grid grid-cols-3 gap-2 text-center mb-6">
 ...
 </div> : <p className="text-sm text-muted-foreground text-center italic mb-4">Date is outside the typical pregnancy range.</p>}
 ...
 {milestones.map((m, i) => <div key={i} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
