...
export function MealPlannerClient() {
 ...
 lunch: [{ id:"2", name:"Chicken Salad", calories: 500 }],
 dinner: [{ id:"3", name:"Salmon & Rice", calories: 700 }],
 snacks: [{ id:"4", name:"Apple", calories: 95 }],
 });
 ...
 else if (goal ==="Muscle Gain") { p = 0.3; c = 0.5; f = 0.2; }
 ...
 setDayPlan(prev => ({
 ...prev,
 [selectedMealType]: [...prev[selectedMealType], { id: Date.now().toString(), name: newItemName, calories: Number(newItemCalories) }]
 }));
 setNewItemName("");
 setNewItemCalories("");
 ...
 description="Plan weekly meals and track macronutrients."
 actions={
 <React.Fragment>
 <CopyButton getText={getSummary} label="Copy Plan"/>
 <ResetButton onClick={() => setDayPlan({ breakfast: [], lunch: [], dinner: [], snacks: [] })} label="Clear Plan"/>
 </React.Fragment>
 } 
 />
 ...
 {(Object.keys(dayPlan) as (keyof DayPlan)[]).map((type) => (
 <div key={type} className="p-4 border rounded-lg space-y-3">
 <h3 className="font-semibold capitalize border-b pb-2">{type}</h3>
 {dayPlan[type].map(meal => (
 <div key={meal.id} className="flex justify-between items-center bg-muted/30 p-2 rounded">
 <div>
