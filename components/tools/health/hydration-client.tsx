...
export function HydrationClient() {
 ...
 <ToolPageHeader icon={Droplets} title="Hydration Reminder" description="Track your daily water intake with visual progress." actions={<ResetButton onClick={handleReset} label="Reset Today" />} />
 ...
 <div className="absolute bottom-0 left-0 w-full bg-blue-400 transition-all duration-1000 ease-in-out opacity-80" style={{
                  height: `${percentage}%`
                }} />
 ...
 {historyDates.map(date => {
                  ...
                  const dayName = dObj.toLocaleDateString(undefined, {
                    weekday: 'short'
                  });
                  return <div key={date} className="flex-1 flex flex-col items-center gap-2">
 <div className="w-full bg-muted rounded-t-sm h-full flex items-end justify-center relative overflow-hidden group">
 <div className={cn("w-full bg-blue-300 transition-all", isToday && "bg-blue-500")} style={{
                        height: `${dayPercent}%`
                      }} />
 <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/40 text-primary-foreground text-xs transition-opacity">
 {dayIntake}
 </div>
 </div>
