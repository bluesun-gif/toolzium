...
export function CalorieLookupClient() {
 ...
 {filteredFoods.map(food => <div key={food.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-lg bg-muted/30 hover:bg-muted/50 border border-transparent transition-colors gap-3">
 ...
 {filteredFoods.length === 0 && <div className="text-center py-8 text-muted-foreground">No foods found matching"{search}"</div>}
 </div>
 </CardContent>
 </GlassCard>
 </div>
 ...
 {mealPlan.map(item => <div key={item.instanceId} className="flex items-center justify-between p-2 rounded bg-muted/20 border text-sm">
 ...
 {mealPlan.length === 0 && <div className="text-center py-4 text-muted-foreground text-sm">No items added yet.</div>}
 </div>
 </div>
 </CardContent>
 </GlassCard>
