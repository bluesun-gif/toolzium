...
type BloodType = "A+" | "A-" | "B+" | "B-" | "AB+" | "AB-" | "O+" | "O-";
const compatibility: Record<BloodType, {
  give: string[];
  receive: string[];
}> = {
  "A+": {
...
export function BloodTypeClient() {
 ...
 <SelectContent>
 {Object.keys(compatibility).map(type => <SelectItem key={type} value={type}>{type}</SelectItem>)}
 </SelectContent>
 </Select>
 </div>
 </CardContent>
 ...
 {compatibility[selectedType].give.map(t => <span key={t} className="px-3 py-1 bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-100 rounded-full text-lg font-bold border border-red-200 dark:border-red-800">{t}</span>)}
 </div>
 ...
 <CardContent>
 <div className="flex flex-wrap gap-2">
 {compatibility[selectedType].receive.map(t => <span key={t} className="px-3 py-1 bg-primary/10 text-primary dark:bg-blue-900/50 rounded-full text-lg font-bold border border-blue-200 dark:border-blue-800">{t}</span>)}
 </div>
