...
export default function ImageToTextClient() {
 ...
 <CardContent>
 <input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/png, image/jpeg, image/webp, image/bmp" className="hidden" />
 
 {!imageSrc ? <div className={cn("border-2 border-dashed rounded-lg p-10 flex flex-col items-center justify-center text-center cursor-pointer transition-colors", isDragging ? "border-primary bg-primary/10" : "border-border hover:bg-muted/50")} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={triggerFileInput}>
 ...
 {!isTesseractReady && <p className="text-xs text-amber-500 mt-2">Loading OCR engine...</p>}
 ...
 {isProcessing && <div className="absolute inset-0 bg-background/80 flex flex-col items-center justify-center p-6 text-center backdrop-blur-sm">
 ...
 <Button variant="outline" onClick={triggerFileInput} className="w-full" disabled={isProcessing}>
 ...
 <TextareaField label="Text Content" value={text} onChange={e => setText(e.target.value)} placeholder={isProcessing ? "Extracting text..." : imageSrc ? "Extracted text will appear here. You can also type or edit..." : "Upload an image first..."} rows={16} disabled={!imageSrc || isProcessing} />
