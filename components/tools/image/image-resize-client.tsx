...
export default function ImageResizerClient() {
 ...
 {/* Social Media 1-Click Presets — Dropdown UI */}
   ...
   <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-end">
     ...
     <div className="flex-1 min-w-0">
       ...
       <select
         ...
         className="w-full h-9 rounded-lg border border-border/60 bg-background px-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition"
       >
     ...
     <div className="flex-[2] min-w-0">
       <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Preset Size</label>
       <select
         onChange={(e) => {
           const idx = parseInt(e.target.value, 10);
           if (!isNaN(idx) && filteredPresets[idx]) applyPreset(filteredPresets[idx]);
         }}
         defaultValue=""
         className="w-full h-9 rounded-lg border border-border/60 bg-background px-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition"
       >
     ...
     {filteredPresets.some(p => p.w === targetWidth && p.h === targetHeight) && (
       <div className="shrink-0 flex items-center gap-1.5 h-9 px-3 rounded-lg bg-primary/10 border border-primary/20 text-xs font-semibold text-primary whitespace-nowrap">
 ...
 <div className="grid grid-cols-3 gap-1.5 p-1 rounded-xl border border-border/60 bg-muted/30">
 {([
   { mode: "stretch" as FitMode, label: "Stretch", sub: "No White Bars" },
   { mode: "cover" as FitMode, label: "Smart Crop", sub: "Fill & Crop" },
   { mode: "contain" as FitMode, label: "Fit & Pad", sub: "With Background" },
 ] as { mode: FitMode; label: string; sub: string }[]).map(({ mode, label, sub }) => (
   <button
     key={mode}
     type="button"
     onClick={() => setFitMode(mode)}
     className={cn(
       "py-2.5 px-1 rounded-lg text-center transition-all duration-150 cursor-pointer select-none",
       "flex flex-col items-center justify-center gap-0.5 min-h-[54px]",
       fitMode === mode
         ? "bg-primary text-white shadow-sm"
         : "text-foreground hover:bg-muted/60 hover:text-primary"
     )}
   >
     <span className="font-bold text-[11px] leading-tight block">{label}</span>
     <span className={cn("text-[10px] leading-tight block", fitMode === mode ? "text-white/75" : "text-muted-foreground")}>{sub}</span>
   </button>
 ))}
 </div>
 </div>
 ...
 <Input type="number" value={targetWidth} onChange={e => handleWidthChange(Number(e.target.value))} className="font-mono font-semibold text-sm" />
 </div>
 <div className="space-y-1.5">
 <label className="text-xs font-semibold text-muted-foreground">Height (px):</label>
 <Input type="number" value={targetHeight} onChange={e => handleHeightChange(Number(e.target.value))} className="font-mono font-semibold text-sm" />
 </div>
 </div>
 ...
 <select value={format} onChange={e => setFormat(e.target.value as "original" | "image/jpeg" | "image/png" | "image/webp")} className="w-full">
 ...
 <input type="range" min="50" max="100" value={quality} onChange={e => setQuality(Number(e.target.value))} className="w-full mt-2" style={{"--range-progress": `${quality}%`} as React.CSSProperties} />
 </div>
 </div>
 ...
 <Button onClick={processImages} disabled={isProcessing || images.length === 0} size="lg" className="w-full gap-2">
 {isProcessing ? <>
 ...
 {images.some(i => i.resizedUrl) && <Button variant="outline" onClick={downloadAllZip} className="w-full gap-2">
