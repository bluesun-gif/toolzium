        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            ...
            <div className="space-y-4 flex-1">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input type="number" min={50} max={3000} value={width} onChange={e => setWidth(Math.max(50, Math.min(3000, parseInt(e.target.value) || 100)))} className="bg-background border-border text-foreground font-mono" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-muted-foreground">Height (px)</Label>
                  <Input type="number" min={50} max={3000} value={height} onChange={e => setHeight(Math.max(50, Math.min(3000, parseInt(e.target.value) || 100)))} className="bg-background border-border text-foreground font-mono" />
                </div>
              </div>

              <div className="space-y-1.5">
                <Label className="text-xs font-semibold text-muted-foreground">Custom Text Label (Optional)</Label>
                <Input placeholder={`Default: ${width} × ${height}`} value={customText} onChange={e => setCustomText(e.target.value)} className="bg-background border-border text-foreground" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2 items-center">
                    <input type="color" value={bgColor} onChange={e => setBgColor(e.target.value)} className="w-10 h-10 rounded-xl border border-border cursor-pointer bg-background p-1" />
                    <Input value={bgColor} onChange={e => setBgColor(e.target.value)} className="font-mono text-xs bg-background border-border text-foreground" />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2 items-center">
                    <input type="color" value={textColor} onChange={e => setTextColor(e.target.value)} className="w-10 h-10 rounded-xl border border-border cursor-pointer bg-background p-1" />
                    <Input value={textColor} onChange={e => setTextColor(e.target.value)} className="font-mono text-xs bg-background border-border text-foreground" />
                  </div>
                </div>
              </div>
          ...
          <GlassCard className="p-5 flex flex-col items-center justify-center bg-card border-border shadow-sm rounded-2xl min-h-[380px]">
            ...
            <div className="p-4 bg-muted/30 rounded-2xl border border-border flex items-center justify-center max-w-full overflow-hidden">
              <img src={svgDataUrl} alt="Placeholder Preview" className="max-h-[320px] max-w-full object-contain rounded-lg shadow-sm" />
            </div>
          </GlassCard>
        </div>
