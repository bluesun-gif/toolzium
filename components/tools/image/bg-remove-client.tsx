...
export default function BgRemoveClient() {
    ...
    try {
      ...
      img.onload = () => {
        ...
        if (mode === "instant") {
          ...
          fetch(sampleUrl).then(res => res.blob()).then(blob => {
 ...
 <div className="border-2 border-dashed border-primary/30 hover:border-primary/60 rounded-2xl p-10 text-center cursor-pointer transition-all duration-200 bg-muted/10 hover:bg-muted/30 group flex-1 flex flex-col items-center justify-center" onClick={() => fileInputRef.current?.click()} onDragOver={handleDragOver} onDrop={handleDrop}>
 ...
 {SAMPLE_IMAGES.map(sample => <Button key={sample.name} type="button" onClick={() => loadSampleImage(sample.url, sample.name)} disabled={isProcessing} className="group relative rounded-xl border bg-card hover:border-primary/50 overflow-hidden p-2 text-left transition-all duration-200 hover:shadow-md flex items-center gap-3">
 <img src={sample.url} alt={sample.name} className="h-12 w-12 rounded-lg object-cover border group-hover:scale-105 transition-transform" />
 ...
 {resultUrl && <Button onClick={handleDownload} size="sm" className="gap-1.5 shadow-sm font-semibold">
 ...
 {isProcessing ? <div className="py-20 text-center space-y-4 my-auto">
 ...
 {viewMode === "split" && resultUrl && <div className="relative flex-1 flex flex-col min-h-[360px] max-h-[460px]">
 ...
 <img src={resultUrl} alt="Cutout result" className="absolute inset-0 h-full w-full object-contain p-3 select-none pointer-events-none z-10" />
 ...
 {viewMode === "side" && resultUrl && <div className="grid gap-4 md:grid-cols-2 flex-1 min-h-[340px]">
 <div className="space-y-1.5 flex flex-col">
 <p className="text-xs font-semibold text-muted-foreground flex items-center gap-1.5">
 <ImageIcon className="h-3.5 w-3.5" /> Original Image
 </p>
 <div className="relative flex-1 rounded-xl overflow-hidden border bg-black/5 flex items-center justify-center">
 <img src={originalUrl} alt="Original" className="max-h-[340px] w-full object-contain p-2" />
 </div>
 </div>
