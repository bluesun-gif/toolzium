...
export function ImageAsciiClient() {
 ...
 description="Convert your images into text-based ASCII art."
 actions={
 <>
 {asciiResult && (
 <>
 ...
 {colorMode ==="color"&& (
 <ActionButton onClick={downloadHtml} icon={Download} label="HTML"/>
 )}
 ...
 max="200"
 value={widthChars} 
 onChange={(e) => setWidthChars(parseInt(e.target.value))}
 className="flex-1"
 />
 ...
 onCheckedChange={(c) => setColorMode(c ?"color":"monochrome")}
 />
 </div>
 </div>
 </CardContent>
 ...
 {asciiResult ? (
 <div className="w-full h-full overflow-auto bg-black rounded-lg border p-4 custom-scrollbar">
 <pre 
 className="font-mono whitespace-pre text-[8px] leading-[8px] sm:text-[10px] sm:leading-[10px] select-all w-max"
 style={{ color: colorMode ==="monochrome"?"#fff": undefined }}
 >
 {asciiResult.map((row, rIdx) => (
 <div key={rIdx} className="flex">
 {row.map((cell, cIdx) => (
 <span 
 key={cIdx} 
 style={{ color: colorMode ==="color"? cell.color :"inherit"}}
 >
 {cell.char}
 </span>
 ))}
 ...
 <div className="w-full h-full min-h-[400px] border-2 border-dashed rounded-lg flex flex-col items-center justify-center text-muted-foreground/50">
