...
export function ContrastComplianceSheetClient() {
 ...
 <ToolPageHeader icon={Palette} title="Color Contrast Ratio Compliance Sheet" description="Design system WCAG accessibility contrast compliance test sheet. Generate matrices for your palette." actions={<>
 ...
 {colors.map((c, i) => <div key={i} className="space-y-3 p-4 bg-muted/30 rounded-lg border">
 ...
 <Input type="color" value={c.hex} onChange={e => updateColor(i, "hex", e.target.value)} className="w-12 p-1 h-10" />
 <Input value={c.hex} onChange={e => updateColor(i, "hex", e.target.value)} className="flex-1 font-mono uppercase" />
 </div>
 </div>
 </div>)}
 </div>
 ...
 {colors.map((c, i) => <th key={i} className="p-3 border-b-2 bg-muted/50">
 ...
 <tbody>
 {colors.map((bg, i) => <tr key={i} className="border-b">
 ...
 {colors.map((fg, j) => {
                    ...
                    return <td key={j} className="p-3">
 <div className="flex items-center gap-2">
 <div className="w-8 h-8 rounded border flex items-center justify-center font-bold font-serif text-lg leading-none pt-1" style={{
                          backgroundColor: bg.hex,
                          color: fg.hex
                        }}>
 A
 ...
 <div className={cn("text-xs flex items-center gap-1", comp.normalAA ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400")}>
 ...
 {comp.normalAA ? "AA Pass" : "AA Fail"}
      ...
      <ToolFeatureGuides features={[{
        ...
        <div className="prose dark:prose-invert max-w-none">
          <h3>Why Use Our Color Contrast Ratio Compliance Sheet?</h3>
          <p>
            This free online tool is designed to help you get accurate results quickly and securely.
            Whether you're a developer, designer, student, or professional, our Color Contrast Ratio Compliance Sheet provides
      ...
      <RelatedTools currentToolUrl="/tools/image/contrast-compliance-sheet" max={6} />
