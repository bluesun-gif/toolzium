...
export function PaletteExtractClient() {
 ...
 <ToolPageHeader icon={Palette} title="Color Palette from Image" description="Upload an image to extract its dominant colors instantly." actions={<>
 <input type="file" accept="image/*" className="hidden" ref={fileInputRef} onChange={handleImageUpload} />
 <ActionButton icon={Upload} label="Upload Image" onClick={() => fileInputRef.current?.click()} variant="default" />
 {palette.length > 0 && <>
 ...
 {imageSrc ? <img src={imageSrc} alt="Uploaded" className="max-w-full max-h-[400px] rounded-lg object-contain" /> : <div className="w-full h-64 border-2 border-dashed rounded-lg flex flex-col items-center justify-center text-muted-foreground cursor-pointer hover:bg-secondary/50 transition-colors" onClick={() => fileInputRef.current?.click()}>
 ...
 <CardContent>
 {isProcessing ? <div className="flex justify-center items-center h-64">
 <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
 </div> : palette.length > 0 ? <div className="grid gap-4">
 {palette.map((color, i) => <div key={i} className="flex items-center gap-4 p-3 bg-secondary/30 rounded-lg border hover:border-primary/50 transition-colors group">
 <div className="w-16 h-16 rounded-md shadow-sm border border-border/50 shrink-0 cursor-pointer" style={{
                  backgroundColor: color.hex
                }} onClick={() => copyToClipboard(color.hex)} title="Click to copy Hex" />
 <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm font-mono">
 <div className="cursor-pointer hover:text-primary transition-colors" onClick={() => copyToClipboard(color.hex)}>
 {color.hex}
 </div>
 <div className="cursor-pointer hover:text-primary transition-colors" onClick={() => copyToClipboard(color.rgb)}>
 {color.rgb}
 </div>
 <div className="cursor-pointer hover:text-primary transition-colors text-xs flex items-center" onClick={() => copyToClipboard(color.hsl)}>
 {color.hsl}
 </div>
 </div>
 <Button variant="ghost" size="icon" className="opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => copyToClipboard(color.hex)}>
 <Copy className="w-4 h-4" />
 </Button>
 </div>)}
 </div> : <div className="flex flex-col items-center justify-center h-64 text-muted-foreground text-center">
...
export default PaletteExtractClient;
