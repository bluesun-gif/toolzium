...
export function SvgPathClient() {
 ...
 <textarea value={path} onChange={e => {
                setPath(e.target.value);
                setPreset("custom");
              }} className="w-full min-h-[100px] p-3 border rounded-md font-mono text-sm bg-background resize-y" placeholder="M 10 10 L 90 90..." />
 </div>
 ...
 <CardContent>
 <div className="w-full aspect-square border-2 border-dashed rounded-lg bg-grid flex items-center justify-center overflow-hidden" style={{
              backgroundImage: 'linear-gradient(to right, hsl(var(--border)) 1px, transparent 1px), linear-gradient(to bottom, hsl(var(--border)) 1px, transparent 1px)',
              backgroundSize: '20px 20px'
            }}>
 <svg viewBox="0 0 100 100" className="w-full h-full max-w-[400px] max-h-[400px]" style={{
                transform: "scale(" + scale + ")",
                transition: "transform 0.2s"
              }}>
 <path d={path} stroke={strokeColor} strokeWidth={strokeWidth} fill={fillColor} strokeLinecap="round" strokeLinejoin="round" />
 </svg>
 </div>
 </CardContent>
 </GlassCard>
