...
export default function UrlEncoderClient() {
  ...
  const output = useMemo(() => {
    ...
    try {
      ...
      return mode === "encode" ? "⚠️ Unable to encode input." : "⚠️ Invalid URL-encoded string. Check for malformed % sequences.";
    }
  }, [input, mode]);
  const handleSwap = () => {
 ...
 <textarea value={input} onChange={e => setInput(e.target.value)} rows={8} className="w-full rounded-lg border border-border/70 bg-background/80 p-3 font-mono text-sm outline-none focus:ring-2 focus:ring-primary/50" placeholder={mode === "encode" ? "hello world & foo=bar" : "hello%20world%20%26%20foo%3Dbar"} />
