...
import { Braces, Check, XCircle, Sparkles, Shield, Zap, AlignLeft, Copy } from "lucide-react";
...
export default function JsonFormatterClient() {
 ...
 <CardContent className="p-3 sm:p-4 space-y-3">
 {error ? <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-500 font-mono">
 {error}
 </div> : <>
 <textarea value={output} readOnly rows={12} className="w-full rounded-lg border border-border/70 bg-muted/30 p-3 font-mono text-sm outline-none" />
 ...
 <Button onClick={() => setMode("format")} className={cn(`px-4 py-2 text-sm font-medium ${mode === "format" ? "bg-primary text-primary-foreground" : "bg-background/80 hover:bg-muted/40"}`)}>
 Format
 </Button>
 <Button onClick={() => setMode("minify")} className={cn(`px-4 py-2 text-sm font-medium ${mode === "minify" ? "bg-primary text-primary-foreground" : "bg-background/80 hover:bg-muted/40"}`)}>
 Minify
 </Button>
 </div>
 {mode === "format" && <div className="inline-flex overflow-hidden rounded-lg border border-border/70">
 <Button onClick={() => setIndent(2)} className={cn(`px-4 py-2 text-sm font-medium ${indent === 2 ? "bg-primary text-primary-foreground" : "bg-background/80 hover:bg-muted/40"}`)}>
 2-space
 </Button>
 <Button onClick={() => setIndent(4)} className={cn(`px-4 py-2 text-sm font-medium ${indent === 4 ? "bg-primary text-primary-foreground" : "bg-background/80 hover:bg-muted/40"}`)}>
 4-space
 </Button>
 </div>}
 </div>
