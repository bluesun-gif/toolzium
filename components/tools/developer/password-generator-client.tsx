...
function evaluateStrength(length: number, useUpper: boolean, useLower: boolean, useNumbers: boolean, useSymbols: boolean): {
  label: string;
  color: string;
  percent: number;
} {
...
export default function PasswordGeneratorClient() {
  ...
  const strength = useMemo(() => evaluateStrength(length, useUpper, useLower, useNumbers, useSymbols), [length, useUpper, useLower, useNumbers, useSymbols]);
 ...
 <input type="range" min={8} max={64} value={length} onChange={e => setLength(parseInt(e.target.value, 10))} className="w-full accent-primary" />
 ...
 {[{
              ...
              value: useSymbols,
              set: setUseSymbols
            }].map(opt => <label key={opt.label} className="flex cursor-pointer items-center gap-2 rounded-lg border border-border/60 bg-background/60 p-3 text-sm hover:bg-muted/40">
 ...
 <div className={`h-full ${strength.color} transition-all`} style={{
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Set Length",
        description: "Choose a password length between 8 and 64 characters. Longer is always stronger.",
        icon: Lock
      }, {
        step: "02",
        title: "Pick Character Sets",
        description: "Enable uppercase, lowercase, numbers, and symbols based on your requirements.",
        icon: Key
      }, {
        step: "03",
        title: "Generate & Copy",
        description: "Click generate and copy the cryptographically secure password to your clipboard.",
        icon: Shield
 ...
 <ToolFeatureGuides features={[{
        icon: Lock,
        title: "CSPRNG",
        description: "Uses crypto.getRandomValues() for true cryptographic randomness, not Math.random()."
      }, {
        icon: Shield,
        title: "Strength Meter",
        description: "Calculates password entropy to show Weak, Fair, Strong, or Very Strong ratings."
      }, {
        icon: Key,
        title: "Customizable",
        description: "Full control over length and character sets to meet any password policy."
      }, {
        icon: RefreshCw,
        title: "Private",
        description: "Passwords are generated locally and never transmitted to any server."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Is Math.random() unsafe for passwords?",
        answer: "Yes. Math.random() is a predictable pseudo-random generator. An attacker who observes a few outputs can predict future values. Always use crypto.getRandomValues() for security purposes."
      }, {
        question: "How long should my password be?",
        answer: "Minimum 12 characters for general use, 16+ for sensitive accounts. Longer passwords made of random words (passphrases) are often easier to remember and just as secure."
      }, {
        question: "Are my passwords stored anywhere?",
        answer: "No. Passwords are generated locally in your browser memory. Nothing is transmitted, logged, or saved. Close the tab and the password is gone."
      }, {
