...
<meta property="og:description"content="${v.description}"/>`;
 ...
 <Input value={topic} onChange={e => setTopic(e.target.value)} placeholder="e.g., Best mechanical keyboards for programming" />
 </div>
 
 <div className="space-y-2">
 <Label>Page Type</Label>
 <select className="w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50" value={pageType} onChange={e => setPageType(e.target.value)}>
 {["Homepage", "Blog Post", "Product Page", "Service Page", "Landing Page", "Category Page"].map((t: string) => <option key={t} value={t}>{t}</option>)}
 </select>
 </div>

 <div className="space-y-2">
 <Label>Target Audience (Optional)</Label>
 <Input value={audience} onChange={e => setAudience(e.target.value)} placeholder="e.g., Senior software engineers" />
 </div>
 </div>
 ...
 {loading && <div className="space-y-4 mt-6 border-t border-border/50 pt-6">
 ...
 {variations.length > 0 && !loading && <div className="space-y-6 mt-6 border-t border-border/50 pt-6">
 <h3 className="font-semibold text-lg">Generated Variations (Click to edit)</h3>
 <div className="space-y-6">
 {variations.map((v, i) => {
                const rules = checkSeoRules(v);
                return <Card key={i} className="border border-border/50 bg-muted/10">
 ...
 <Input value={v.title} onChange={e => handleUpdateVariation(i, 'title', e.target.value)} className={`font-semibold ${rules.titleOk ? 'border-green-500/50' : 'border-yellow-500/50'}`} />
 </div>
 ...
 <textarea className={textareaClass} rows={3} value={v.description} onChange={e => handleUpdateVariation(i, 'description', e.target.value)} />
 </div>

 <div className="space-y-2">
 <Label>Keywords</Label>
 <Input value={v.keywords} onChange={e => handleUpdateVariation(i, 'keywords', e.target.value)} className="text-muted-foreground" />
 </div>
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Define Your Page Context",
        description: "Enter your primary keyword, page type, and target audience to give the AI precise context for generation.",
        icon: Wand2
      }, {
        step: "02",
        title: "Generate Variations",
        description: "Our AI engine crafts 3 distinct title and description combinations optimized for CTR and search intent.",
        icon: Loader2
      }, {
        step: "03",
        title: "Refine & Export",
        description: "Edit the generated tags inline to perfect them, then copy the complete HTML block to your CMS.",
        icon: Copy
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Tailors the tone and structure of meta tags based on whether you are writing a blog post, product page, or landing page."
      }, {
        icon: CheckCircle2,
        title: "Real-Time SEO Auditing",
        description: "Instantly validates your title and description lengths against Google's truncation limits to ensure maximum visibility."
      }, {
        icon: Copy,
        title: "Full HTML Export",
        description: "Copies a complete block including standard meta tags, OG tags, and Twitter cards with a single click."
      }, {
        icon: RotateCcw,
        title: "Inline Editing",
        description: "Not quite right? Click any generated title or description to manually tweak the copy while maintaining the character counters."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Does Google use meta keywords for ranking?",
        answer: "No, Google has publicly stated that the 'keywords' meta tag has no impact on search rankings. However, generating them is still useful for internal tagging systems or legacy CMS platforms."
      }, {
        question: "Will AI content be penalized by Google?",
        answer: "Google's guidelines state that they reward high-quality, helpful content regardless of how it is produced. Use the AI output as a strong foundation, but always review and edit to ensure it accurately reflects your page content."
      }, {
        question: "What is the perfect length for a title tag?",
        answer: "Aim for 50-60 characters. Google typically displays the first 600 pixels of a title, which roughly translates to 60 characters. Our tool highlights titles that exceed this limit."
      }, {
