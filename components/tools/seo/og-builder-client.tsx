...
export default function OgBuilderClient() {
  ...
  const generatedCode = useMemo(() => {
    ...
    if (ogDescription.trim()) lines.push(`<meta property="og:description"content="${escapeHtml(ogDescription.trim())}"/>`);
    ...
    if (ogDescription.trim()) lines.push(`<meta name="twitter:description"content="${escapeHtml(ogDescription.trim())}"/>`);
 ...
 <Input value={ogTitle} onChange={e => setOgTitle(e.target.value)} placeholder="e.g. The Best Free Marketing Tools" />
 </div>

 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">OG Description</label>
 <textarea value={ogDescription} onChange={e => setOgDescription(e.target.value)} rows={4} className={fieldClass} placeholder="Write a short description that looks good when shared..." />
 </div>
 ...
 <Input value={pageUrl} onChange={e => setPageUrl(e.target.value)} placeholder="https://example.com/article" />
 </div>
 </div>
 ...
 <select value={ogType} onChange={e => setOgType(e.target.value)} className={fieldClass}>
 ...
 <select value={twitterCard} onChange={e => setTwitterCard(e.target.value)} className={fieldClass}>
 ...
 {imageUrl ? <img src={imageUrl} alt="Social preview" className="h-44 w-full object-cover" /> : <div className="flex h-44 items-center justify-center bg-muted/30 text-sm text-muted-foreground">
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "A common recommendation is 1200x630 pixels, which works well across many social platforms."
      }, {
        question: "Do I need both Open Graph and Twitter Card tags?",
        answer: "It is recommended. Some platforms prefer Open Graph, while X uses Twitter Card metadata for best results."
      }, {
