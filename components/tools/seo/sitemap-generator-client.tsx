...
export default function SitemapGeneratorClient() {
  ...
  const sitemapXml = useMemo(() => {
    ...
    const urlBlocks = validRows.map(row => {
      return ["<url>", ` <loc>${escapeXml(row.url.trim())}</loc>`, row.lastmod ? ` <lastmod>${escapeXml(row.lastmod)}</lastmod>` : "", ` <changefreq>${escapeXml(row.changefreq)}</changefreq>`, ` <priority>${escapeXml(row.priority)}</priority>`, "</url>"].filter(Boolean).join("\n");
    }).join("\n");
 ...
 <ToolPageHeader icon={Map} title="Sitemap Generator" description="Create a valid XML sitemap with URL priority, change frequency, and last modified dates." />
 ...
 {rows.map(row => <div key={row.id} className="grid grid-cols-1 gap-3 rounded-xl border border-border/60 bg-background/60 p-3 md:grid-cols-[2fr_1fr_1fr_1fr_auto] md:items-end">
 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">URL</label>
 <Input value={row.url} onChange={e => updateRow(row.id, {
                url: e.target.value
 ...
 <select value={row.priority} onChange={e => updateRow(row.id, {
 ...
 <select value={row.changefreq} onChange={e => updateRow(row.id, {
                changefreq: e.target.value
              })} className={fieldClass}>
 {changeFrequencies.map(frequency => <option key={frequency} value={frequency}>
 {frequency}
 </option>)}
 </select>
 </div>
 ...
 <Input type="date" value={row.lastmod} onChange={e => updateRow(row.id, {
                lastmod: e.target.value
              })} />
 </div>

 <Button variant="outline" onClick={() => removeRow(row.id)} className="h-10 w-full md:w-auto">
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "Usually to your website root, such as example.com/sitemap.xml, or another publicly accessible URL."
      }, {
        question: "Do all websites need a sitemap?",
        answer: "Not always, but sitemaps are helpful for large, new, or complex websites."
      }, {
        question: "Does priority guarantee indexing?",
        answer: "No. Priority is a hint, not a command. Search engines decide what to crawl and index."
      }]} />
