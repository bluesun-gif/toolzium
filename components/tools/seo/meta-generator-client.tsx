...
const fieldClass = "w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50";
...
export default function MetaGeneratorClient() {
 ...
 <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Best Free Online SEO Tools" />
 </div>
 ...
 <textarea value={description} onChange={e => setDescription(e.target.value)} rows={4} className={fieldClass} placeholder="Write a concise, compelling description for search engines..." />
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">Keywords</label>
 <Input value={keywords} onChange={e => setKeywords(e.target.value)} placeholder="seo tools, meta tags" />
 </div>
 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">Author</label>
 <Input value={author} onChange={e => setAuthor(e.target.value)} placeholder="Toolzium Team" />
 </div>
 </div>
 ...
 <Input value={canonicalUrl} onChange={e => setCanonicalUrl(e.target.value)} placeholder="https://example.com/page" />
 </div>
 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">OG Image URL</label>
 <Input value={ogImage} onChange={e => setOgImage(e.target.value)} placeholder="https://example.com/og.png" />
 </div>
 </div>
 </CardContent>
 </GlassCard>
 ...
 {description || "Your meta description appears here. Keep it clear, relevant, and under 160 characters for best display in search results."}
 </p>
 </div>
 </CardContent>
 </GlassCard>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "Google does not use the meta keywords tag as a ranking signal, but some platforms and internal search systems may still parse it. The title and description are far more important."
      }, {
        question: "What is a canonical URL?",
        answer: "A canonical URL tells search engines which version of a page should be treated as the preferred one when duplicate or similar URLs exist."
      }, {
        question: "Should I include Open Graph tags?",
        answer: "Yes. Open Graph tags improve how your page appears when shared on social platforms and messaging apps."
      }, {
