...
const schemaTypes = ["Article", "FAQ", "Product", "LocalBusiness", "HowTo", "Recipe", "Event"] as const;
...
function getFallbackSchema(type: SchemaType, fields: Record<string, string>): string {
  ...
  if (type === "HowTo") {
    return JSON.stringify({
      ...base,
      step: (fields.steps || "Step one, Step two, Step three").split(",").map((step, index) => ({
        "@type": "HowToStep",
        position: index + 1,
        text: step.trim()
      }))
...
Return ONLY valid JSON-LD. No explanations, no markdown fences.`;
      ...
      setOutput(getFallbackSchema(schemaType, {
        name,
        description,
        url,
        image,
 ...
 <select value={schemaType} onChange={e => setSchemaType(e.target.value as SchemaType)} className={fieldClass}>
 {schemaTypes.map(type => <option key={type} value={type}>
 {type}
 </option>)}
 </select>
 </div>
 ...
 <Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Best SEO Practices" />
 </div>
 </div>

 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">Description</label>
 <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} className={fieldClass} placeholder="Describe the page, product, event, or business..." />
 </div>
 ...
 {extraFields.length > 0 && <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 {extraFields.map(field => <div key={field.key} className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">{field.label}</label>
 <Input value={extra[field.key] || ""} onChange={e => setExtra(prev => ({
                  ...prev,
 ...
 <a href="https://validator.schema.org/" target="_blank" rel="noreferrer" className="inline-flex h-9 items-center justify-center gap-2 rounded-md border border-border/70 px-4 text-sm font-medium hover:bg-muted/40">
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "No. Schema helps search engines understand content, but eligibility and display depend on the search engine."
      }, {
        question: "Where should I place JSON-LD?",
        answer: "Usually inside a script tag with type application/ld+json in the head or body of the page."
      }, {
