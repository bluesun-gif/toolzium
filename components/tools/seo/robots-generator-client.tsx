...
export default function RobotsGeneratorClient() {
  ...
  const updateRule = (id: string, patch: Partial<RobotsRule>) => {
    setRules(prev => prev.map(rule => rule.id === id ? {
      ...rule,
      ...patch
    } : rule));
  };
 ...
 <select value={userAgent} onChange={e => setUserAgent(e.target.value)} className={fieldClass}>
 ...
 {userAgent === "custom" && <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">Custom User-agent</label>
 <Input value={customAgent} onChange={e => setCustomAgent(e.target.value)} placeholder="e.g. DuckDuckBot" />
 </div>}
 </div>

 <div className="space-y-3">
 <label className="text-xs font-medium text-muted-foreground">Rules</label>
 {rules.map(rule => <div key={rule.id} className="grid grid-cols-1 gap-3 rounded-xl border border-border/60 bg-background/60 p-3 md:grid-cols-[140px_1fr_auto] md:items-end">
 <select value={rule.type} onChange={e => updateRule(rule.id, {
                  type: e.target.value as RobotsRule["type"]
                })} className={fieldClass}>
 <option value="Allow">Allow</option>
 <option value="Disallow">Disallow</option>
 </select>

 <Input value={rule.path} onChange={e => updateRule(rule.id, {
                  path: e.target.value
                })} placeholder="/private/" />
 ...
 <Input value={crawlDelay} onChange={e => setCrawlDelay(e.target.value)} placeholder="e.g. 10" />
 </div>
 <div className="space-y-2">
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "Not directly. It controls crawling, not necessarily indexing. Use noindex if you want to prevent indexing."
      }, {
