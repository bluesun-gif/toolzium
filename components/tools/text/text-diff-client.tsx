...
export default function TextDiffClient() {
  ...
  const stats = useMemo(() => {
    let added = 0,
      ...
      same = 0;
    diff.forEach(line => {
      if (line.type === "added") added++;else if (line.type === "removed") removed++;else same++;
    });
    return {
      added,
      removed,
 ...
 <textarea value={text1} onChange={e => setText1(e.target.value)} rows={10} className={textareaClass} placeholder="Paste original text here..." />
 </CardContent>
 </GlassCard>
 <GlassCard>
 <CardHeader className={headerClass}>
 ...
 <CardContent className="p-3 sm:p-4">
 <textarea value={text2} onChange={e => setText2(e.target.value)} rows={10} className={textareaClass} placeholder="Paste modified text here..." />
 </CardContent>
 </GlassCard>
 </div>
 ...
 {diff.map((line, idx) => <div key={idx} className={`px-3 py-1 ${getBgClass(line.type)}`}>
 {line.type === "added" && "+"}
 {line.type === "removed" && "-"}
 {line.type === "same" && ""}
 {line.text || "\u00A0"}
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Input Versions",
        description: "Paste the original draft in the left box and the revised version in the right box.",
        icon: FileText
      }, {
        step: "02",
        title: "Run Comparison",
        description: "Click the compare button to analyze the differences line by line.",
        icon: Search
      }, {
        step: "03",
        title: "Review Changes",
        description: "Identify exactly what was added, deleted, or kept the same with color coding.",
        icon: BarChart3
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Compares documents sequentially to highlight structural changes and edits."
      }, {
        icon: Search,
        title: "Visual Highlighting",
        description: "Uses green for additions and red with strikethrough for deletions for instant recognition."
      }, {
        icon: BarChart3,
        title: "Change Statistics",
        description: "Provides a quick summary of how many lines were added, removed, or left untouched."
      }, {
        icon: FileText,
        title: "Code & Prose Friendly",
        description: "Works equally well for comparing programming scripts, essays, or configuration files."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "This tool performs a strict line-by-line comparison. If a single word changes on a line, the entire line will be marked as removed and re-added."
      }, {
        question: "Are empty lines counted in the diff?",
        answer: "Yes, empty lines are treated as valid lines of text. Adding or removing blank lines will register as a change in the diff output."
      }, {
