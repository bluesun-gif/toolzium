...
export default function TextRepeaterClient() {
  ...
  const handleGenerate = () => {
    ...
    let sep = "";
    if (separator === "newline") sep = "\n";else if (separator === "space") sep = " ";else if (separator === "comma") sep = ", ";else if (separator === "custom") sep = customSep;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
          ...
          <GlassCard className="p-5 flex flex-col bg-background border-border shadow-sm rounded-2xl">
            ...
            <div className="space-y-4 flex-1">
              <div className="space-y-1.5">
                ...
                <Input value={text} onChange={e => setText(e.target.value)} placeholder="e.g. Hello World or 🚀 Fire" className="bg-background border-border text-foreground" />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-muted-foreground">Repeat Count (1–10,000)</Label>
                  <Input type="number" min={1} max={10000} value={count} onChange={e => setCount(parseInt(e.target.value) || 1)} className="bg-background border-border text-foreground font-mono" />
                </div>

                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-muted-foreground">Separator</Label>
                  <select value={separator} onChange={e => setSeparator(e.target.value)} className="w-full rounded-xl border border-border bg-background p-2.5 text-xs text-foreground font-medium outline-none focus:ring-2 focus:ring-primary/50">
              ...
              {separator === "custom" && <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-muted-foreground">Custom Separator String</Label>
                  <Input value={customSep} onChange={e => setCustomSep(e.target.value)} placeholder="e.g. | or --- or ⭐" className="bg-background border-border text-foreground font-mono" />
                </div>}

              <Button onClick={handleGenerate} className="w-full gap-2 mt-4 bg-primary hover:bg-primary/90 text-primary-foreground font-bold shadow-lg shadow-primary/20 rounded-xl h-12 text-base">
          ...
          <GlassCard className="p-5 flex flex-col bg-card border-border shadow-sm rounded-2xl min-h-[380px]">
            ...
            <div className="flex-1">
              <textarea value={output} readOnly rows={12} placeholder="Click 'Generate Repeated Text' on the left to display output here..." className="w-full rounded-xl border border-border bg-muted/30 p-3.5 text-xs font-mono outline-none text-foreground min-h-[280px]" />
            </div>
          </GlassCard>
        </div>
...
<ToolHowItWorks steps={[{
          ...
          description: "Choose how many times to repeat it and select the separator between instances.",
          icon: Settings
        }, {
          step: "03",
          title: "Generate & Copy",
          description: "Click generate to build the string and copy it to your clipboard instantly.",
          icon: Copy
        }]} badges={["100% Free", "Up to 10,000 Repeats", "No Signup Required"]} />

        <ToolFeatureGuides features={[{
          icon: Repeat,
          title: "High Volume Multiplier",
          description: "Generates up to 10,000 repetitions instantly without browser lag."
        }, {
          icon: Settings,
          title: "Custom Separators",
          description: "Join repeated text with newlines, spaces, commas, or custom characters."
        }, {
          icon: Copy,
          title: "One-Click Copy",
          description: "Copy massive generated text blocks straight to your clipboard."
        }]}>
        ...
        <ToolFaqAccordion faqs={[{
          ...
          answer: "You can generate up to 10,000 repetitions in a single click."
        }, {
