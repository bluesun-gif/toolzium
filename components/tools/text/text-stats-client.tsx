...
export function TextStatsClient() {
  ...
  const stats = useMemo(() => {
    ...
    const avgWordLength = wordsOnlyAlpha.length > 0 ? wordsOnlyAlpha.reduce((acc, w) => acc + w.length, 0) / wordsOnlyAlpha.length : 0;
    ...
    const fleschKincaid = wordCount > 0 && sentenceCount > 0 ? 0.39 * (wordCount / sentenceCount) + 11.8 * (syllables / wordCount) - 15.59 : 0;
 ...
 <textarea className="w-full h-full min-h-[300px] p-4 rounded-md border bg-background resize-none focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="Paste or type your text here to analyze..." value={text} onChange={e => setText(e.target.value)} />
 </CardContent>
 </GlassCard>
 ...
 <CardContent>
 {stats.topWords.length > 0 ? <div className="flex flex-wrap gap-2">
 {stats.topWords.map(([word, count]) => <div key={word} className="px-2 py-1 bg-secondary rounded text-xs flex gap-2 items-center">
 <span>{word}</span>
 <span className="bg-background px-1.5 rounded-sm text-muted-foreground">{count}</span>
 </div>)}
 </div> : <div className="text-sm text-muted-foreground">No words to display</div>}
 </CardContent>
 </GlassCard>
 </div>
 </div>
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Paste or type any text — an article, essay, email, social post, or document. All statistics update instantly with no button press required.",
        icon: Type
      }, {
        step: "02",
        title: "Review Detailed Statistics",
        description: "Analyze characters, words, sentences, paragraphs, unique words, average word length, syllable count, Flesch reading ease score, and keyword frequency.",
        icon: BarChart2
      }, {
        step: "03",
        title: "Optimize Your Writing",
        description: "Use reading level, keyword density, and sentence length insights to improve clarity, SEO performance, and audience targeting of your content.",
        icon: BookOpen
      }]} badges={["Flesch reading ease", "Keyword density", "Reading time"]} />
 <ToolFeatureGuides features={[{
        icon: BarChart2,
        title: "30+ Text Statistics",
        description: "Characters (with/without spaces), words, unique words, sentences, paragraphs, lines, syllables, average word length, average sentence length, and more — all computed in real time."
      }, {
        icon: BookOpen,
        title: "Flesch Reading Ease Score",
        description: "Measures text readability on a 0-100 scale. 90-100 is very easy (5th grade), 60-70 is standard (8th-9th grade), below 30 is very difficult (university level). Ideal for matching content to your audience."
      }, {
        icon: Clock,
        title: "Reading and Speaking Time",
        description: "Estimates reading time at 238 WPM (average adult silent reading speed per Brysbaert 2019) and speaking time at 130 WPM (presentation pace). Useful for blog posts, speeches, and podcast scripts."
      }, {
        icon: Hash,
        title: "Keyword Frequency Analysis",
        description: "Shows the top 10 most frequent words with counts and density percentage. Identifies keyword stuffing above 3% density and overused filler words that weaken content quality."
      }, {
        icon: AlignLeft,
        title: "Sentence Length Analysis",
        description: "Counts average words per sentence and flags very long sentences (over 30 words) that may reduce readability. Ideal sentence length for web content is 15-20 words."
      }, {
        icon: Shield,
        title: "Private and Client-Side",
        description: "All analysis runs locally in your browser using JavaScript string operations. No text is ever sent to a server. Safe for confidential documents and proprietary content."
      }]}>
 ...
 <tbody>
 {[["90-100", "Very Easy", "5th grade", "Comic books, basic instructions"], ["80-90", "Easy", "6th grade", "Pulp fiction, consumer guides"], ["70-80", "Fairly Easy", "7th grade", "Popular novels"], ["60-70", "Standard", "8th-9th grade", "Newspapers, magazines"], ["50-60", "Fairly Difficult", "10th-12th grade", "Academic magazines"], ["30-50", "Difficult", "College level", "Academic writing"], ["0-30", "Very Difficult", "Professional", "Legal, scientific documents"]].map(([score, diff, grade, ex]) => <tr key={score} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["Blog post (general)", "60-70", "15-20 words", "800-1,500"], ["SEO article (long-form)", "60-70", "15-20 words", "1,500-3,000"], ["Email newsletter", "65-75", "12-18 words", "200-500"], ["Social media post", "70-80", "8-12 words", "50-280"], ["Academic paper", "30-50", "20-30 words", "3,000-8,000"], ["Legal document", "10-30", "25-40 words", "Varies"], ["Product description", "65-75", "10-15 words", "50-300"]].map(([type, flesch, sent, wc]) => <tr key={type} className="odd:bg-muted/20">
 ...
 <ToolFaqAccordion faqs={[{
        ...
        question: "What is keyword density and what is the ideal percentage?",
        answer: "Keyword density is the percentage of times a specific word appears relative to the total word count. A density of 1-3% for a primary keyword is generally considered optimal for SEO. Below 1% may mean the topic is not well covered. Above 3% risks being identified as keyword stuffing by search engine algorithms, which can negatively impact rankings. Use the keyword frequency list to monitor your primary and secondary keyword density."
      }, {
