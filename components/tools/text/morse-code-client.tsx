...
export default function MorseCodeClient() {
  ...
  const output = useMemo(() => {
    ...
    if (mode === "text-to-morse") {
      return input.toUpperCase().split("").map(word => word.split("").map(char => MORSE_MAP[char] || "").filter(Boolean).join("")).join("/");
    } else {
      return input.split("/").map(word => word.trim().split("").map(morse => REVERSE_MORSE_MAP[morse] || "").join("")).join("");
    }
  }, [input, mode]);
 ...
 <textarea value={input} onChange={e => setInput(e.target.value)} rows={6} className="w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50" placeholder={mode === "text-to-morse" ? "Enter text to convert to Morse code..." : "Enter Morse code (e.g., .... . .-.. .-.. --- / .-- --- .-. .-.. -..)"} />
 ...
 <textarea value={output} readOnly rows={6} className="w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50" />
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Type your message or paste Morse code into the input field.",
        icon: Radio
      }, {
        step: "02",
        title: "Select Conversion Mode",
        description: "Choose between converting text to Morse or decoding Morse back to text.",
        icon: ArrowRightLeft
      }, {
        step: "03",
        title: "Copy Result",
        description: "Get the translated output instantly and copy it with a single click.",
        icon: Copy
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Uses the official International Morse Code standard with all letters, numbers, and common punctuation."
      }, {
        icon: ArrowRightLeft,
        title: "Bidirectional Translation",
        description: "Seamlessly switch between text-to-Morse and Morse-to-text modes."
      }, {
        icon: Copy,
        title: "Word Separation",
        description: "Characters are separated by spaces and words by ' / ' for clear readability."
      }, {
        icon: Radio,
        title: "Real-Time Processing",
        description: "See the translation results update live as you type."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "What Morse code standard does this tool use?",
        answer: "This tool uses the International Morse Code standard, which is the most widely recognized and used worldwide."
      }, {
        question: "How are words separated in the output?",
        answer: "Characters within a word are separated by single spaces, while words are separated by ' / ' (space-slash-space)."
      }, {
