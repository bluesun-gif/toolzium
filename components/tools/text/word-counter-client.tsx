...
import { FileText, Clock, BarChart3, List, Search } from "lucide-react";
...
export default function WordCounterClient() {
 ...
 <textarea value={text} onChange={e => setText(e.target.value)} rows={10} className={textareaClass} placeholder="Paste your essay, article, or speech transcript here..." />
 </CardContent>
 </GlassCard>

 <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
 {statItems.map(item => <Card key={item.label} className={cardClass}>
 ...
 {stats.top10.length > 0 && <GlassCard>
 <CardHeader className={headerClass}>
 <CardTitle className={titleClass}><List className="w-4 h-4 text-primary" /> Top 10 Most Frequent Words</CardTitle>
 </CardHeader>
 <CardContent className="p-3 sm:p-4">
 <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
 {stats.top10.map(([word, count], idx) => <div key={word} className="flex items-center justify-between p-2 rounded-lg bg-muted/40 border border-border/50">
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Paste Content",
        description: "Drop your article, essay, or speech transcript into the text area.",
        icon: FileText
      }, {
        step: "02",
        title: "View Analytics",
        description: "Instantly see word counts, reading times, and structural metrics.",
        icon: BarChart3
      }, {
        step: "03",
        title: "Analyze Density",
        description: "Review the top keywords to ensure your content is focused and optimized.",
        icon: List
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Tracks words, characters, sentences, and paragraphs simultaneously."
      }, {
        icon: Clock,
        title: "Time Estimations",
        description: "Calculates reading time (200 wpm) and speaking time (130 wpm) for presentations."
      }, {
        icon: List,
        title: "Keyword Density",
        description: "Identifies the top 10 most used words to help avoid repetition and improve SEO."
      }, {
        icon: BarChart3,
        title: "Live Updates",
        description: "Metrics recalculate dynamically with every keystroke for immediate feedback."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "Reading time is estimated based on the average adult reading speed of 200 words per minute (wpm). Speaking time uses a slower average of 130 wpm."
      }, {
        question: "Are stop words included in the top 10 list?",
        answer: "Yes, this tool counts all words including common stop words like 'the', 'and', and 'is'. This gives a raw frequency map of your text."
      }, {
        question: "Does it count hyphenated words as one or two?",
        answer: "Hyphenated words are generally counted as a single word by our regex matching algorithm, which aligns with most standard word processors."
      }]} />
