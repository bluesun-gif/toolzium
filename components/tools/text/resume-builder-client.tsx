...
interface Experience { company: string; role: string; from: string; to: string; bullets: string[]; }
interface Education { institution: string; degree: string; field: string; from: string; to: string; gpa: string; }
...
interface ResumeData {
  personal: { name: string; title: string; email: string; phone: string; location: string; website: string; linkedin: string; };
  summary: string;
...
const initialData: ResumeData = {
  personal: { name: "Jane Doe", title: "Senior Frontend Engineer", email: "jane@example.com", phone: "555-0100", location: "New York, NY", website: "janedoe.dev", linkedin: "linkedin.com/in/janedoe" },
  ...
  experience: [{ company: "TechCorp", role: "Lead Developer", from: "2020", to: "Present", bullets: ["Led a team of 5 developers.", "Increased performance by 40%."] }],
  education: [{ institution: "MIT", degree: "B.S.", field: "Computer Science", from: "2014", to: "2018", gpa: "3.9" }],
  skills: [{ name: "React", level: "Expert" }, { name: "TypeScript", level: "Expert" }],
  projects: [{ name: "Toolzium", desc: "Collection of dev tools.", tech: "Next.js, React", link: "toolzium.com" }],
  certs: [{ name: "AWS Certified", issuer: "Amazon", date: "2022" }]
};
...
export function ResumeBuilderClient() {
  ...
  const addExp = () => setData((p) => ({ ...p, experience: [...p.experience, { company: "", role: "", from: "", to: "", bullets: [""] }] }));
  const removeExp = (i: number) => setData((p) => ({ ...p, experience: p.experience.filter((_, idx) => idx !== i) }));
  ...
  const addBullet = (i: number) => setData((p) => { const n = [...p.experience]; n[i].bullets.push(""); return { ...p, experience: n }; });
  const updateBullet = (i: number, j: number, v: string) => { setData((p) => { const n = [...p.experience]; n[i].bullets[j] = v; return { ...p, experience: n }; }); };
  const removeBullet = (i: number, j: number) => { setData((p) => { const n = [...p.experience]; n[i].bullets = n[i].bullets.filter((_, idx) => idx !== j); return { ...p, experience: n }; }); };
  ...
  const moveItem = (arrName: keyof ResumeData, index: number, dir: "up" | "down") => {
    setData((p) => {
      ...
      return { ...p, [arrName]: arr };
    });
  };

  const addEdu = () => setData((p) => ({ ...p, education: [...p.education, { institution: "", degree: "", field: "", from: "", to: "", gpa: "" }] }));
  const removeEdu = (i: number) => setData((p) => ({ ...p, education: p.education.filter((_, idx) => idx !== i) }));
  const updateEdu = (i: number, field: string, value: string) => { setData((p) => { const n = [...p.education]; (n[i] as any)[field] = value; return { ...p, education: n }; }); };

  const addSkill = () => setData((p) => ({ ...p, skills: [...p.skills, { name: "", level: "Beginner" }] }));
  const removeSkill = (i: number) => setData((p) => ({ ...p, skills: p.skills.filter((_, idx) => idx !== i) }));
  const updateSkill = (i: number, field: string, value: string) => { setData((p) => { const n = [...p.skills]; (n[i] as any)[field] = value; return { ...p, skills: n }; }); };

  const exportHTML = () => {
    const html = document.getElementById("resume-preview")?.innerHTML || "";
    const blob = new Blob([`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Resume</title><style>body{font-family:sans-serif;padding:40px;max-width:800px;margin:auto;}</style></head><body>${html}</body></html>`], { type: "text/html" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "resume.html"; a.click(); URL.revokeObjectURL(url);
    ...
    <div className="max-w-7xl mx-auto space-y-8 p-4">
      ...
      <div className="grid lg:grid-cols-2 gap-8 mb-8">
          ...
          <Card className={cardClass}>
            ...
            <CardContent className="p-4 space-y-4">
              {data.experience.map((exp, i) => (
                <div key={i} className="p-3 border rounded space-y-2 bg-muted/10">
        ...
        <div className="lg:sticky lg:top-20 self-start">
          ...
          <div id="resume-preview" className={`p-8 bg-white text-gray-900 rounded-lg shadow-xl min-h-[800px] ${fontClass} print:shadow-none`}>
            <header className="mb-6 border-b-2 pb-4" style={{borderColor: accent}}>
              <h1 className="text-3xl font-bold mb-1" style={{color: accent}}>{data.personal.name}</h1>
            ...
            {data.summary && (
              <section className="mb-6">
                <h2 className="text-lg font-bold mb-2 uppercase tracking-wide" style={{color: accent}}>Summary</h2>
            ...
            {data.experience.length > 0 && (
              <section className="mb-6">
                <h2 className="text-lg font-bold mb-3 uppercase tracking-wide" style={{color: accent}}>Experience</h2>
                {data.experience.map((exp, i) => (
                  <div key={i} className="mb-4">
                    <div className="flex justify-between font-bold text-sm">
                      <span>{exp.role} <span className="font-normal text-gray-600">at {exp.company}</span></span>
                      <span className="text-gray-500 font-normal">{exp.from} - {exp.to}</span>
                    </div>
            ...
            {data.education.length > 0 && (
              <section className="mb-6">
                <h2 className="text-lg font-bold mb-3 uppercase tracking-wide" style={{color: accent}}>Education</h2>
                {data.education.map((edu, i) => (
                  <div key={i} className="mb-2">
                    <div className="flex justify-between font-bold text-sm">
                      <span>{edu.degree} {edu.field} <span className="font-normal text-gray-600">- {edu.institution}</span></span>
                      <span className="text-gray-500 font-normal">{edu.from} - {edu.to}</span>
                    </div>
            ...
            {data.skills.length > 0 && (
              <section>
                <h2 className="text-lg font-bold mb-2 uppercase tracking-wide" style={{color: accent}}>Skills</h2>
                <div className="flex flex-wrap gap-2">
                  {data.skills.map((sk, i) => (
                    <span key={i} className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs border border-gray-200">{sk.name} ({sk.level})</span>
                  ))}
                </div>
              </section>
            )}
      ...
      <ToolHowItWorks steps={[
        ...
        { step: "02", title: "Customize", description: "Choose a template style, pick an accent color, and reorder your entries.", icon: Plus },
        { step: "03", title: "Export", description: "Review the live preview and export your resume as HTML or print it directly.", icon: Download }
      ]} />

      <ToolFeatureGuides features={[
        { icon: FileText, title: "Live Preview", description: "See exactly how your resume looks as you type, with instant updates to the formatted document." },
        { icon: Plus, title: "Multiple Templates", description: "Switch between Professional (serif), Modern (sans-serif), and Minimal (monospace) styles." },
        { icon: Download, title: "Custom Accent Colors", description: "Use the color picker to match your personal brand or apply a subtle, professional tint." },
        { icon: ArrowUp, title: "Drag & Reorder", description: "Easily move experience entries up and down to prioritize your most relevant work history." }
      ]}>
      ...
      <ToolFaqAccordion faqs={[
        { question: "Is my data stored anywhere?", answer: "No. All your information is stored locally in your browser's memory. Once you close the tab, the data is cleared for your privacy." },
        { question: "Can I print my resume as a PDF?", answer: "Yes! Simply click the Print button and select 'Save as PDF' as your printer destination in the browser dialog." },
        { question: "Are these templates ATS friendly?", answer: "Yes. The underlying HTML structure is clean and semantic, ensuring Applicant Tracking Systems (ATS) can easily parse your text." },
