...
export default function TextCleanerClient() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 px-2 sm:px-4 py-4 sm:py-6">
          ...
          <CardContent className="p-3 sm:p-4 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                { key: "lineBreaks", label: "Remove all line breaks" },
                { key: "emptyLines", label: "Remove empty lines" },
                { key: "trimLines", label: "Trim whitespace per line" },
                { key: "htmlTags", label: "Strip HTML tags" },
                { key: "specialChars", label: "Remove special characters" },
              ].map((opt) => (
                <label key={opt.key} className="flex items-center gap-2 text-sm cursor-pointer hover:text-primary transition-colors">
                  <input
                    type="checkbox"
      ...
      <ToolHowItWorks
        steps={[
          { step: "01", title: "Paste Messy Text", description: "Input text with irregular spacing, HTML artifacts, or unwanted line breaks.", icon: FileInput },
          { step: "02", title: "Select Filters", description: "Toggle the specific cleaning rules you want to apply to your document.", icon: Settings },
          { step: "03", title: "Get Clean Data", description: "Click clean to process the text and copy the perfectly formatted result.", icon: Sparkles },
        ]}
      ...
      <ToolFeatureGuides
        features={[
          { icon: Eraser, title: "HTML Stripping", description: "Safely removes all HTML tags while preserving the inner text content for clean reading." },
          { icon: Settings, title: "Granular Control", description: "Mix and match different cleaning rules to achieve the exact format you need." },
          { icon: FileInput, title: "Whitespace Management", description: "Collapses multiple spaces into one and trims trailing/leading whitespace automatically." },
          { icon: Sparkles, title: "Data Preparation", description: "Ideal for cleaning scraped data, PDF exports, or copied web text before analysis." },
        ]}
      >
      ...
      <ToolFaqAccordion
        faqs={[
          { question: "Will removing special characters delete punctuation?", answer: "The 'Remove special characters' filter preserves standard sentence punctuation like periods, commas, and question marks, but removes symbols like @, #, $, and emojis." },
          { question: "Can I undo the cleaning process?", answer: "The tool does not have an undo button, but your original text remains in the left input box. You can simply adjust the options and click 'Clean Text' again." },
          { question: "Does this work offline?", answer: "Yes. Because the text processing relies entirely on browser-based JavaScript, it works perfectly without an internet connection once the page is loaded." },
        ]}
      />
