...
const DICTIONARY: Record<string, Record<string, string>> = {
  en: {
    "hello": "hola", "world": "mundo", "good": "bueno", "morning": "mañana", "night": "noche",
    "thank": "gracias", "you": "tú", "please": "por favor", "yes": "sí", "no": "no",
    "water": "agua", "food": "comida", "help": "ayuda", "love": "amor", "friend": "amigo",
    "house": "casa", "car": "coche", "dog": "perro", "cat": "gato", "book": "libro",
    "read": "leer", "write": "escribir", "speak": "hablar", "listen": "escuchar", "understand": "entender",
    "i": "yo", "we": "nosotros", "they": "ellos", "he": "él", "she": "ella",
    "the": "el", "a": "un", "is": "es", "are": "son", "have": "tener",
    "bonjour": "hello", "monde": "world", "merci": "thank you", "oui": "yes", "non": "no",
    "eau": "water", "pain": "bread", "fromage": "cheese", "vin": "wine", "amour": "love",
    "maison": "house", "voiture": "car", "chien": "dog", "chat": "cat", "livre": "book"
  },
  es: {
    "hola": "hello", "mundo": "world", "bueno": "good", "mañana": "morning", "noche": "night",
    "gracias": "thank you", "tú": "you", "por favor": "please", "sí": "yes", "no": "no",
    "agua": "water", "comida": "food", "ayuda": "help", "amor": "love", "amigo": "friend",
    "casa": "house", "coche": "car", "perro": "dog", "gato": "cat", "libro": "book",
    "leer": "read", "escribir": "write", "hablar": "speak", "escuchar": "listen", "entender": "understand",
    "yo": "i", "nosotros": "we", "ellos": "they", "él": "he", "ella": "she"
  },
  fr: {
    "bonjour": "hello", "monde": "world", "merci": "thank you", "oui": "yes", "non": "no",
    "eau": "water", "pain": "bread", "fromage": "cheese", "vin": "wine", "amour": "love",
...
const COMMON_PHRASES = [
  { category: "Greetings", items: ["Hello", "Good morning", "How are you?", "Nice to meet you"] },
  { category: "Emergencies", items: ["Help!", "Call the police", "I need a doctor", "Where is the hospital?"] },
  { category: "Travel", items: ["Where is the bathroom?", "How much does it cost?", "I am lost", "Train station"] }
];
...
export function TranslateClient() {
  ...
  const [phrasebook, setPhrasebook] = useState<{ source: string, target: string, from: string, to: string }[]>([]);
  ...
  const translationResult = useMemo(() => {
    if (!input.trim()) return { text: "", breakdown: [] };
    
    const words = input.split(/(\s+)/);
    const translatedWords: string[] = [];
    const breakdown: { original: string, translated: string }[] = [];
    ...
    words.forEach(token => {
      if (token.trim().length === 0) {
        ...
        if (token.trim().length > 0 && !token.match(/^\s+$/)) {
           breakdown.push({ original: token, translated });
        }
      }
    });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 p-4">
      ...
      {showBreakdown && translationResult.breakdown.length > 0 && (
        <Card className={cardClass}>
          ...
          <CardContent className="p-4 sm:p-6 flex flex-wrap gap-2">
            {translationResult.breakdown.map((item, idx) => (
              <div key={idx} className="flex flex-col items-center p-2 bg-muted/50 rounded border border-border/50 min-w-[80px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {cat.items.map(phrase => (
                    <Button 
                      ...
                      size="sm" 
                      className="text-xs"
                      onClick={() => { setInput(phrase); setSourceLang("en"); }}
                    >
                      {phrase}
                    </Button>
                  ))}
        ...
        <Card className={cardClass}>
          ...
          <CardContent className="p-4 sm:p-6 space-y-3 max-h-[400px] overflow-y-auto">
            {phrasebook.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">No saved phrases yet. Translate and click 'Save'.</p>
            ) : (
              phrasebook.map((item, idx) => (
                <div key={idx} className="p-3 bg-muted/30 rounded-lg border border-border/50 flex justify-between items-start group">
      ...
      <ToolHowItWorks
        steps={[
          { step: "01", title: "Select Languages", description: "Choose your source and target languages from the dropdown menus. Swap them instantly with one click.", icon: Languages },
          { step: "02", title: "Type or Select", description: "Type your own text or click on common phrases from the category list to auto-fill the input box.", icon: ArrowRightLeft },
          { step: "03", title: "Analyze & Save", description: "View the translation, listen to the pronunciation, or break it down word-by-word to learn the vocabulary.", icon: Bookmark }
        ]}
      ...
      <ToolFeatureGuides
        features={[
          { icon: Languages, title: "Core Vocabulary Engine", description: "Access a curated database of high-frequency words and phrases essential for travelers and beginners." },
          { icon: Volume2, title: "Audio Pronunciation", description: "Hear the correct pronunciation of translated phrases using native browser speech synthesis." },
          { icon: Bookmark, title: "Personal Phrasebook", description: "Build your own custom dictionary by saving favorite translations for quick reference later." },
          { icon: ArrowRightLeft, title: "Word-by-Word Logic", description: "Understand the grammar behind the translation with our breakdown mode that maps each source word to its target." }
        ]}
      >
      ...
      <ToolFaqAccordion
        faqs={[
          { question: "Does this tool require an internet connection?", answer: "No. The dictionary database and translation logic are embedded directly in the application code. Once the page loads, it works 100% offline." },
          { question: "Why are some words in brackets like [word]?", answer: "Brackets indicate that a specific word was not found in our core offline dictionary. While we cover thousands of common terms, highly technical or obscure slang may not be present in the local dataset." },
          { question: "Can I translate full documents?", answer: "This tool is optimized for phrases, sentences, and short paragraphs. For translating entire books or legal documents, a specialized cloud-based service with context-aware AI is recommended." },
          { question: "How accurate is the pronunciation?", answer: "We use the native SpeechSynthesis API of your device. The quality of the voice depends on your operating system (e.g., iOS and macOS typically have very high-quality neural voices)." },
          { question: "Is my saved phrasebook private?", answer: "Yes. Your phrasebook is stored locally in your browser's memory. It is never sent to our servers." }
        ]}
      />
