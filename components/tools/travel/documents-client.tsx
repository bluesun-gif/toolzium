...
import { CheckSquare, Plus, Copy, Plane } from"lucide-react";
...
export function TravelDocumentsClient() {
 ...
 setItems([...items, { id: Date.now().toString(), text: newItem.trim(), completed: false }]);
 setNewItem("");
 };
 ...
 description="Never forget a crucial document again. Comprehensive checklists for any trip."
 icon={CheckSquare}
 actions={
 ...
 {items.map(item => (
 <div key={item.id} className={"flex items-center justify-between p-3 rounded-lg border transition-colors"+ (item.completed ?"bg-muted/50":"bg-card")}>
 ...
 {items.length === 0 && (
 <div className="text-center py-8 text-muted-foreground">List is empty.</div>
 )}
 </div>
 </CardContent>
 </GlassCard>
