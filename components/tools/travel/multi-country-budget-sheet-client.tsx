...
export function MultiCountryBudgetSheetClient() {
 ...
 { id:"1", name:"Country 1", currency:"EUR", rateToHome: 1.1 }
 ]);
 ...
 const saveToLocal = () => {
 localStorage.setItem("travel-budget", JSON.stringify({ homeCurrency, countries, expenses }));
 toast.success("Budget saved!");
 };

 const resetForm = () => {
 setHomeCurrency("USD");
 setCountries([{ id:"1", name:"Country 1", currency:"EUR", rateToHome: 1.1 }]);
 setExpenses([]);
 ...
 setCountries([...countries, { id: Date.now().toString(), name:"New Country", currency:"GBP", rateToHome: 1.25 }]);
 };
 ...
 setExpenses([...expenses, { id: Date.now().toString(), category: CATEGORIES[0], amount: 0, countryId }]);
 };
 ...
 <ToolPageHeader
 icon={Globe}
 title="Multi-Country Budget Sheet"
 description="Plan your travel expenses across multiple destinations with built-in currency conversion."
 actions={
 ...
 <div key={country.id} className="p-4 border rounded-md relative space-y-4">
 ...
 {countries.length > 1 && (
 <Button variant="ghost"size="icon"className="absolute top-2 right-2 text-destructive"onClick={() => removeCountry(country.id)}>
 ...
 return (
 <div key={country.id} className="flex justify-between p-2 bg-muted/50 rounded-md">
 ...
 <div className="text-muted-foreground text-xs">{homeTotal.toFixed(2)} {homeCurrency}</div>
      ...
      <RelatedTools currentToolUrl="/tools/travel/multi-country-budget-sheet" max={6} />
