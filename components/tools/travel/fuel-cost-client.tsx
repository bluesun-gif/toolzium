...
export function FuelCostClient() {
  ...
  const handleAddVehicle = () => {
    ...
    setVehicles([...vehicles, {
      ...
      name: newVehicleName,
      efficiency: Number(newVehicleEff)
    }]);
    setNewVehicleName("");
    setNewVehicleEff("");
 ...
 <Switch checked={isMetric} onCheckedChange={v => {
                setIsMetric(v);
 ...
 <Input type="number" min="0" value={distance} onChange={e => setDistance(e.target.value === "" ? "" : Number(e.target.value))} placeholder={`e.g. ${isMetric ? "150" : "100"}`} />
 </div>

 <div className="space-y-2">
 <Label>Fuel Price per {isMetric ? "Liter" : "Gallon"}</Label>
 <Input type="number" min="0" step="0.01" value={price} onChange={e => setPrice(e.target.value === "" ? "" : Number(e.target.value))} placeholder="e.g. 1.50" />
 </div>
 </CardContent>
 </GlassCard>
 ...
 <Input value={newVehicleName} onChange={e => setNewVehicleName(e.target.value)} placeholder="e.g. SUV" />
 </div>
 <div className="flex-1 space-y-2">
 <Label>Efficiency ({isMetric ? "L/100km" : "MPG"})</Label>
 <Input type="number" min="0" step="0.1" value={newVehicleEff} onChange={e => setNewVehicleEff(e.target.value === "" ? "" : Number(e.target.value))} placeholder={`e.g. ${isMetric ? "8.5" : "25"}`} />
 </div>
 <ActionButton onClick={handleAddVehicle} icon={Plus} label="Add" variant="default" size="default" />
 </div>

 <div className="space-y-3 mt-4">
 {vehicles.map(v => <div key={v.id} className="flex items-center justify-between p-3 border rounded-md">
 <div>
 ...
 {vehicles.map(v => {
              ...
              return <div key={`res-${v.id}`} className="p-4 border rounded-lg bg-card space-y-2">
