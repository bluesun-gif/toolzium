...
import { DollarSign, Globe, Calculator, Copy, Save } from "lucide-react";
                ...
                {CURRENCIES.filter(c => c !== baseCurrency).map(c => {
                  ...
                  return (
                    <button
                      ...
                      className={"px-3 py-1.5 rounded-full text-sm font-medium transition-colors border " + (isSelected ? "bg-primary text-primary-foreground border-primary" : "bg-background text-muted-foreground hover:bg-muted")}
                    >
                      {c}
                    </button>
                  );
