...
export function TravelBudgetMatrixClient() {
  ...
  const savePrefs = () => {
    localStorage.setItem("travel-budget-prefs", JSON.stringify({
      budget,
      baseCurrency,
      targetCurrencies
    }));
 ...
 {results.map(res => <div key={res.currency} className="p-4 rounded-lg border bg-card text-card-foreground shadow-sm">
