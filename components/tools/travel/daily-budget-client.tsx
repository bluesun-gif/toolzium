...
import { Wallet, Calendar, DollarSign, Download, Plus, Trash, Sparkles, Shield, Zap, Copy } from "lucide-react";
...
export function DailyBudgetClient() {
 ...
 <span className={cn("font-bold", spentTotal > totalBudget ? "text-red-500" : "text-green-500")}>
 {currency}{spentTotal.toFixed(2)}
 </span>
 </div>
 <div className="flex justify-between text-sm">
 <span>Remaining:</span>
 <span className={cn("font-bold", totalBudget - spentTotal < 0 ? "text-red-500" : "text-primary")}>
 {currency}{(totalBudget - spentTotal).toFixed(2)}
 </span>
 </div>
 
 <div className="h-3 w-full bg-muted rounded-full overflow-hidden mt-2">
 <div className={cn("h-full", spentTotal > totalBudget ? "bg-red-500" : "bg-primary")} style={{
                  width: Math.min(100, spentTotal / totalBudget * 100) + "%"
                }} />
 </div>
 </div>
 </CardContent>
 ...
 <Input type="number" value={alloc} onChange={e => updateAlloc(cat, Number(e.target.value))} className="w-24 h-7 text-right text-sm" />
 </div>
 </div>
 <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
 <div className={cn("h-full", spent > alloc ? "bg-red-500" : "bg-blue-500")} style={{
                      width: progress + "%"
                    }} />
 </div>
 </div>;
 ...
 <select value={newExpCat} onChange={e => setNewExpCat(e.target.value)} className={"flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"}>
 ...
 {expenses.length === 0 ? <p className="text-sm text-muted-foreground">No expenses logged yet.</p> : <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
 {expenses.map(exp => <div key={exp.id} className="flex justify-between items-center p-3 rounded-lg border bg-card text-card-foreground shadow-sm">
      ...
      <ToolFeatureGuides features={[{
        ...
        <div className="prose dark:prose-invert max-w-none">
          ...
          <p>
            ...
            Whether you're a developer, designer, student, or professional, our Daily Travel Budget Planner provides
