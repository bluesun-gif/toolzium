...
export function AiGrammarPolishClient() {
  ...
  const handlePolish = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      polished = polished.replace(/\bteh\b/gi, "the").replace(/\brecieve\b/gi, "receive").replace(/\bseperate\b/gi, "separate").replace(/\bi\b/g, "I").replace(/\s+/g, "");
 ...
 <textarea className="w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50 min-h-[140px]" placeholder="e.g. teh team have completed the project due to the fact that we worked hard..." value={rawText} onChange={e => setRawText(e.target.value)} />
 </div>

 <div>
 <Label className="text-xs mb-1 block">Target Writing Style</Label>
 <select className="w-full rounded-md border border-input bg-background px-3 py-2 text-xs" value={styleMode} onChange={e => setStyleMode(e.target.value as any)}>
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Paste Manuscript",
        description: "Input raw draft text, email body, or essay paragraphs.",
        icon: Wand2
      }, {
        step: "02",
        title: "Select Tone",
        description: "Choose Professional, Academic, Concise, or Casual styling rules.",
        icon: Sliders
      }, {
        step: "03",
        title: "Copy Polished Copy",
        description: "Copy grammar-checked text directly into your document or editor.",
        icon: CheckCircle2
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Refines sentence structure to match exact business or academic tone standards."
      }, {
        icon: BookOpen,
        title: "Readability Analysis",
        description: "Evaluates score metrics to ensure your writing remains easily readable."
      }, {
        icon: CheckCircle2,
        title: "Private Execution",
        description: "Processes text strictly in local client memory without third-party logging."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "No. All text parsing occurs strictly within your web browser."
      }, {
