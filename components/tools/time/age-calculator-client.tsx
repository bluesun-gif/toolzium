...
export default function AgeCalculatorClient() {
  ...
  const results = useMemo(() => {
    ...
    const milestones = [{
      label: "10,000th day",
      at: new Date(birth.getTime() + 10000 * msIn.day)
    }, {
      label: "20,000th day",
      at: new Date(birth.getTime() + 20000 * msIn.day)
    }, {
      label: "1 billion seconds",
      at: new Date(birth.getTime() + 1_000_000_000 * msIn.second)
    }];
    return {
      ymd,
      total,
  ...
  const shareLink = useMemo(() => {
    ...
    if (dobDate) url.searchParams.set("date", dobDate);else url.searchParams.delete("date");
    if (hasTime) url.searchParams.set("time", dobTime);else url.searchParams.delete("time");
    return url.toString();
  }, [dobDate, dobTime, hasTime]);
  const summary = birth && results ? `Age: ${results?.ymd.years}y ${results?.ymd.months}m ${results?.ymd.days}d
...
Next birthday: ${results?.until.exact} (${results?.until.days} days)` : "";
 ...
 {/* HEADER */}
 <ToolPageHeader icon={HeartPulse} title="Age Calculator" description="Find exact age in years, months, days — plus next birthday and fun milestones." actions={<>
 ...
 <InputField label="Time of Birth (optional)" className={cn(!hasTime && "opacity-50")} id="dob-time" type="time" value={dobTime} onChange={e => setDobTime(e.target.value)} disabled={!hasTime} />
 ...
 {!birth && <div className="text-sm text-muted-foreground flex items-center gap-2">
 ...
 {stats.map(s => <Stat key={s.label} label={s.label} value={s.value} hint={s.hint} Icon={s.Icon} />)}
 </div>
 ...
 {results.milestones.map(m => <div key={m.label} className="flex items-center justify-between rounded-md border p-3">
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: Milestone
      }]} badges={["Exact years, months & days", "Leap year accurate", "Next birthday countdown"]} />
 ...
 <tbody>
 {[["Western (international)", "Most of the world", "Age increases on birthday; 0 at birth"], ["East Asian traditional", "Korea, China (traditional)", "Born at age 1; increases on Lunar New Year"], ["Korean (international)", "South Korea (modern)", "Born at age 0; increases on January 1"], ["Japanese traditional", "Japan (historical)", "Born at 1; increases on New Year's Day"]].map(([sys, region, how]) => <tr key={sys} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["1 year old", "365 days", "366 in a leap year birth"], ["10 years old", "~3,652 days", "Depending on leap years"], ["18 years (legal adult)", "~6,574 days", "~157,776 hours"], ["21 years", "~7,670 days", "~184,080 hours"], ["1 million minutes old", "~694.4 days", "~1 year 11 months"], ["1 billion seconds old", "~31.7 years", "~11,574 days"], ["65 years (retirement)", "~23,741 days", "~570,000 hours"], ["100 years (centenarian)", "~36,524 days", "~876,600 hours"]].map(([milestone, days, context]) => <tr key={milestone} className="odd:bg-muted/20">
