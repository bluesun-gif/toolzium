...
export function WorkHoursClient() {
  const [logs, setLogs] = useState<DayLog[]>(daysOfWeek.map(d => ({
    day: d,
    start: "09:00",
    end: "17:00",
    breakMins: 60
  ...
  const updateLog = (day: string, field: keyof DayLog, value: any) => {
    setLogs(logs.map(log => log.day === day ? {
      ...log,
      [field]: value
    } : log));
  };
  const reset = () => {
    setLogs(daysOfWeek.map(d => ({
      day: d,
      start: "09:00",
      end: "17:00",
      breakMins: 60
 ...
 {logs.map(log => <div key={log.day} className="grid grid-cols-4 gap-2 items-center">
