...
export function WorldClockClient() {
 ...
 <ToolPageHeader
 title="World Clock"
 description="Monitor time across different cities and timezones simultaneously."
 icon={Globe}
 actions={
 <div className="flex items-center space-x-2 bg-secondary/30 p-2 rounded-md">
 ...
 <SelectContent>
 {ALL_CITIES.filter(c => !cities.some(sc => sc.id === c.id)).map(city => (
 ...
 const dateString = new Intl.DateTimeFormat('en-US', { timeZone: city.timezone, weekday: 'short', month: 'short', day: 'numeric' }).format(time);
 const hour = parseInt(new Intl.DateTimeFormat('en-US', { timeZone: city.timezone, hour: 'numeric', hour12: false }).format(time));
 const isDay = hour >= 6 && hour < 18;
 
 const offsetParts = new Intl.DateTimeFormat('en-US', { timeZone: city.timezone, timeZoneName: 'shortOffset' }).formatToParts(time);
 ...
 return (
 <GlassCard key={city.id} className="relative group">
 ...
 <div className={"p-2 rounded-full"+ (isDay ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-slate-100 text-slate-600 ')}>
