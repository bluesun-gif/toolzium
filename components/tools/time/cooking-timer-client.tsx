...
const PRESETS = [
 { label:"Boiled Egg", duration: 7 * 60 },
 { label:"Pasta", duration: 10 * 60 },
 { label:"Rice", duration: 20 * 60 },
 { label:"Chicken", duration: 45 * 60 },
 { label:"Roast", duration: 120 * 60 },
];
...
export function CookingTimerClient() {
 ...
 if (nextRemaining <= 0) {
 playBeep();
 return { ...t, remainingSeconds: 0, state:"done"};
 }
 ...
 if (t.id === id) {
 if (newState ==="stopped") {
 return { ...t, state: newState, remainingSeconds: t.durationSeconds };
 }
 ...
 icon={Timer}
 title="Cooking Timer"
 description="Run multiple kitchen timers simultaneously."
 actions={<ResetButton onClick={() => setTimers([])} label="Clear All"/>}
 />
 ...
 {PRESETS.map(p => (
 <Button key={p.label} variant="outline"size="sm"onClick={() => addTimer(p.label, p.duration)}>
 {p.label}
 </Button>
 ))}
 </div>
 ...
 {timers.length === 0 ? (
 <div className="col-span-full h-40 flex items-center justify-center text-muted-foreground border-2 border-dashed rounded-xl">
 No active timers. Add one to begin.
 </div>
 ) : timers.map(t => (
 <GlassCard key={t.id} className={cn("relative overflow-hidden", (t.state ==="done"?"animate-pulse border-red-500 bg-red-500/10":""))}>
 ...
 <div className="text-4xl font-mono text-center mb-6"style={{ color: t.state ==="done"?"#ef4444":"inherit"}}>
 {formatTime(t.remainingSeconds)}
 </div>
 
 <div className="flex justify-center gap-2">
 {t.state ==="running"? (
 <Button variant="outline"size="icon"onClick={() => updateTimerState(t.id,"paused")}>
 <Pause className="w-4 h-4"/>
 </Button>
 ) : (
 <Button variant="outline"size="icon"onClick={() => updateTimerState(t.id,"running")} disabled={t.state ==="done"}>
 ...
 {t.state ==="done"&& (
 <Button variant="default"size="icon"onClick={() => updateTimerState(t.id,"stopped")}>
