  ...
  return <div className="w-full min-h-screen pb-20 relative"><ToolBackground /><div className="relative z-10">
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 space-y-8 relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {SUPPORTED_PLATFORMS.map(p => <Badge key={p.name} variant="outline" className="text-xs gap-1.5 py-1 px-3 bg-card/70 border-border text-foreground font-semibold">
        ...
        <GlassCard className="p-6 md:p-8 space-y-6 bg-background border-border shadow-sm rounded-2xl">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input placeholder="https://www.youtube.com/watch?v=... or any social video URL" value={url} onChange={e => {
                setUrl(e.target.value);
                setResult(null);
                setError("");
              }} className="text-sm font-mono bg-background border-border text-foreground" />
              {detectedPlatform && <Badge variant="outline" className="shrink-0 px-3 bg-primary/10 text-primary border-primary/20 font-bold">
                  {detectedPlatform}
                </Badge>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            {/* Quality */}
            {mode !== "audio" && <div className="space-y-1.5">
            ...
            {isTikTok && mode !== "audio" && <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-3 h-10 px-3 rounded-xl border border-border bg-background">
                  <Switch id="tiktok-wm" checked={tiktokNoWatermark} onCheckedChange={setTiktokNoWatermark} />
                  <label htmlFor="tiktok-wm" className="text-xs cursor-pointer font-semibold">
                    {tiktokNoWatermark ? <span className="text-emerald-500">No Watermark</span> : <span className="text-muted-foreground">With Watermark</span>}
                  </label>
                </div>
              </div>}
          </div>
          ...
          {/* Download Button */}
          <Button onClick={handleDownload} disabled={loading || !url.trim()} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold shadow-lg shadow-primary/20 rounded-xl h-12 text-base">
            {loading ? <>
          ...
          {/* Error */}
          {error && <div className="flex items-start gap-3 p-4 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-xs">
          ...
          {/* Result */}
          {result && result.status !== "error" && <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-5 space-y-4 text-xs">
              ...
              {/* Video metadata card */}
              {"title" in result && result.title && <div className="flex gap-3 p-3 rounded-xl bg-card border border-border">
                  {"thumbnail" in result && result.thumbnail &&
              // eslint-disable-next-line @next/next/no-img-element
              <img src={result.thumbnail} alt={result.title} className="w-24 h-16 object-cover rounded-lg shrink-0" />}
                  <div className="min-w-0 space-y-1">
                    <p className="font-bold text-foreground line-clamp-2">{result.title}</p>
                    <div className="flex flex-wrap gap-2">
                      {"platform" in result && result.platform && <Badge variant="outline" className="text-[10px]">{result.platform}</Badge>}
                      {"quality" in result && result.quality && <Badge variant="secondary" className="text-[10px]">{result.quality}</Badge>}
                      {"duration" in result && result.duration && <Badge variant="secondary" className="text-[10px]">⏱ {result.duration}</Badge>}
                    </div>
                  </div>
                </div>}

              {/* Stream / Redirect / Tunnel download buttons */}
              {(result.status === "redirect" || result.status === "tunnel" || result.status === "stream") && <div className="space-y-3">
                  {"filename" in result && result.filename && <p className="text-xs text-muted-foreground font-mono truncate">
                      📁 {result.filename}
                    </p>}
                  <div className="flex gap-2">
                    <Button onClick={() => handleDirectDownload((result as {
                  url: string;
                }).url, (result as {
                  filename?: string;
                }).filename)} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-primary-foreground font-bold h-10 rounded-xl">
                      <Download className="w-4 h-4 mr-2" />
                      Download Now
                    </Button>
                    <Button variant="outline" onClick={() => window.open((result as {
                  url: string;
                }).url, "_blank")} title="Open in new tab" className="h-10 rounded-xl border-border">
                      <ExternalLink className="w-4 h-4" />
                    </Button>
                  </div>

                  {/* Alternative quality options for YouTube */}
                  {"availableQualities" in result && result.availableQualities && result.availableQualities.length > 1 && <div className="space-y-1 pt-1">
                      <p className="text-xs text-muted-foreground font-semibold">Other available qualities:</p>
                      <div className="flex flex-wrap gap-2">
                        {result.availableQualities.map(q => <Button key={q.itag} variant="outline" size="sm" className="text-xs h-8 border-border font-semibold" onClick={() => handleDirectDownload(q.url, result.filename?.replace(/\.[^.]+$/, `.mp4`) ?? `video_${q.label}.mp4`)}>
                            {q.label}
                          </Button>)}
                      </div>
                    </div>}
              ...
              {result.status === "picker" && <div className="space-y-3">
                  <p className="text-xs text-muted-foreground font-medium">
                    Multiple items found ({result.picker.length}). Click each to download:
                  </p>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {result.picker.map((item, i) => <Button key={i} variant="outline" size="sm" onClick={() => handleDirectDownload(item.url)} className="gap-2 border-border font-semibold">
                        {item.type === "video" ? <Video className="w-4 h-4" /> : <Download className="w-4 h-4" />}
                        Item {i + 1}
                      </Button>)}
                  </div>
                  {result.audio && <Button variant="outline" size="sm" onClick={() => handleDirectDownload(result.audio!)} className="gap-2 border-border font-semibold">
        ...
        <ToolHowItWorks steps={[{
          ...
          description: "Select your preferred video quality (up to 4K), download mode (video+audio, audio only, or mute), and for TikTok — toggle the watermark off.",
          icon: ChevronDown
        }, {
          step: "03",
          title: "Download Instantly",
          description: "Click 'Get Download Link' and your file will be ready in seconds. Click Download Now to save it directly to your device — no signup required.",
          icon: Download
        ...
        <ToolFeatureGuides features={[{
          ...
          description: "Download any public YouTube video in 240p to 4K resolution. Choose video+audio (MP4), audio only (MP3), or video-only. Supports Shorts, regular videos, and playlists."
        }, {
          icon: Video,
          title: "TikTok — No Watermark",
          description: "Download TikTok videos without the @username watermark in the corner. Toggle between watermark and no-watermark versions. Also supports audio-only downloads."
        }, {
          icon: Instagram,
          title: "Instagram — Reels, Posts & Carousels",
          description: "Download Instagram Reels, regular video posts, and carousel posts with multiple media items. Each item in a carousel is individually downloadable."
        }, {
          icon: Twitter,
          title: "Twitter / X — All Qualities",
          description: "Download videos from tweets in the best available quality. Works with regular tweets, quote tweets, and threads containing video media."
        }, {
          icon: Music,
          title: "Audio Extraction",
          description: "Extract audio from any video in MP3, OGG, WAV, or Opus format. Perfect for downloading music, podcasts, interviews, and speech from video content."
        }, {
          icon: Download,
          title: "Facebook, Reddit & More",
          description: "Works with Facebook videos, Reddit video posts, Pinterest videos, Vimeo, Twitch clips, Bilibili, SoundCloud, Dailymotion, and 10+ additional platforms."
        }]}>
          <div className="prose prose-sm dark:prose-invert max-w-none space-y-4 mt-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-xs">
                ...
                <tbody>
                  {[["YouTube", "✅", "✅ MP3/OGG/WAV", "N/A", "4K (2160p)"], ["TikTok", "✅", "✅", "✅ Yes", "1080p"], ["Instagram", "✅ Reels/Posts/Carousels", "✅", "N/A", "1080p"], ["Twitter / X", "✅", "✅", "N/A", "720p"], ["Facebook", "✅", "✅", "N/A", "1080p"], ["Reddit", "✅", "✅", "N/A", "1080p"], ["Pinterest", "✅", "✅", "N/A", "720p"], ["Vimeo", "✅", "✅", "N/A", "4K"], ["Twitch Clips", "✅", "✅", "N/A", "1080p"], ["SoundCloud", "N/A", "✅ MP3", "N/A", "320kbps"], ["Bilibili", "✅", "✅", "N/A", "1080p"], ["Dailymotion", "✅", "✅", "N/A", "1080p"]].map(([platform, video, audio, wm, quality]) => <tr key={platform} className="border-b border-border/50 text-muted-foreground">
        ...
        <ToolFaqAccordion faqs={[{
          ...
          answer: "Yes, completely free with no signup, no limits, and no ads injected into downloads. You can download as many videos as you need."
        }, {
          question: "How do I download a TikTok video without a watermark?",
          answer: "Paste the TikTok video URL, make sure the 'No Watermark' toggle is enabled (it's on by default for TikTok), and click Download. The tool fetches the original video file without the @username watermark overlay."
        }, {
          question: "Why is my YouTube video not downloading?",
          answer: "YouTube may restrict downloads for age-restricted videos, live streams, or region-blocked content. Try a different quality setting (720p instead of 1080p) or try downloading audio-only. Private and members-only videos cannot be downloaded."
        }, {
          question: "Can I download Instagram Reels?",
          answer: "Yes. Paste the Instagram Reel URL (e.g., instagram.com/reel/...) and click Download. For carousels (multiple photos/videos in one post), the tool shows each item separately so you can download them individually."
        }, {
          question: "Is it legal to download videos?",
          answer: "Downloading videos for personal, offline viewing may be acceptable in some jurisdictions. However, downloading copyrighted content without permission for commercial use, redistribution, or public display is generally illegal. Always respect copyright, creative rights, and platform terms of service. Only download content you have rights to access."
        }, {
