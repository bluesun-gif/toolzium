...
Return ONLY the 5 captions separated by ||| with no labels.`;
      ...
      setCaptions([`Wait for the end 👀 ${topic} hits different #fyp #${topic.replace(/\s+/g, "").toLowerCase()}`, `POV: you finally understand ${topic} 🤯 #learnontiktok #fyp`, `This is your sign to try ${topic} ✨ #trending #viral`, `Nobody talks about ${topic} enough 👇 #foryou #tips`, `How ${topic} changed the game 🚀 #fypage #viralvideo`]);
 ...
 <Input value={topic} onChange={e => setTopic(e.target.value)} placeholder="e.g. morning routine hacks" />
 </div>

 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">Vibe</label>
 <select value={vibe} onChange={e => setVibe(e.target.value as Vibe)} className={fieldClass}>
 {vibes.map(item => <option key={item} value={item}>
 {item}
 </option>)}
 </select>
 </div>
 ...
 <Input value={audience} onChange={e => setAudience(e.target.value)} placeholder="e.g. students, creators" />
 </div>
 </div>
 ...
 {captions.length > 0 && <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
 {captions.map((caption, index) => <Card key={`${caption.slice(0, 12)}-${index}`} className={cardClass}>
 <CardHeader className={headerClass}>
 <CardTitle className={titleClass}>
 ...
 <span className={`text-xs ${caption.length > 2200 ? "text-red-500" : "text-muted-foreground"}`}>
 {caption.length}/2200
 </span>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "Use a balanced set of relevant hashtags. Relevance matters more than stuffing too many tags."
      }, {
