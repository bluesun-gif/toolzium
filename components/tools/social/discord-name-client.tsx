...
const SERVER_TYPES = [{
  value: "gaming",
  label: "🎮 Gaming & Esports Syndicate"
}, {
  value: "anime",
  label: "⛩️ Anime & Chill Lounge"
}, {
  value: "coding",
  label: "💻 Developer, Coding & AI Hub"
}, {
  value: "study",
  label: "📚 Study, Productivity & Lofi"
}, {
  value: "crypto",
  label: "🚀 Crypto, Web3 & Trading"
}, {
  value: "roleplay",
  label: "⚔️ Fantasy Roleplay & Worldbuilding"
}];
export default function DiscordNameClient() {
  ...
  const generateDiscordNames = async () => {
    setLoading(true);
    try {
      const prompt = `Generate 10 aesthetic, creative Discord server name ideas for a '${serverType}' community. ${keyword ? `Incorporate the keyword '${keyword}'.` : ""} Include modern clean emojis and aesthetic formatting (e.g. ✦ Nexus Gaming ⚡). Output 1 server name per line. No markdown formatting.`;
      ...
      setServerNames(["✦ Nexus Gaming Hub ⚡", "❖ Viper Strike Syndicate ⚔️", "✧ Pixel Haven Lounge ✨", "◈ Apex Realm Esports 🏆", "✦ Cyber Pulse Network 🌃", "❖ Midnight Code Collective 💻", "✧ Valhalla Gamers 🛡️", "◈ Echo Chamber Lounge 🎧"]);
  ...
  return (
    <div className="relative space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select value={serverType} onValueChange={v => setServerType(v)}>
                ...
                <SelectContent>
                  {SERVER_TYPES.map(cat => <SelectItem key={cat.value} value={cat.value}>
                      {cat.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Input id="keyword" value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="e.g. Apex, Realm, Cyber, Coffee" />
            </div>
          </div>

          <div className="flex justify-end pt-2">
            <Button onClick={generateDiscordNames} disabled={loading} className="gap-2 font-bold h-11 px-6 shadow-md">
      ...
      {/* RESULTS GRID */}
      {serverNames.length > 0 && <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {serverNames.map((name, idx) => <GlassCard key={idx} className="p-4 flex items-center justify-between hover:border-primary/40 transition-all">
