...
const VIBE_CATEGORIES = [{
  value: "lofi",
  label: "☕ Lofi Beats, Chill & Midnight Study",
  emoji: "☕"
}, {
  value: "workout",
  label: "⚡ Gym, Beast Mode & Heavy Lifting",
  emoji: "⚡"
}, {
  value: "heartbreak",
  label: "💔 Sad Hours & Midnight Crying",
  emoji: "💔"
}, {
  value: "indie",
  label: "🌸 Aesthetic Indie & Bedroom Pop",
  emoji: "🌸"
}, {
  value: "drive",
  label: "🌃 Night Drive & Synthwave Cyber",
  emoji: "🌃"
}, {
  value: "gaming",
  label: "🎮 Gaming Flow & EDM Hype",
  emoji: "🎮"
}];
export default function SpotifyPlaylistClient() {
  ...
  const generateSpotifyPlaylists = async () => {
    setLoading(true);
    try {
      const prompt = `Generate 6 aesthetic Spotify playlist titles with short mood descriptions for a '${vibe}' music vibe. ${customArtist ? `Incorporate inspiration from artists like ${customArtist}.` : ""} Format output strictly as JSON array of objects with keys: title, description, tags (array of 3 hashtags). Do not use markdown backticks.`;
  ...
  return <div className="relative max-w-6xl mx-auto space-y-8"><ToolBackground /><div className="relative z-10">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select value={vibe} onValueChange={v => setVibe(v)}>
                ...
                <SelectContent>
                  {VIBE_CATEGORIES.map(cat => <SelectItem key={cat.value} value={cat.value}>
                      {cat.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Input id="artist-input" type="text" value={customArtist} onChange={e => setCustomArtist(e.target.value)} placeholder="e.g. Frank Ocean, Taylor Swift, Joji, Lofi Girl" />
            </div>
          </div>

          <div className="flex justify-end pt-2">
            <Button onClick={generateSpotifyPlaylists} disabled={loading} className="gap-2 font-bold h-11 px-6 shadow-md">
      ...
      <div className="space-y-4">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {playlists.map((playlist, idx) => <GlassCard key={idx} className="p-5 flex flex-col justify-between hover:border-primary/40 transition-all duration-200">
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <CopyButton getText={() => `${playlist.title}\n\n${playlist.description}\n\n${playlist.tags.join(" ")}`} label="Copy" />
                </div>
              ...
              <div className="flex flex-wrap items-center gap-1.5 pt-4 mt-2 border-t border-border/60">
                {playlist.tags.map((tag, tIdx) => <span key={tIdx} className="text-[10px] font-mono font-medium px-2 py-0.5 rounded-md bg-secondary/60 text-secondary-foreground">
                    {tag}
                  </span>)}
              </div>
            </GlassCard>)}
