...
const platforms: Platform[] = [{
  ...
  id: "pinterest",
  name: "Pinterest",
  urlFormat: "https://pinterest.com/{username}",
  ...
  id: "behance",
  name: "Behance",
  urlFormat: "https://behance.net/{username}",
  icon: <BookOpen className="w-5 h-5" />
}, {
  id: "dribbble",
  name: "Dribbble",
  urlFormat: "https://dribbble.com/{username}",
  icon: <BookOpen className="w-5 h-5" />
}, {
  id: "soundcloud",
  name: "SoundCloud",
  urlFormat: "https://soundcloud.com/{username}",
  ...
  id: "facebook",
  name: "Facebook",
  urlFormat: "https://facebook.com/{username}",
...
export default function UsernameCheckClient() {
 ...
 <Input id="username" placeholder="e.g. johndoe, cool_dev99" value={username} onChange={e => setUsername(e.target.value)} className={!isValidUsername && username ? "border-destructive" : ""} />
 {!isValidUsername && username && <p className="text-sm text-destructive mt-1">Username can only contain alphanumeric characters, dots, dashes, and underscores.</p>}
 </div>
 ...
 <Input id="search" placeholder="Search platforms..." className="pl-9" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
 </div>
 </div>
 </div>
 
 <div className="flex justify-end">
 <Button variant="outline" onClick={copyAll} disabled={!username || !isValidUsername || filteredPlatforms.length === 0}>
 {copiedId === "all" ? <><CheckCircle2 className="w-4 h-4 mr-2" /> Copied All Links</> : <><Copy className="w-4 h-4 mr-2" /> Copy All Links</>}
 </Button>
 </div>
 </CardContent>
 </Card>

 {username && isValidUsername && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
 ...
 <Button variant="ghost" size="icon" className="h-8 w-8" title="Copy link" onClick={() => copyToClipboard(url, platform.id)}>
 ...
 {filteredPlatforms.length === 0 && <div className="col-span-full py-8 text-center text-muted-foreground">
 No platforms found matching"{searchQuery}"
 </div>}
 </div>}
 
 {(!username || !isValidUsername) && <Alert>
 <AlertDescription>
