...
import { Wifi, Clock, HardDrive, Download, Trash2, Plus } from"lucide-react";
import toast from"react-hot-toast";

type Unit ="KB"|"MB"|"GB"|"TB";
type FileItem = { id: string; size: number; unit: Unit; name: string };
...
export function BandwidthClient() {
 ...
 setFiles([...files, { id: Date.now().toString(), name: newName || `File ${files.length + 1}`, size: Number(newSize), unit: newUnit }]);
 setNewName("");
 setNewSize("");
 };
 ...
 setFiles([{ id: Date.now().toString(), name:"Preset Task", size, unit }]);
 };
 ...
 description="Calculate transfer times and bandwidth requirements"
 actions={<ResetButton onClick={() => { setFiles([]); setBandwidthMbps("100"); }} label="Reset"/>}
 />
