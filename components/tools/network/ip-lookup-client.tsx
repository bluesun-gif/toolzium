...
import { Copy, History, Search, AlertCircle, MapPin, Globe, Clock, CreditCard, Network, Map, Hash, Phone } from"lucide-react";
...
Calling Code: ${result.country_calling_code || 'N/A'}
 ...
 description="Find the geographical location, ISP, and other details for any IP address."
 />
 ...
 label="Location"
 value={`${result.city || 'N/A'}, ${result.region || 'N/A'}`} 
 />
 <ResultItem 
 icon={<Globe className="h-4 w-4"/>} 
 label="Country"
 value={result.country_name ? `${result.country_name} ${getFlagEmoji(result.country)}` : 'N/A'} 
 />
 <ResultItem 
 icon={<Map className="h-4 w-4"/>} 
 label="Coordinates"
 value={result.latitude ? `${result.latitude}, ${result.longitude}` : 'N/A'} 
 />
 <ResultItem 
 ...
 icon={<MapPin className="h-4 w-4"/>} 
 label="Postal Code"
 value={result.postal || 'N/A'} 
 />
 <ResultItem 
 icon={<Phone className="h-4 w-4"/>} 
 label="Calling Code"
 value={result.country_calling_code || 'N/A'} 
 />
 </div>
 </CardContent>
 </Card>
 ...
 <CardContent>
 {history.length === 0 ? (
 <p className="text-sm text-muted-foreground">No recent lookups.</p>
 ) : (
