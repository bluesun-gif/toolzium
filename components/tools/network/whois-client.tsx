...
DNSSEC: ${result.dnssec ? "Enabled" : "Disabled"}`;
 ...
 <Input placeholder="e.g., example.com" value={input} onChange={e => setInput(e.target.value)} className="flex-1" />
 <Button type="submit" disabled={isLoading || !input}>
 {isLoading ? <Clock className="w-4 h-4 mr-2 animate-spin" /> : <Search className="w-4 h-4 mr-2" />}
 Lookup
 </Button>
 </form>
 ...
 {result.dnssec ? <Badge variant="default" className="bg-green-600">Enabled</Badge> : <Badge variant="secondary">Disabled</Badge>}
 </p>
 </div>
 </div>
 ...
 {result.nameservers.length > 0 ? result.nameservers.map((ns, i) => <Badge key={i} variant="outline" className="text-sm py-1">
 {ns}
 </Badge>) : <p className="text-sm text-muted-foreground">No nameservers found.</p>}
 </div>
 </div>
 ...
 <div className="flex flex-wrap gap-2">
 {result.statuses.length > 0 ? result.statuses.map((status, i) => <Badge key={i} variant="secondary" className="text-xs font-normal">
 {formatStatus(status)}
 </Badge>) : <p className="text-sm text-muted-foreground">No status codes found.</p>}
 </div>
 </div>
 ...
 {showRaw && <div className="mt-4 bg-muted rounded-md p-4 overflow-x-auto">
 ...
 <CardContent>
 {history.length === 0 ? <p className="text-sm text-muted-foreground text-center py-6">
 No recent searches.
 </p> : <ScrollArea className="h-[300px] pr-4">
 <div className="space-y-2">
 {history.map((item, index) => <Button key={index} variant="ghost" className="w-full justify-start font-normal text-sm" onClick={() => handleHistoryClick(item)}>
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Type any root domain (like google.com or github.com) into the lookup bar. Subdomains and protocol parts are automatically stripped."
      }, {
        step: "2",
        title: "Query RDAP WHOIS API",
        description: "Click the Search icon. The tool contacts standard RDAP directories to fetch registration JSON details directly."
      }, {
        step: "3",
        title: "Review Registration Info",
        description: "Read registrar name, creation date, expiry status, and name servers. View the complete, raw RDAP response for advanced troubleshooting."
      }]} badges={["ICANN Standard", "RDAP Query", "Instant Results", "Free & Private"]} />

 {/* ─── Feature Guides + SEO Content ─── */}
 <ToolFeatureGuides features={[{
        icon: Globe,
        title: "Real-Time Queries",
        description: "Bypasses slow third-party scrapers to query live domain registry nodes directly using the latest RDAP protocol standards."
      }, {
        icon: Clock,
        title: "Domain Expiry Checker",
        description: "Check precisely when a domain registration expires, helping you track domain lifecycles or catch dropping names."
      }, {
        icon: ShieldCheck,
        title: "GDPR Redaction Info",
        description: "Highlights domain privacy protections, identifying proxy servers or redacted contact records transparently."
      }, {
        icon: Server,
        title: "Name Servers & DNS",
        description: "Retrieves active authoritative name servers and DNSSEC validation flags associated with the domain registry."
      }, {
        icon: History,
        title: "Cached History Log",
        description: "Keeps a local history list of your 10 most recent domain searches, allowing for one-click re-evaluation."
      }, {
        icon: FileText,
        title: "Raw JSON Response",
        description: "Access the entire unstructured JSON payload returned by RDAP nodes, essential for developers and systems engineers."
      }]}>
