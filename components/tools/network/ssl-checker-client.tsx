...
import { Loader2, Search, Copy, Clock, ShieldCheck, AlertTriangle, ShieldX, History, Trash2, Calendar } from "lucide-react";
...
export default function SslCheckerClient() {
  ...
  const saveToHistory = (newDomain: string) => {
    const updated = [
      { domain: newDomain, date: new Date().toISOString() },
      ...history.filter((h) => h.domain !== newDomain)
    ].slice(0, 10);
    setHistory(updated);
  ...
  const getStatusDetails = (notAfter: string) => {
    ...
    if (days < 0) {
      return { label: "Expired", color: "bg-red-500", icon: <ShieldX className="w-5 h-5 text-red-500" /> };
    } else if (days < 30) {
      return { label: "Expiring Soon", color: "bg-red-500", icon: <AlertTriangle className="w-5 h-5 text-red-500" /> };
    } else if (days <= 60) {
      return { label: "Valid", color: "bg-yellow-500", icon: <AlertTriangle className="w-5 h-5 text-yellow-500" /> };
    }
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent>
              ...
              {error && (
                <div className="mt-4 p-4 bg-red-50 text-red-600 rounded-md border border-red-200 flex items-start gap-2">
                  ...
                  {history.map((item, i) => (
                    <li key={i}>
                      <button 
                        ...
                        className="w-full text-left px-3 py-2 rounded-md hover:bg-muted transition-colors flex flex-col gap-1"
                      >
