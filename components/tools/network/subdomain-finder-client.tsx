...
export default function SubdomainFinderClient() {
  ...
  const fetchSubdomains = async (targetDomain: string) => {
    const clean = targetDomain.trim().toLowerCase().replace(/^(https?:\/\/)?(www\.)?/, "").replace(/\/.*$/, "");
    if (!clean) {
 ...
 <Input id="domain" placeholder="e.g. github.com" value={domain} onChange={e => setDomain(e.target.value)} disabled={isLoading} className="pr-10" />
 ...
 <Input placeholder="Filter results..." value={filterText} onChange={e => setFilterText(e.target.value)} className="h-9" />
 </div>
 
 <div className="flex items-center space-x-2">
 <input type="checkbox" id="hideWildcards" checked={hideWildcards} onChange={e => setHideWildcards(e.target.checked)} className="rounded border-gray-300 text-primary focus:ring-primary h-4 w-4" />
 ...
 <TableBody>
 {filteredResults.length > 0 ? filteredResults.map((r, index) => <TableRow key={index}>
 <TableCell className="font-mono text-sm break-all">
 {r.name}
 </TableCell>
 <TableCell className="text-right">
 {r.isWildcard ? <Badge variant="outline" className="text-amber-500 border-amber-200 bg-amber-50/20">
 Wildcard
 </Badge> : <Badge variant="outline" className="text-green-500 border-green-200 bg-green-50/20">
 Host
 </Badge>}
 </TableCell>
 </TableRow>) : <TableRow>
 ...
 </Card> : <Card className="h-[300px] flex items-center justify-center border-dashed">
