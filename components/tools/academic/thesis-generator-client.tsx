...
export function ThesisGeneratorClient() {
  ...
  const generateThesis = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (essayStyle === "argumentative") {
        statements = [`Although ${c}, ${t} should be ${s} because ${a}.`, `By addressing ${c}, it becomes clear that ${t} must ${s} due to ${a}.`, `While critics argue that ${c}, the evidence demonstrates that ${t} is essential for ${s} as shown by ${a}.`];
      } else if (essayStyle === "analytical") {
        statements = [`An analysis of ${t} reveals that ${s}, which is primarily driven by ${a}.`, `Examining ${t} through the lens of ${a} demonstrates how ${s}.`, `Although ${c} suggests a simple trend, a deeper examination of ${t} highlights how ${a} creates ${s}.`];
      } else if (essayStyle === "expository") {
        statements = [`The key factors influencing ${t} include ${a}, demonstrating why ${s}.`, `${t} represents a critical shift because ${a}, ultimately proving that ${s}.`, `In evaluating ${t}, the combination of ${a} illustrates why ${s}.`];
      } else {
        statements = [`To resolve ${t}, policymakers and scholars must embrace ${s} because ${a}.`, `Without addressing ${a}, efforts surrounding ${t} will fail to achieve ${s}.`, `Despite claims regarding ${c}, adopting ${s} for ${t} remains urgent due to ${a}.`];
      }
 ...
 <select className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={essayStyle} onChange={e => setEssayStyle(e.target.value as any)}>
 ...
 <Input placeholder="e.g. Remote work adoption in tech companies" value={topic} onChange={e => setTopic(e.target.value)} />
 </div>

 <div>
 <Label className="text-xs mb-1 block">Your Stance / Claim</Label>
 <Input placeholder="e.g. should be permanently adopted as a standard policy" value={stance} onChange={e => setStance(e.target.value)} />
 </div>

 <div>
 <Label className="text-xs mb-1 block">Main Evidence / Reason</Label>
 <Input placeholder="e.g. it boosts developer productivity, reduces burnout, and lowers operating costs" value={mainArgument} onChange={e => setMainArgument(e.target.value)} />
 </div>

 <div>
 <Label className="text-xs mb-1 block">Counterargument / Concession (Optional)</Label>
 <Input placeholder="e.g. concerns about team collaboration and onboarding difficulty" value={counterArgument} onChange={e => setCounterArgument(e.target.value)} />
 </div>
 ...
 <CardContent className="p-4 sm:p-6 space-y-4">
 {generatedStatements.length > 0 ? generatedStatements.map((stmt, idx) => <div key={idx} className="p-4 rounded-xl bg-muted/30 border border-border/50 space-y-2">
 ...
 </div>) : <div className="h-[300px] flex flex-col items-center justify-center text-center p-6 text-muted-foreground border border-dashed border-border/60 rounded-xl bg-muted/10">
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Define Topic & Stance",
        description: "Enter your research topic, your specific position or claim, and your main supporting evidence.",
        icon: Compass
      }, {
        step: "02",
        title: "Select Essay Genre",
        description: "Choose whether your paper is Argumentative, Analytical, Expository, or Persuasive.",
        icon: Sliders
      }, {
        step: "03",
        title: "Review Variations",
        description: "Compare 3 distinct thesis statement structures and copy your favorite draft into your paper.",
        icon: CheckCircle2
      }]} badges={["100% Free", "Academic Structure", "Instant Results"]} />

 <ToolFeatureGuides features={[{
        icon: GraduationCap,
        title: "Academic Formula",
        description: "Utilizes proven concession + claim + reason formulas used by top university writing centers."
      }, {
        icon: Compass,
        title: "Concession Support",
        description: "Integrates counterarguments smoothly to create nuanced, high-scoring argumentative thesis statements."
      }, {
        icon: Sparkles,
        title: "Instant Variations",
        description: "Generates multiple structural variations so you can select the formula that fits your essay flow best."
      }, {
        icon: CheckCircle2,
        title: "Zero Plagiarism Risk",
        description: "Constructed dynamically from your own specific inputs and ideas without relying on static templates."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Where should the thesis statement be placed in an essay?",
        answer: "In standard academic papers, the thesis statement is typically placed at the end of the introductory paragraph."
      }, {
        question: "Is a thesis statement only one sentence?",
        answer: "Yes, in most undergraduate and high school essays, a thesis is a single, clear sentence. In longer graduate dissertations, it may span two sentences."
      }, {
        question: "Can I revise the generated thesis?",
        answer: "Absolutely! The generated options serve as structured building blocks that you can polish to fit your exact writing style and paper outline."
      }, {
