...
export default function Magic8BallClient() {
 ...
 <Input placeholder="Type your yes/no question here..." value={question} onChange={e => setQuestion(e.target.value)} className="max-w-md text-center" />

 <div className={`relative w-64 h-64 rounded-full bg-gradient-to-br from-slate-800 to-black shadow-2xl flex items-center justify-center border-8 border-slate-900 transition-transform duration-300 ${shaking ? "animate-bounce rotate-12" : ""}`}>
 ...
 {shaking ? <span className="text-muted-foreground italic animate-pulse">Shaking the ethereal plane...</span> : <span className="text-2xl font-bold text-primary">{answer || "Ask to receive an answer"}</span>}
 </div>
 ...
 {history.map((h, i) => <div key={i} className="p-2 bg-muted/20 rounded-lg text-sm">
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Think of a clear yes-or-no question and type it into the input field.",
        icon: Circle
      }, {
        step: "02",
        title: "Shake the Ball",
        description: "Click the ask button and watch the 8-ball shake as it consults the universe.",
        icon: Circle
      }, {
        step: "03",
        title: "Read Prophecy",
        description: "The mystical triangle reveals your answer, which is also saved to your history.",
        icon: Circle
      }]} badges={["100% Free", "Client-Side", "Fun"]} />

 <ToolFeatureGuides features={[{
        icon: Circle,
        title: "Classic Responses",
        description: "Features all 20 of the original, iconic Magic 8 Ball answers from positive to negative."
      }, {
        icon: Circle,
        title: "Shake Animation",
        description: "Includes a satisfying visual bounce and rotation effect while the answer is being determined."
      }, {
        icon: Circle,
        title: "Question History",
        description: "Keeps a log of your last 10 questions and answers for easy reference."
      }, {
        icon: Circle,
        title: "Zero Latency",
        description: "The randomness is generated instantly in your browser for immediate gratification."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "Just like the physical toy, this digital version contains exactly 20 possible responses: 10 positive, 5 non-committal, and 5 negative."
      }, {
        question: "Is the answer truly random?",
        answer: "Yes, it uses a high-quality pseudo-random number generator to ensure every shake has an equal probability of yielding any of the 20 answers."
      }, {
