...
export default function DiceRollerClient() {
 ...
 <select value={sides} onChange={e => setSides(parseInt(e.target.value))} className="w-full rounded-lg border border-border/70 bg-background/80 p-2 text-sm outline-none focus:ring-2 focus:ring-primary/50">
 ...
 {results.map((r, i) => <div key={i} className="w-20 h-20 rounded-xl bg-primary/10 border-2 border-primary/30 flex items-center justify-center text-3xl font-bold text-primary shadow-lg">
 {r}
 </div>)}
 </div>
 ...
 {history.length > 0 && <div className="mt-8 border-t border-border/50 pt-4">
 <h3 className="text-sm font-semibold mb-2">Recent Rolls</h3>
 <div className="space-y-2 max-h-40 overflow-y-auto">
 {history.map((h, i) => <div key={i} className="flex justify-between text-sm p-2 bg-muted/20 rounded-lg">
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Select Dice",
        description: "Choose how many dice to roll and the number of sides (d4, d6, d20, etc.).",
        icon: Dice1
      }, {
        step: "02",
        title: "Add Modifier",
        description: "Apply a positive or negative modifier for RPG buffs or penalties.",
        icon: Dice1
      }, {
        step: "03",
        title: "Roll & Tally",
        description: "Hit roll to see individual results, the grand total, and save it to your history.",
        icon: Dice1
      }]} badges={["100% Free", "Client-Side", "Fun"]} />

 <ToolFeatureGuides features={[{
        icon: Dice1,
        title: "Polyhedral Support",
        description: "Supports all standard RPG dice including d4, d6, d8, d10, d12, d20, and d100."
      }, {
        icon: Dice1,
        title: "Custom Modifiers",
        description: "Easily add your character's strength or magic bonuses to the final roll total."
      }, {
        icon: Dice1,
        title: "Roll History",
        description: "Keeps a log of your last 10 rolls so you can verify past turns during a game."
      }, {
        icon: Dice1,
        title: "Multi-Dice",
        description: "Roll up to 6 dice simultaneously, ideal for fireball spells or multiple attacks."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "What does '2d6+3' mean?",
        answer: "It means rolling two 6-sided dice, summing their face values, and then adding 3 to the final result."
      }, {
        question: "Can I roll a d100?",
        answer: "Yes, simply select 1 die and choose d100 from the sides dropdown, which is commonly used for percentile checks."
      }, {
        question: "Is this truly random?",
        answer: "Yes, it uses the browser's cryptographic random number generators to ensure fair and unpredictable rolls."
      }]} />
