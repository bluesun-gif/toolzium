...
export function MemoryMatchClient() {
 ...
 <select value={difficulty} onChange={e => setDifficulty(e.target.value as any)} className="px-3 py-2 rounded-lg bg-background border border-border text-sm" disabled={phase !== "idle" && phase !== "gameover"}>
 ...
 {Array(9).fill(0).map((_, idx) => <Button key={idx} className={cn(`aspect-square rounded-2xl transition-all duration-200 border-b-4 border-black/20 active:scale-95 ${getTileColor(idx)}`)} onClick={() => handleTileClick(idx)} disabled={phase !== "input"} />)}
 </div>
 </CardContent>
 </GlassCard>
