...
import { Dices, BarChart3, Calculator, TrendingUp, Type } from "lucide-react";
...
export default function DiceProbabilityClient() {
 ...
 <input type="range" min="1" max="6" value={numDice} onChange={e => setNumDice(parseInt(e.target.value))} className="w-full accent-primary" />
 <div className="text-center font-bold text-lg">{numDice}</div>
 </div>
 <div className="space-y-2">
 <label className="text-sm font-medium">Dice Type</label>
 <div className="grid grid-cols-3 gap-2">
 {DICE_TYPES.map((d, idx) => <Button key={d.label} onClick={() => setDiceIdx(idx)} className={cn(`p-2 rounded-lg border text-sm font-semibold transition-all ${diceIdx === idx ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border hover:border-primary"}`)}>
 {d.label}
 </Button>)}
 </div>
 </div>
