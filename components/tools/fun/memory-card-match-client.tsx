...
export default function MemoryCardMatchClient() {
  ...
  const initGame = useCallback(() => {
    ...
    const deck = [...selected, ...selected].sort(() => Math.random() - 0.5).map((emoji, idx) => ({
      id: idx,
      emoji,
      isFlipped: false,
      isMatched: false
 ...
 {Object.keys(DIFFICULTIES).map(d => <Button key={d} variant={difficulty === d ? "default" : "outline"} size="sm" onClick={() => setDifficulty(d as keyof typeof DIFFICULTIES)}>
 {d}
 </Button>)}
 </div>
 </div>
 ...
 {cards.map(card => <Button key={card.id} onClick={() => handleClick(card.id)} className={cn(`aspect-square rounded-xl text-3xl sm:text-4xl flex items-center justify-center transition-all duration-300 transform ${card.isFlipped || card.isMatched ? "bg-background border-2 border-primary scale-95" : "bg-primary/10 hover:bg-primary/20 border border-border shadow-md"}`)} disabled={card.isMatched || flipped.length === 2}>
 {card.isFlipped || card.isMatched ? card.emoji : "?"}
 </Button>)}
 </div>
 {isWon && <div className="mt-6 text-center p-6 bg-primary/10 rounded-xl border border-primary/20 space-y-3">
