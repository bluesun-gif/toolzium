...
export default function WordScrambleClient() {
  ...
  const skipWord = () => {
    setTotal(t => t + 1);
    toast(`Skipped! The word was ${currentWord.w}.`, {
      icon: "⏭️"
    });
    nextWord();
  };
 ...
 {scrambled.split("").map((char, idx) => <div key={idx} className="w-10 h-12 sm:w-14 sm:h-16 bg-primary/10 border-2 border-primary/30 rounded-lg flex items-center justify-center text-2xl sm:text-3xl font-bold text-primary">
 {char}
 </div>)}
 </div>

 {showHint && <div className="text-lg text-muted-foreground animate-in fade-in slide-in-from-top-2">
 Hint: Starts with <span className="font-bold text-foreground">{currentWord.w[0]}</span>
 </div>}

 <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
 <Input value={guess} onChange={e => setGuess(e.target.value)} placeholder="Type your guess..." className="text-center text-lg uppercase tracking-widest" autoFocus />
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Analyze the Letters",
        description: "Look at the scrambled letters and the category hint provided.",
        icon: Shuffle
      }, {
        step: "02",
        title: "Type Your Guess",
        description: "Unscramble the word in your head and type it into the input box.",
        icon: CheckCircle2
      }, {
        step: "03",
        title: "Use Hints if Stuck",
        description: "Reveal the first letter or skip the word to keep your streak alive.",
        icon: Lightbulb
      }]} badges={["100% Free", "Client-Side", "Fun"]} />

 <ToolFeatureGuides features={[{
        icon: Shuffle,
        title: "50+ Unique Words",
        description: "A massive library of words spanning Animals, Food, Countries, Sports, and Colors."
      }, {
        icon: Lightbulb,
        title: "Smart Hints",
        description: "Stuck? Click the hint button to reveal the very first letter of the word."
      }, {
        icon: Timer,
        title: "Speed Timer",
        description: "Track how long it takes your brain to decode each scrambled puzzle."
      }, {
        icon: SkipForward,
        title: "Skip Option",
        description: "Don't waste time on impossible words; skip them and move to the next challenge."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Are the scrambled words randomized?",
        answer: "Yes, the scrambling algorithm uses a Fisher-Yates shuffle, ensuring that every time you play, the letters are in a completely different order."
      }, {
        question: "Does capitalization matter?",
        answer: "No, the game automatically converts your guess to uppercase before comparing it to the target word."
      }, {
