...
export function SudokuUnlimitedClient() {
  ...
  const [stats, setStats] = useState<Record<Mode, number>>({
    '4x4': 0,
    '9x9': 0,
    '16x16': 0
  });
  const [bestTimes, setBestTimes] = useState<Record<Mode, number>>({
    '4x4': Infinity,
    '9x9': Infinity,
    '16x16': Infinity
  });
  ...
  const generatePuzzle = useCallback((m: Mode, diff: Difficulty) => {
    ...
    let cellsToRemove = 0;
    if (N === 2) cellsToRemove = diff === 'easy' ? 4 : diff === 'medium' ? 6 : 8;else if (N === 3) cellsToRemove = diff === 'easy' ? 35 : diff === 'medium' ? 45 : 55;else cellsToRemove = diff === 'easy' ? 120 : diff === 'medium' ? 160 : 200;
 ...
 `)}>
 {cell ? getSymbol(cell, N) : <div className={`grid gap-0.5 p-0.5 w-full h-full ${mode === '16x16' ? 'grid-cols-4' : 'grid-cols-3'}`}>
 {notes[r]?.[c]?.map((hasNote, i) => <span key={i} className="text-[7px] text-muted-foreground flex items-center justify-center">
 ...
 {symbols.map(num => <Button key={num} variant="outline" className="h-10 text-sm font-bold" onClick={() => handleNumberInput(num)}>
 {getSymbol(num, N)}
 </Button>)}
 </div>
