...
export default function LoveCalculatorClient() {
 ...
 const [history, setHistory] = useState<{ n1: string; n2: string; s: number }[]>([]);

 const calculate = () => {
 if (!name1.trim() || !name2.trim()) return;
 const s = getLoveScore(name1, name2);
 setScore(s);
 setHistory(prev => [{ n1: name1, n2: name2, s }, ...prev].slice(0, 5));
 };
 ...
 {score !== null && (
 <div className="w-full max-w-md mt-6 p-6 rounded-xl bg-pink-500/10 border border-pink-500/20 text-center space-y-4 animate-in fade-in zoom-in duration-500">
 ...
 {history.map((h, i) => (
 <div key={i} className="flex justify-between items-center p-2 bg-muted/20 rounded-lg text-sm">
 <span>{h.n1} & {h.n2}</span>
 ...
 <ToolHowItWorks 
 steps={[
 { step:"01", title:"Enter Names", description:"Type your name and the name of your partner or crush into the fields.", icon: Heart },
 { step:"02", title:"Calculate", description:"Hit the button to run our highly advanced* compatibility algorithm.", icon: Heart },
 { step:"03", title:"View Result", description:"See your love percentage and a humorous summary of your romantic fate.", icon: Heart }
 ]} 
 badges={["100% Free","Client-Side","Fun"]} 
 />

 <ToolFeatureGuides features={[
 { icon: Heart, title:"Deterministic Hash", description:"The same two names will always produce the exact same score, ensuring consistency."},
 { icon: Heart, title:"Animated Feedback", description:"High scores trigger a pulsing, bouncing heart animation to celebrate the match."},
 { icon: Heart, title:"Humorous Messages", description:"Custom written compatibility advice ranging from 'soulmates' to 'just friends'."},
 { icon: Heart, title:"Match History", description:"Keeps track of your last 5 calculations so you can compare different pairings."}
 ]}>
 ...
 { question:"Is this scientifically accurate?", answer:"Absolutely not. Love is a complex human emotion. This tool uses a mathematical string hash for entertainment purposes only."},
 { question:"Why do I get the same score every time?", answer:"The calculator is deterministic. The same input names will always generate the same hash value and resulting percentage."},
 { question:"Does the order of names matter?", answer:"No, the algorithm combines both names symmetrically, so swapping Name 1 and Name 2 will yield the exact same result."}
 ]} />
