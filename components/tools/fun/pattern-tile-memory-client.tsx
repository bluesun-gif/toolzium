...
export function PatternTileMemoryClient() {
  ...
  useEffect(() => {
    if (isPlaying && !isShowingTarget && playerGrid.length > 0 && playerGrid.every((c, i) => c === targetGrid[i])) {
      ...
      let s = 1;
      if (moves <= optimalMoves * 1.5) s = 3;else if (moves <= optimalMoves * 2.5) s = 2;
      setStars(s);
  ...
  const useHint = () => {
    ...
    setIsShowingTarget(true);
    setMoves(m => m + 5);
    toast("Hint used! +5 moves penalty", {
      icon: "💡"
    });
 ...
 {[3, 4, 5].map(size => <Button key={size} variant={gridSize === size ? "default" : "outline"} size="sm" onClick={() => setGridSize(size)}>
 {size}x{size}
 </Button>)}
 </div>
 ...
 {!isPlaying ? <Button onClick={startGame} className="gap-2"><Play className="w-4 h-4" /> {stars > 0 ? "Next Level" : "Start Game"}</Button> : <>
