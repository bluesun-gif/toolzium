...
import { Swords, RotateCcw, History, Play } from "lucide-react";
...
export default function RpsClient() {
 ...
 <ToolPageHeader icon={Swords} title="Rock Paper Scissors" description="Play the classic hand game against the computer." />
 ...
 {lastMatch && <div className="flex items-center justify-center gap-8 text-6xl animate-in fade-in zoom-in duration-300">
 ...
 {CHOICES.map(c => <Button key={c.id} onClick={() => play(c.id)} className="h-24 w-24 text-5xl flex flex-col items-center justify-center gap-2" variant="outline">
 <span>{c.emoji}</span>
 ...
 {history.map((m, i) => <div key={i} className={`p-2 rounded text-center text-sm border ${m.outcome === "win" ? "bg-green-500/10 border-green-500/30" : m.outcome === "lose" ? "bg-red-500/10 border-red-500/30" : "bg-muted/50 border-border/50"}`}>
 ...
 <ToolHowItWorks steps={[{
        ...
        step: "02",
        title: "CPU Picks",
        description: "The computer randomly selects its move simultaneously.",
        icon: Swords
      }, {
        step: "03",
        title: "Resolve",
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Large emojis and animations make the battle clear."
      }, {
        icon: Swords,
        title: "Match History",
        description: "Review your last 10 rounds to spot patterns."
      }, {
        icon: Swords,
        title: "Score Tracking",
        description: "Persistent score counter for your current session."
      }, {
        icon: Swords,
        title: "Fair RNG",
        description: "The CPU uses Math.random() ensuring a perfectly fair 33.3% chance for each move."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Is the computer truly random?",
        answer: "Yes, it uses JavaScript's built-in Math.random() which provides a uniform distribution across the three choices."
      }, {
        question: "Can I play best of 5?",
        answer: "You can manually track your wins. The tool keeps a running tally and a history of your last 10 matches."
      }, {
        question: "What if we both pick the same?",
        answer: "It's a draw! The round is recorded, but no points are awarded to either side."
      }]} />
    </div>
    </div>
);
