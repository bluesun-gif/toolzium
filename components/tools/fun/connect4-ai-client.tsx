...
const evaluateWindow = (window: number[], piece: number): number => {
  ...
  if (countPiece === 4) score += 100;else if (countPiece === 3 && countEmpty === 1) score += 5;else if (countPiece === 2 && countEmpty === 2) score += 2;
...
export function Connect4AiClient() {
 ...
 <select value={difficulty} onChange={e => setDifficulty(e.target.value as any)} className="px-3 py-2 rounded-lg bg-background border border-border text-sm">
 ...
 {Array(COLS).fill(0).map((_, cIdx) => <div key={`hover-${cIdx}`} className="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center" onMouseEnter={() => setHoverCol(cIdx)} onMouseLeave={() => setHoverCol(null)} onClick={() => handleColClick(cIdx)}>
 {hoverCol === cIdx && turn === "player" && board[0][cIdx] === EMPTY && <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-red-500 opacity-50 animate-pulse" />}
 </div>)}
 {board.map((row, rIdx) => row.map((cell, cIdx) => <div key={`${rIdx}-${cIdx}`} className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-900/40 rounded-full flex items-center justify-center cursor-pointer border border-blue-950/50 shadow-inner" onClick={() => handleColClick(cIdx)}>
 {cell === PLAYER && <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-red-500 shadow-md ${isWinningCell(rIdx, cIdx) ? 'ring-4 ring-white animate-pulse' : ''}`} />}
 {cell === AI_PIECE && <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-yellow-400 shadow-md ${isWinningCell(rIdx, cIdx) ? 'ring-4 ring-white animate-pulse' : ''}`} />}
 </div>))}
 </div>
 </div>
 </div>
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Select Easy, Medium, or Hard. Hard mode uses the Minimax algorithm with Alpha-Beta pruning for a perfect game.",
        icon: Zap
      }, {
        step: "02",
        title: "Drop Your Disc",
        description: "Click any column to drop your red disc. It will fall to the lowest available row due to gravity physics.",
        icon: Grid3X3
      }, {
        step: "03",
        title: "Connect Four",
        description: "Align four of your discs horizontally, vertically, or diagonally before the AI does to win the match.",
        icon: Shield
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "The Hard difficulty uses a depth-5 Minimax search tree with Alpha-Beta pruning to evaluate millions of board states."
      }, {
        icon: Grid3X3,
        title: "Gravity Physics",
        description: "Discs obey gravity, falling to the lowest available slot in the chosen column just like the physical board game."
      }, {
        icon: Zap,
        title: "Win Tracking",
        description: "Advanced algorithms scan the board for horizontal, vertical, and diagonal connections, highlighting the winning line."
      }, {
        icon: Shield,
        title: "Strategic Depth",
        description: "Develop your foresight and planning skills by anticipating the AI's counter-moves and setting up multi-turn traps."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "How does the Hard AI work?",
        answer: "The Hard AI uses the Minimax algorithm with Alpha-Beta pruning. It simulates future moves up to a certain depth, evaluating board positions based on piece alignment and center control, to choose the mathematically optimal move."
      }, {
        question: "Can I play offline?",
        answer: "Yes! The entire game logic, including the AI engine, runs 100% client-side in your browser. No internet connection or server API calls are required after the page loads."
      }, {
        question: "Is the board standard size?",
        answer: "Yes, we use the classic 7-column by 6-row grid, ensuring the game feels exactly like the physical tabletop version you know and love."
      }, {
