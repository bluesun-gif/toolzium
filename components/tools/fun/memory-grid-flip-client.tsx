...
export function MemoryGridFlipClient() {
        ...
        setTimeout(() => {
          ...
          if (matchedCards.every((c) => c.matched)) {
            ...
            if (!currentBest || (moves + 1) < currentBest) {
              setBestScores((prev) => ({ ...prev, [gridSize]: moves + 1 }));
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      <ToolPageHeader
        icon={Grid3X3}
        title="Memory Grid Flip"
        description="Test your spatial recall and working memory with this classic card-matching game. Flip cards, find pairs, and beat your best time."
      />
      ...
      <Card className={cardClass}>
        ...
        <CardContent className="p-4 sm:p-6 space-y-6">
          ...
          {isCompleted && (
            <div className="text-center p-4 bg-green-500/10 text-green-500 rounded-lg font-bold text-lg animate-pulse">
            ...
            {cards.map((card) => (
              ...
              >
                {card.flipped || card.matched ? (
                  <div className={`absolute inset-0 w-full h-full rounded-xl bg-white dark:bg-gray-800 flex items-center justify-center text-3xl sm:text-4xl shadow-inner border-2 transition-all duration-300 ${card.matched ? 'border-green-500 ring-4 ring-green-400/50 scale-95' : 'border-gray-200 dark:border-gray-700'}`}>
                    {card.emoji}
                  </div>
                ) : (
                  <div className="absolute inset-0 w-full h-full rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-2xl shadow-lg hover:scale-105 transition-transform border-2 border-indigo-400/50">
                    ?
                  </div>
                )}
              </div>
      ...
      <ToolHowItWorks
        steps={[
          { step: "01", title: "Select Grid Size", description: "Choose your difficulty level from 4x3 (12 cards), 4x4 (16 cards), or 6x6 (36 cards) layouts.", icon: Grid3X3 },
          { step: "02", title: "Flip & Memorize", description: "Click cards to reveal their hidden emojis. Memorize their locations before they flip back face-down.", icon: RotateCcw },
          { step: "03", title: "Match Pairs", description: "Select two matching emojis to lock them in place. Clear the entire board in the fewest moves possible.", icon: Timer }
        ]}
      ...
      <ToolFeatureGuides
        features={[
          { icon: Grid3X3, title: "Dynamic Grids", description: "Scale your challenge with 3 different board sizes, accommodating both beginners and memory champions." },
          { icon: RotateCcw, title: "Visual Feedback", description: "Matched pairs lock in with a satisfying green glow and scale animation, providing clear progress tracking." },
          { icon: Timer, title: "Performance Tracking", description: "Monitor your moves and time, with local best-score tracking to encourage continuous improvement." },
          { icon: Move, title: "Spatial Recall", description: "Specifically designed to exercise your visuospatial sketchpad and working memory capacity." }
        ]}
      >
      ...
      <ToolFaqAccordion
        faqs={[
          { question: "How is the best score calculated?", answer: "The best score is determined by the fewest number of moves taken to clear the board. Time is tracked for personal reference, but moves are the primary metric for efficiency." },
          { question: "Are the emojis randomized?", answer: "Yes, every new game selects a random subset of emojis from our diverse pool and shuffles their positions, ensuring no two games are exactly alike." },
          { question: "Does this game help with real-world memory?", answer: "Yes, practicing spatial recall tasks like this can improve your working memory capacity, which is useful for everyday tasks like remembering where you placed your keys or navigating new environments." },
          { question: "Can I play on mobile?", answer: "Absolutely. The grid is fully responsive and optimized for touch interactions, making it perfect for quick brain-training sessions on your phone or tablet." }
        ]}
      />
