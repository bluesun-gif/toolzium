...
export function SudokuClient() {
 ...
 `)}>
 {cell || <div className="grid grid-cols-3 gap-0.5 p-0.5 w-full h-full">
 {notes[r]?.[c]?.map((hasNote, i) => <span key={i} className="text-[8px] text-muted-foreground flex items-center justify-center">
 {hasNote ? i + 1 : ''}
 </span>) || null}
 </div>}
 </Button>;
