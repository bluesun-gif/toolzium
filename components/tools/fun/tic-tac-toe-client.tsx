...
const WINNING_LINES = [[0, 1, 2], [3, 4, 5], [6, 7, 8],
// Rows
[0, 3, 6], [1, 4, 7], [2, 5, 8],
// Cols
...
export default function TicTacToeClient() {
 ...
 {board.map((cell, i) => {
              ...
              return <Button key={i} onClick={() => handleClick(i)} variant="outline" className={`text-4xl font-bold h-full w-full p-0 transition-colors ${isWinCell ? "bg-green-500/20 border-green-500 text-green-600 dark:text-green-400" : ""} ${cell === "X" ? "text-primary" : "text-red-500"}`} disabled={!!cell || !!winner}>
 {cell}
 </Button>;
            })}
 </div>
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Take Turns",
        description: "Player X goes first, followed by Player O.",
        icon: Grid3X3
      }, {
        step: "02",
        title: "Claim Squares",
        description: "Click any empty square to place your mark.",
        icon: Grid3X3
      }, {
        step: "03",
        title: "Get Three",
        description: "Align three marks horizontally, vertically, or diagonally to win.",
        icon: Grid3X3
      }]} badges={["100% Free", "Client-Side", "Fun"]} />

 <ToolFeatureGuides features={[{
        icon: Grid3X3,
        title: "Win Highlighting",
        description: "Winning cells turn green to clearly show the victory line."
      }, {
        icon: Grid3X3,
        title: "Score Tracking",
        description: "Keeps a tally of wins for X, O, and draws across rounds."
      }, {
        icon: Grid3X3,
        title: "Turn Indicator",
        description: "Always shows whose turn it is to prevent confusion."
      }, {
        icon: Grid3X3,
        title: "Quick Reset",
        description: "Instantly clear the board for a rematch without losing scores."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Can I play against a computer?",
        answer: "This version is designed for local 2-player hot-seat gaming. Pass the device back and forth!"
      }, {
        question: "What happens if the board fills up?",
        answer: "If all 9 squares are filled and no one has 3 in a row, the game is declared a draw."
      }, {
