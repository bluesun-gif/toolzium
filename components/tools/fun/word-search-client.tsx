...
type Placement = { word: string, cells: string[] };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 px-2 sm:px-4 py-4 sm:py-6">
      <ToolPageHeader 
        ...
        description="Generate custom word search puzzles and find the hidden words in the grid." 
      />
                ...
                {grid.map((row, r) => 
                  row.map((char, c) => {
                    ...
                    const isFound = Array.from(foundWords).some(w => 
                      placements.find(p => p.word === w)?.cells.includes(cellId)
                    );
                    ...
                    return (
                      <button
                        ...
                        className={`w-7 h-7 sm:w-9 sm:h-9 flex items-center justify-center font-bold text-sm sm:text-base rounded transition-all border ${
                          isFound 
                ...
                {placements.map(p => (
                  <li 
                    ...
                    className={`font-mono tracking-wider text-sm ${
                      foundWords.has(p.word) ? "line-through text-muted-foreground" : "text-foreground"
                    }`}
                  >
                    {p.word}
                  </li>
      ...
      <ToolHowItWorks 
        steps={[
          { step: "01", title: "Pick a Theme", description: "Choose Animals, Fruits, or Colors to generate your custom puzzle.", icon: Grid3X3 },
          { step: "02", title: "Select Letters", description: "Click and highlight the cells in the grid that spell a hidden word.", icon: Search },
          { step: "03", title: "Verify & Win", description: "Hit 'Check Selection' to mark the word as found and clear the board.", icon: CheckCircle2 }
        ]} 
        badges={["100% Free", "Client-Side", "Fun"]} 
      />

      <ToolFeatureGuides features={[
        { icon: Search, title: "Dynamic Generation", description: "Every time you switch themes or click reset, a brand new puzzle is created." },
        { icon: Grid3X3, title: "Multiple Themes", description: "Choose from Animals, Fruits, or Colors to tailor the vocabulary." },
        { icon: CheckCircle2, title: "Smart Validation", description: "The game checks your selected cells against all hidden word coordinates." },
        { icon: RotateCcw, title: "Endless Replay", description: "Generate infinite variations of puzzles for continuous brain training." }
      ]}>
      ...
      <ToolFaqAccordion faqs={[
        { question: "How do I highlight a word?", answer: "Click on each individual letter cell that makes up the word to highlight it, then click the 'Check Selection' button to verify." },
        { question: "Are words placed diagonally?", answer: "To ensure a smooth digital experience, words are currently placed only horizontally and vertically." },
        { question: "Can I generate a new puzzle with the same theme?", answer: "Yes! Simply click the 'New Grid' button to reshuffle the words and random letters for the current theme." }
      ]} />
