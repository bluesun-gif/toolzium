...
export function MemorySequenceClient() {
 ...
 <select value={mode} onChange={e => setMode(e.target.value as any)} className="px-3 py-2 rounded-lg bg-background border border-border text-sm" disabled={phase !== "idle" && phase !== "result"}>
 <option value="numbers">Numbers Only</option>
 <option value="mixed">Numbers + Letters</option>
 </select>
 {phase === "idle" || phase === "result" ? <Button onClick={startGame} variant="default" size="sm">
 <Play className="w-4 h-4 mr-2" /> Start
 </Button> : <Button onClick={resetGame} variant="outline" size="sm">
 ...
 {phase === "countdown" && <div className="text-6xl font-bold text-primary animate-pulse">{countdown}</div>}
 {phase === "show" && <div className="text-4xl sm:text-5xl font-mono font-bold tracking-widest text-foreground">
 {sequence.join("")}
 </div>}
 {phase === "input" && <div className="text-4xl sm:text-5xl font-mono font-bold tracking-widest text-muted-foreground">
 {userInput.map((c, i) => <span key={i} className="text-green-500">{c}</span>)}
 <span className="animate-pulse ml-1">_</span>
 </div>}
 {phase === "result" && <div className="text-center space-y-2">
 ...
 {phase === "input" && <div className="grid grid-cols-6 sm:grid-cols-9 gap-2 max-w-2xl mx-auto">
 {pool.map(char => <Button key={char} variant="outline" className="h-12 text-lg font-mono font-bold" onClick={() => handleInput(char)}>
 {char}
 </Button>)}
 </div>}
 </CardContent>
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "A sequence of numbers or letters will appear for a few seconds. Memorize the exact order.",
        icon: Hash
      }, {
        step: "02",
        title: "Recall from Memory",
        description: "Once hidden, use the on-screen keypad to input the sequence exactly as you saw it.",
        icon: Type
      }, {
        step: "03",
        title: "Advance the Levels",
        description: "Each successful recall increases the sequence length and decreases the viewing time.",
        icon: Play
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Sequences start at 3 items and grow infinitely, pushing the boundaries of your short-term memory."
      }, {
        icon: Type,
        title: "Mixed Modes",
        description: "Switch between pure numbers or a complex mix of alphanumeric characters to increase cognitive load."
      }, {
        icon: Play,
        title: "Time Pressure",
        description: "The viewing window shrinks as you level up, forcing your brain to encode information faster."
      }, {
        icon: RotateCcw,
        title: "Streak Tracking",
        description: "Monitor your consecutive successful recalls to maintain focus and build cognitive endurance."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "Chunking is a memory strategy where you group individual items into larger, meaningful units (e.g., remembering '192' and '845' instead of '1,9,2,8,4,5'). This tool helps you practice this technique."
      }, {
        question: "Why does the time decrease?",
        answer: "To simulate real-world cognitive demands. As your brain becomes more efficient at encoding, the tool forces you to process information faster, preventing plateauing."
      }, {
        question: "Is the mixed mode harder?",
        answer: "Yes, switching between numbers and letters requires more cognitive effort and working memory resources than processing a single category."
      }, {
