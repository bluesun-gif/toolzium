...
Return exactly 10 slogans, one per line, with no numbering and no extra text.`;
      ...
      setSlogans([`${brandName}: Built for what matters.`, `${brandName}: Simple. Strong. Ready.`, `Move forward with ${brandName}.`, `${brandName} makes it easier.`, `Where quality meets ${industry || "innovation"}: ${brandName}.`, `${brandName}: The smarter way ahead.`, `Feel the difference with ${brandName}.`, `${brandName}: Designed to deliver.`, `Your next step starts with ${brandName}.`, `${brandName}: Small details, big impact.`]);
 ...
 <Input value={brandName} onChange={e => setBrandName(e.target.value)} placeholder="e.g. Nova Labs" />
 </div>
 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">Industry</label>
 <Input value={industry} onChange={e => setIndustry(e.target.value)} placeholder="e.g. software, coffee, fitness" />
 </div>
 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">Values / Personality</label>
 <Input value={values} onChange={e => setValues(e.target.value)} placeholder="e.g. bold, premium, friendly" />
 </div>
 </div>
 ...
 {slogans.length > 0 && <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 {slogans.map((slogan, index) => <Card key={`${slogan.slice(0, 12)}-${index}`} className={cardClass}>
 <CardHeader className={headerClass}>
 <CardTitle className={titleClass}>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "You may be able to, but you should check trademark availability and consult a legal professional."
      }, {
