...
import { Mail, Sparkles, RefreshCw, Target, Copy, Type } from "lucide-react";
...
...`;
      ...
      if (data.success && data.raw) {
        const parts = String(data.raw).replace(/```[a-z]*\n?/gi, "").split("|||").map((part: string) => part.trim()).filter(Boolean);
        if (parts.length >= 3) {
      ...
      setEmails([{
        ...
        body: `Hello,\n\nI understand that ${painPoint || "operational efficiency"} is a priority for teams like yours. ${product} was designed to help ${recipientRole || "teams"} solve this more effectively.\n\nWould you be open to a brief conversation?\n\nBest regards,\nYour Name`
      }, {
        label: "Friendly",
        subject: `Quick idea for ${recipientRole || "your team"}`,
        body: `Hi there,\n\nI came across your work and thought ${product} could be useful for ${painPoint || "improving your workflow"}.\n\nIf you're open to it, I'd love to share a quick idea.\n\nCheers,\nYour Name`
      }, {
        label: "AIDA Framework",
        subject: `Reduce ${painPoint || "friction"} with ${product}`,
        body: `Attention: If ${painPoint || "slow processes"} are holding your team back, this is relevant.\n\nInterest: ${product} helps ${recipientRole || "businesses"} streamline the work that matters most.\n\nDesire: Teams can save time, reduce errors, and move faster.\n\nAction: ${cta || "Book a call"} and see whether it fits your needs.\n\nYour Name`
      }]);
 ...
 <Input value={product} onChange={e => setProduct(e.target.value)} placeholder="e.g. AI-powered CRM" />
 </div>
 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">Recipient Role</label>
 <Input value={recipientRole} onChange={e => setRecipientRole(e.target.value)} placeholder="e.g. Marketing Director" />
 </div>
 </div>
 ...
 <Input value={painPoint} onChange={e => setPainPoint(e.target.value)} placeholder="e.g. slow reporting" />
 </div>
 <div className="space-y-2">
 <label className="text-xs font-medium text-muted-foreground">Desired CTA</label>
 <Input value={cta} onChange={e => setCta(e.target.value)} placeholder="e.g. book a demo" />
 </div>
 </div>
 ...
 {emails.length > 0 && <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
 {emails.map((email, index) => <Card key={`${email.label}-${index}`} className={cardClass}>
 <CardHeader className={headerClass}>
 <CardTitle className={titleClass}>
 ...
 <CopyButton getText={() => `Subject: ${email.subject}\n\n${email.body}`} label="Copy Email" />
 </CardContent>
 </Card>)}
 </div>}
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "Short is usually better. Aim for clarity and a single clear call to action."
      }, {
        ...
        answer: "Yes. Personalize each email and make sure the message matches the recipient's context."
      }]} />
    </div>
    </div>
);
