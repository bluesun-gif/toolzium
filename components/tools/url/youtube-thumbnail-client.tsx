      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 space-y-8 relative z-10">
        ...
        <div className="grid gap-6 md:grid-cols-3">
            ...
            <GlassCard className="p-5 bg-background border-border shadow-sm rounded-2xl sticky top-24">
              ...
              <form onSubmit={handleGetThumbnails} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <Input placeholder="https://www.youtube.com/watch?v=..." value={url} onChange={e => setUrl(e.target.value)} disabled={loading} className="w-full bg-background border-border" />
                </div>

                <div className="flex gap-2">
                  <Button type="submit" className="flex-1 bg-primary hover:bg-primary/90 text-primary-foreground font-bold h-11 rounded-xl shadow-md shadow-primary/20" disabled={loading}>
                    {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Youtube className="mr-2 h-4 w-4" />}
                    Get Thumbnails
                  </Button>
                  {(videoId || url) && <Button type="button" variant="outline" onClick={handleClear} disabled={loading} className="h-11 rounded-xl border-border">
                      <RefreshCw className="h-4 w-4" />
                    </Button>}
                </div>

                {videoId && <Button type="button" variant="outline" onClick={runAiCtrAudit} disabled={isAnalyzingAi} className="w-full mt-2 border-primary/30 text-primary bg-primary/10 font-bold h-10 rounded-xl text-xs gap-1.5">
                    {isAnalyzingAi ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Wand2 className="w-3.5 h-3.5" />}
          ...
          <div className="md:col-span-2 space-y-6">
            {videoId ? <div className="space-y-6">
                <div className="grid gap-6 sm:grid-cols-2">
                  {thumbnails.map((t, idx) => <GlassCard key={idx} className="overflow-hidden flex flex-col justify-between p-0 bg-background border-border shadow-sm rounded-2xl">
                      <div className="relative aspect-video w-full bg-muted overflow-hidden">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={t.url} alt={`${t.quality} preview`} className="h-full w-full object-cover" onError={e => {
                      if (t.name === "maxres") {
                      ...
                      <div className="p-4 space-y-3">
                        ...
                        <Button variant="default" size="sm" onClick={() => downloadThumbnail(t.url, t.name)} className="w-full flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground font-bold h-10 rounded-xl">
                ...
                {aiAnalysis && <GlassCard className="p-5 bg-card/80 backdrop-blur-md border-border rounded-2xl space-y-2">
              ...
              </div> : <GlassCard className="flex flex-col items-center justify-center py-16 text-center border-dashed border-2 border-border rounded-2xl">
...
<ToolHowItWorks steps={[{
        ...
        <ToolFaqAccordion faqs={[{
          ...
          answer: "Copy the YouTube video URL, paste it into our tool, and click 'Get Thumbnails'."
        }, {
