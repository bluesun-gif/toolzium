  ...
  const runAiSafetyAudit = async (targetUrl: string) => {
    ...
    try {
      ...
      if (data.success && data.raw) {
        setResults((prev) =>
          prev.map((r) => (r.final === targetUrl ? { ...r, aiAnalysis: data.raw } : r))
        );
  ...
  return (
    <div className="w-full min-h-screen pb-20 relative">
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 space-y-8 relative z-10">
        <ToolPageHeader
          icon={Link2}
          title="Link Expander & URL Unshortener Studio"
          description="Reveal the final destination of shortened URLs from bit.ly, t.co, tinyurl, and custom short domains with real-time AI security audits."
        />
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className={cn("h-8 text-xs rounded-lg font-bold", !isBatch ? "bg-primary text-white" : "border-border")}
                >
                  Single URL
                </Button>
                <Button
            ...
            {results.length > 0 && (
              <div className="space-y-4 pt-4 border-t border-border">
                ...
                <div className="space-y-4">
                  {results.map((res, i) => (
                    <GlassCard key={i} className="p-5 border-l-4 border-l-primary bg-card/70 backdrop-blur-md rounded-2xl space-y-3">
                      <div className="flex items-center justify-between flex-wrap gap-2">
                        ...
                        {res.safe ? (
                          ...
                          <span className="px-3 py-1 rounded-full bg-red-500/10 text-red-500 border border-red-500/20 text-xs font-bold flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-2 bg-background p-3.5 rounded-xl border border-border">
                        <a
                          ...
                          className="font-mono text-sm font-bold text-primary hover:underline break-all flex-1 flex items-center gap-1.5"
                        >
                      ...
                      <div className="flex items-center justify-between pt-1 text-xs text-muted-foreground">
                        ...
                        <button
                          type="button"
                          onClick={() => runAiSafetyAudit(res.final)}
                      ...
                      {res.aiAnalysis && (
                        <div className="mt-2 p-3 bg-muted/40 rounded-xl border border-border text-xs leading-relaxed text-foreground whitespace-pre-wrap">
                          {res.aiAnalysis}
                        </div>
                      )}
                    </GlassCard>
        ...
        {history.length > 0 && (
          <GlassCard className="p-5 bg-background border-border shadow-sm rounded-2xl">
            ...
            <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1">
              {history.map((h, i) => (
                <div key={i} className="p-3 bg-muted/30 rounded-xl border border-border flex justify-between items-center text-xs">
        ...
        <ToolFaqAccordion
          faqs={[
            { question: "Is it safe to expand links using this tool?", answer: "Yes. The expansion process happens via secure API calls that only read HTTP headers of the redirect chain." },
            { question: "What URL shorteners are supported?", answer: "We support virtually all standard 301 and 302 redirect chains, including Bitly, TinyURL, Twitter's t.co, and YouTube's youtu.be." },
          ]}
        />
