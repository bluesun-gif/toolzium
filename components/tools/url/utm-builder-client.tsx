...
export function UTMBuilderClient() {
  ...
  const exportCSV = () => {
    ...
    const rows = history
      .map((h) => [h.date, `"${h.url}"`, h.source, h.medium, h.campaign].join(","))
      .join("\n");
    ...
    return (
      <div className="relative bg-white p-3 rounded-2xl border border-slate-200 shadow-md inline-block">
        <svg viewBox={`0 0 ${size} ${size}`} className="w-24 h-24">
          {matrix.map((row, r) =>
            row.map((cell, c) =>
              cell === 1 ? <rect key={`${r}-${c}`} x={c} y={r} width="1" height="1" fill="#000000" /> : null
            )
          )}
        </svg>
      </div>
  ...
  return (
    <div className="w-full min-h-screen pb-20 relative">
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 space-y-8 relative z-10">
        <ToolPageHeader
          icon={Link2}
          title="UTM Campaign Builder & Link Tagging Studio"
          description="Build, track, and manage marketing campaign URLs with precise UTM parameters, AI tag suggestions, and bulk generation."
        />
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  className={cn(
                    ...
                    activeTab === "single" ? "bg-primary text-white" : "border-border text-muted-foreground"
                  )}
                >
                  Single URL Builder
                </Button>
                <Button
                  ...
                  className={cn(
                    "rounded-xl font-bold text-xs h-9 px-4",
                    activeTab === "bulk" ? "bg-primary text-white" : "border-border text-muted-foreground"
                  )}
                >
            ...
            {activeTab === "single" ? (
                ...
                <div className="space-y-4 bg-muted/30 p-5 rounded-2xl border border-border flex flex-col justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="h-11 rounded-xl border-border font-bold text-xs"
                    >
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    className="w-full rounded-xl border border-border bg-background p-3.5 text-xs font-mono outline-none focus:ring-2 focus:ring-primary/50 text-foreground min-h-[140px]"
                    value={bulkUrls}
                ...
                <div className="space-y-2">
                  ...
                  <div className="max-h-60 overflow-y-auto space-y-2 p-2 bg-muted/20 rounded-xl border border-border">
                    {bulkResults.map((u, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-2 p-2.5 bg-background rounded-lg border border-border text-xs font-mono break-all text-foreground"
                      >
          ...
          <GlassCard className="p-5 bg-background border-border shadow-sm rounded-2xl">
            <div className="flex justify-between items-center border-b border-border pb-3 mb-3">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  className="h-8 text-xs text-muted-foreground hover:text-red-500 font-semibold"
                >
            ...
            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
              {history.map((h, i) => (
                <div
                  key={i}
                  className="p-3 bg-muted/30 rounded-xl border border-border flex flex-col sm:flex-row sm:items-center justify-between text-xs gap-2"
                >
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => handleCopy(h.url)}
                    className="h-7 text-xs border-border font-semibold"
                  >
                    <Copy className="w-3.5 h-3.5" />
                  </Button>
                </div>
        ...
        <ToolFeatureGuides
          features={[
            ...
            { icon: History, title: "Local History", description: "Automatically saves your last 15 generated links for quick retrieval and copying." },
        ...
        <ToolFaqAccordion
          faqs={[
            { question: "Do I need all 5 UTM parameters?", answer: "No. Only utm_source, utm_medium, and utm_campaign are strictly required for GA4 to track campaigns properly." },
            { question: "Are spaces allowed in UTM parameters?", answer: "It is best practice to avoid spaces. Use underscores (e.g. spring_sale) or hyphens instead." },
          ]}
        />
