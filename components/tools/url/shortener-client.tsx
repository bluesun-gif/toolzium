...
export function ShortenerClient() {
  ...
  const handleShorten = useCallback(async () => {
    const urlsToProcess = isBatchMode ? batchInput.split("\n").map(u => u.trim()).filter(Boolean) : [longUrl.trim()].filter(Boolean);
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 space-y-8 relative z-10">
            ...
            {activeResults.length > 0 ? <motion.div initial={{
              ...
              y: 0
                ...
                <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                  {activeResults.map(item => <GlassCard key={item.id} className="p-5 space-y-3 border-l-4 border-l-primary bg-card/70 backdrop-blur-md rounded-2xl">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <a href={item.shortUrl} target="_blank" rel="noopener noreferrer" className="text-base font-extrabold text-primary hover:underline flex items-center gap-1.5 font-mono">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {item.qrDataUrl && <Button variant="outline" size="sm" onClick={() => setQrModalUrl(item.qrDataUrl || null)} className="h-8 text-xs gap-1 border-border font-semibold">
        ...
        {history.length > 0 && <GlassCard className="p-5 bg-background border-border shadow-sm rounded-2xl">
            <div className="flex justify-between items-center border-b border-border pb-3 mb-3">
              ...
              <Button variant="ghost" size="sm" onClick={handleClearHistory} className="h-7 text-xs text-muted-foreground hover:text-red-500 font-semibold">
                <Trash2 className="w-3.5 h-3.5 mr-1" /> Clear History
              </Button>
            </div>

            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
              {history.map((item, idx) => <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between text-xs bg-muted/30 p-3.5 rounded-xl border border-border gap-2">
                  <div className="truncate max-w-md">
                    <a href={item.shortUrl} target="_blank" rel="noopener noreferrer" className="font-bold text-primary hover:underline font-mono text-sm">
                      {item.shortUrl}
                    </a>
                    <p className="text-[11px] text-muted-foreground truncate">{item.original}</p>
                  </div>
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    <Button variant="outline" size="sm" onClick={() => fetchAnalytics(item.slug)} className="h-8 text-xs text-primary bg-primary/10 border-primary/20 font-semibold gap-1">
                      <BarChart3 className="w-3.5 h-3.5" /> Stats
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => handleCopy(item.shortUrl, "Short link")} className="h-8 text-xs border-border font-semibold">
        ...
        {analyticsSlug && <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-md flex items-center justify-center p-4">
            ...
            opacity: 1
              ...
              {isLoadingAnalytics ? <div className="py-12 flex flex-col items-center justify-center text-muted-foreground space-y-2">
                  <RefreshCcw className="w-6 h-6 animate-spin text-primary" />
                  <span className="text-xs font-semibold">Fetching telemetry data...</span>
                </div> : analyticsData ? <div className="space-y-4 text-xs">
                ...
                </div> : <div className="py-8 text-center text-muted-foreground space-y-1">
