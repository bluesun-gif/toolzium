...
export function ShortenerClient() {
 ...
 <span className="text-xs font-mono bg-slate-100 text-slate-700 dark:text-slate-300 px-2.5 py-2 rounded-lg border border-border">toolzium.com/</span>
 <Input
 ...
 <textarea
 className="w-full rounded-xl border border-border bg-background p-3 text-sm outline-none focus:ring-2 focus:ring-blue-500 font-mono text-foreground min-h-[140px]"
 ...
 <Button variant="outline"size="sm"onClick={() => handleCopy(item.shortUrl,"Short URL")} className="h-8 text-xs gap-1 border-border">
 ...
 {item.qrDataUrl && (
 <Button variant="outline"size="sm"onClick={() => setQrModalUrl(item.qrDataUrl || null)} className="h-8 text-xs gap-1 border-border">
 ...
 <p className="text-xs text-muted-foreground truncate max-w-full font-mono bg-slate-50 p-2 rounded-lg border border-border/60">
 ...
 <p className="text-xs max-w-xs mt-1 text-muted-foreground">Paste your long destination URL on the left to generate clean short links, scannable QR codes, and click analytics.</p>
 </GlassCard>
 )}
 </div>
 </div>
 ...
 <History className="w-3.5 h-3.5 text-primary"/> Your Shortened Links & Analytics ({history.length})
 </span>
 ...
 <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between text-xs bg-slate-50 p-3 rounded-xl border border-border/70 gap-2">
 ...
 <Button variant="outline"size="sm"onClick={() => handleCopy(item.shortUrl,"Short link")} className="h-7 px-2.5 text-xs border-border">
 ...
 className="bg-background border border-border rounded-3xl p-6 max-w-lg w-full shadow-2xl space-y-4"
 >
 <div className="flex justify-between items-center border-b border-slate-100 pb-3">
 <div>
 <span className="text-[10px] font-bold text-primary uppercase tracking-widest">Link Telemetry Report</span>
 ...
 className="h-8 w-8 rounded-full hover:bg-slate-100 dark:hover:bg-muted text-muted-foreground"
 >
 <X className="w-4 h-4"/>
 </Button>
 </div>
 ...
 <div className="bg-slate-50 p-3 rounded-2xl border border-border/80 space-y-2 text-xs">
 {analyticsData.topReferrers.length > 0 ? (
 analyticsData.topReferrers.map(([source, count], i) => (
 <div key={i} className="flex justify-between items-center p-2 rounded-lg bg-background border border-border/60">
 <span className="font-semibold text-foreground dark:text-slate-200 flex items-center gap-2 font-mono">
 {source.includes("QR") ? (
 ...
 <p className="text-muted-foreground text-center text-xs py-3">No click referrers recorded yet.</p>
 )}
 </div>
 </div>
 </div>
 ...
 initial={{ scale: 0.95, opacity: 0 }}
 animate={{ scale: 1, opacity: 1 }}
 className="bg-background border border-border rounded-3xl p-6 max-w-sm w-full shadow-2xl text-center space-y-4"
 >
 <h3 className="text-base font-bold text-foreground">Short Link QR Code</h3>
 <div className="p-4 bg-background rounded-2xl inline-block border border-border shadow-md">
 ...
 <Button variant="outline"size="sm"onClick={() => setQrModalUrl(null)} className="text-xs rounded-xl border-border">
 Close
 </Button>
 </div>
 </motion.div>
