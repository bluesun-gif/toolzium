...
export default function PdfCompressClient() {
 ...
 {file && <div className="text-xs text-muted-foreground">Original size: {(file.size / 1024).toFixed(1)} KB</div>}
 </label>
 </div>
 ...
 {QUALITY_PRESETS.map((p, i) => <Button key={i} type="button" size="sm" variant={preset === i ? "default" : "outline"} onClick={() => setPreset(i)} className="text-xs font-semibold h-9">
 {p.label}
 </Button>)}
 </div>
 ...
 {/* Progress */}
 {compressing && progress && <div className="text-xs font-semibold text-primary animate-pulse">{progress}</div>}

 {/* Result stats */}
 {stats && <div className="p-4 rounded-xl border bg-background/60 text-center space-y-1">
 ...
 {stats.newSize < stats.orig ? <span className="text-green-600 ml-2">
 (↓ {((stats.orig - stats.newSize) / stats.orig * 100).toFixed(1)}% saved)
 </span> : <span className="text-amber-500 ml-2">(file was already optimized)</span>}
 </div>
 </div>}

 {/* Compress button */}
 {file && <div className="flex justify-end pt-2">
 <Button onClick={compressPdf} disabled={compressing} className="gap-2 font-bold h-11 px-6 shadow-md">
 ...
 <ToolHowItWorks steps={[{
        step: "1",
        title: "Upload PDF",
        description: "Select the PDF file you want to compress from your device.",
        icon: Upload
      }, {
        step: "2",
        title: "Select Compression Level",
        description: "Choose a preset quality level to balance size reduction and image clarity.",
        icon: Settings
      }, {
        step: "3",
        title: "Compress & Download",
        description: "Process the file entirely in your browser and download the optimized PDF.",
        icon: Download
      }]} badges={["100% Free", "Client-Side", "Up to 90% Smaller"]} />

 <ToolFeatureGuides features={[{
        title: "Adjustable Compression Levels",
        description: "Choose from maximum quality to smallest file size to fit your needs.",
        icon: FileArchive
      }, {
        title: "Intelligent Image Optimization",
        description: "Re-renders pages as optimized JPEGs to significantly reduce file bloat.",
        icon: Image
      }, {
        title: "Removes Hidden Metadata",
        description: "Strips out unnecessary embedded fonts and metadata during the re-rendering process.",
        icon: FileText
      }, {
        title: "Absolute Privacy",
        description: "All processing happens locally in your browser. Your confidential PDFs are never uploaded to any server.",
        icon: Shield
      }]}>
 <p>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Will I lose quality when compressing my PDF?",
        answer: "Compression inherently involves some loss of quality, especially for images. However, you can select the compression level that best balances file size and visual clarity for your specific needs."
      }, {
        question: "Is there a maximum file size limit?",
        answer: "Since the compression happens entirely in your browser, the maximum file size depends on your device's available memory. Most modern devices can easily handle PDFs up to 50MB."
      }, {
        question: "What kind of compression ratio can I expect?",
        answer: "The compression ratio varies depending on the original file. Image-heavy or scanned PDFs often see the most significant reductions, sometimes up to 90%. Text-heavy PDFs that are already optimized may see less reduction."
      }, {
