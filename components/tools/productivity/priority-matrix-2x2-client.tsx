...
export function PriorityMatrixClient() {
  ...
  const quadrants = [{
    name: "Quick Wins",
    desc: "High Impact, Low Effort. Do it now.",
    colorClass: "bg-green-100 dark:bg-green-900/30 border-green-300"
  }, {
    name: "Major Projects",
    desc: "High Impact, High Effort. Plan it.",
    colorClass: "bg-primary/10 dark:bg-blue-900/30 border-blue-300"
  }, {
    name: "Fill-ins",
    desc: "Low Impact, Low Effort. Do it later.",
    colorClass: "bg-yellow-100 dark:bg-yellow-900/30 border-yellow-300"
  }, {
 ...
 <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="E.g. Redesign homepage" onKeyDown={e => e.key === "Enter" && addTask()} />
 </div>
 <div className="space-y-2">
 ...
 {qTasks.map(t => <div key={t.id} className="bg-background/80 backdrop-blur-sm p-3 rounded-lg border shadow-sm flex items-start justify-between gap-2 group">
 <div className="flex-1 min-w-0">
 <p className="font-medium text-sm truncate">{t.title}</p>
 <p className="text-[10px] text-muted-foreground mt-1">Impact: {t.impact} | Effort: {t.effort}</p>
 </div>
 <Button onClick={() => removeTask(t.id)} className="opacity-0 group-hover:opacity-100 transition-opacity text-red-500 hover:text-red-700">
 <Trash2 className="w-4 h-4" />
 </Button>
 </div>)}
 {qTasks.length === 0 && <div className="flex-1 flex items-center justify-center text-muted-foreground text-sm italic opacity-50">
 No tasks
 </div>}
 </div>
<<<<<<< HEAD
