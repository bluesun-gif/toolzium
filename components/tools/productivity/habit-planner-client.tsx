...
const DEFAULT_HABITS: Habit[] = [
  { id: "h1", name: "Morning Exercise & Stretch", category: "Health", days: [true, true, false, true, false, false, false] },
  { id: "h2", name: "Read 20 Pages of Book", category: "Personal", days: [true, true, true, true, true, false, false] },
  { id: "h3", name: "Code Review & Refactoring", category: "Work", days: [true, true, true, true, true, false, false] },
];
...
export function HabitPlannerClient() {
  ...
  const addHabit = () => {
    ...
    const newHabits = [
      ...habits,
      { id: Date.now().toString(), name: newHabitName.trim(), category: newHabitCategory.trim() || "General", days: Array(7).fill(false) },
    ];
  ...
  const calculateCompletion = (days: boolean[]) => {
    ...
    <div className="relative max-w-6xl mx-auto space-y-8">
      ...
      <ToolPageHeader
        ...
        description="Track your daily habits, build routines, and maintain your weekly streaks with contrast-optimized check-ins."
        actions={<ResetButton onClick={resetWeek} label="Reset Week & Increment Streak" />}
      />
                ...
                <table className="w-full border-collapse min-w-[550px]">
                  <thead>
                    <tr className="border-b border-border/80 text-muted-foreground text-xs uppercase font-bold">
                      ...
                      {DAYS.map((day) => (
                        <th key={day} className="text-center py-3 px-1 w-12">{day}</th>
                      ))}
                  ...
                  <tbody>
                    {habits.length === 0 ? (
                      ...
                      habits.map((habit) => {
                        ...
                        return (
                          <tr key={habit.id} className="border-b border-border/40 hover:bg-muted/20 transition-colors">
                            <td className="py-3 px-3">
                              <div className="font-bold text-sm text-foreground">{habit.name}</div>
                              {habit.category && (
                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{habit.category}</span>
                              )}
                            </td>
                            {habit.days.map((isDone, idx) => (
                              <td key={idx} className="text-center py-3 px-1">
                                <button
                                  ...
                                  className={cn(
                                    "w-8 h-8 rounded-lg flex items-center justify-center transition-all mx-auto shadow-xs",
                                    isDone
                                      ? "bg-primary text-white shadow-md shadow-primary/20 scale-105"
                                      : "bg-muted/60 hover:bg-muted text-muted-foreground/30 border border-border/60"
                                  )}
                                  title={`Toggle ${DAYS[idx]}`}
                                >
