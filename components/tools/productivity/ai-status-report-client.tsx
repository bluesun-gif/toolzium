...
export function AiStatusReportClient() {
 ...
 <ToolPageHeader
 icon={Activity}
 title="AI Weekly Progress & Status Report Generator"
 description="Generate executive Weekly Status Reports (PPP: Progress, Plans, Problems) formatted for leadership and Slack updates."
 />
 ...
 <textarea
 className="w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50 min-h-[90px]"
 placeholder="e.g. Shipped new auth flow, updated database schemas, fixed memory leak in worker thread"
 value={completedWork}
 ...
 <textarea
 className="w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50 min-h-[80px]"
 placeholder="e.g. Waiting for third-party API approval, staging server slowdown"
 value={blockersInput}
 ...
 <textarea
 className="w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50 min-h-[80px]"
 placeholder="e.g. Complete end-to-end load testing, prepare product release notes"
 value={upcomingTasks}
 onChange={(e) => setUpcomingTasks(e.target.value)}
 />
 </div>
 ...
 <span className={`text-xs font-extrabold px-2.5 py-0.5 rounded uppercase font-mono ${
 report.overallHealth ==="On Track"?"bg-emerald-500/10 text-emerald-500 border border-emerald-500/20":"bg-amber-500/10 text-amber-500 border border-amber-500/20"
 }`}>
 ...
 <ToolHowItWorks
 steps={[
 { step:"01", title:"Enter Progress & Blockers", description:"Input weekly wins, current bottlenecks, and upcoming sprint goals.", icon: Activity },
 { step:"02", title:"Automated PPP Structure", description:"Formats notes into Progress, Problems, and Plans standard frameworks.", icon: Sliders },
 { step:"03", title:"Share Status Update", description:"Copy executive executive summaries directly into Slack or email newsletters.", icon: CheckCircle2 }
 ]}
 ...
 <ToolFeatureGuides
 features={[
 { icon: Activity, title:"PPP Methodology Standard", description:"Follows Silicon Valley Progress-Problems-Plans standard for concise reporting."},
 { icon: AlertTriangle, title:"Automated Risk Calculation", description:"Evaluates project health based on blocker count and dependency severity."},
 { icon: CheckCircle2, title:"Private Execution", description:"Generates status reports strictly inside your client browser context."}
 ]}
 >
 ...
 <ToolFaqAccordion
 faqs={[
 { question:"What is the PPP framework?", answer:"PPP stands for Progress (what was accomplished), Problems (current blockers), and Plans (upcoming goals for next week)."},
 { question:"Can I use this for monthly reports?", answer:"Yes! Simply paste monthly accomplishments and goals into the input fields."}
 ]}
 />
