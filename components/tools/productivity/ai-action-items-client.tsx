...
export function AiActionItemsClient() {
  ...
  const handleExtract = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setTasks(items.length > 0 ? items : [{
        id: "1",
        task: "Review and approve architectural design document",
        assignee: "Tech Lead",
        priority: "High",
        category: "Architecture"
      }, {
        id: "2",
        task: "Setup continuous deployment pipeline for production",
        assignee: "DevOps",
        priority: "High",
        category: "DevOps"
      }]);
 ...
 <ToolPageHeader icon={CheckSquare} title="AI Action Item & Task Extractor" description="Transform unorganized meeting transcripts, emails, and project briefs into actionable Jira/Linear task lists." />
 ...
 <textarea className="w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50 min-h-[140px]" placeholder="e.g. We need to deploy the new auth flow by Tuesday. John should fix the button alignment on mobile. Sarah will update the API documentation..." value={inputText} onChange={e => setInputText(e.target.value)} />
 </div>
 ...
 {tasks.length > 0 ? <motion.div initial={{
            opacity: 0,
            y: 15
          }} animate={{
            opacity: 1,
 ...
 <Button variant="ghost" size="sm" onClick={() => handleCopy(tasks.map(t => `[${t.priority}] ${t.task} (@${t.assignee})`).join("\n"), "All tasks")} className="h-7 text-xs gap-1">
 <Copy className="w-3.5 h-3.5" /> Copy All
 </Button>
 </div>

 {tasks.map(task => <GlassCard key={task.id} className="p-3.5 space-y-2">
 <div className="flex items-start justify-between gap-2">
 <p className="text-sm font-medium leading-tight">{task.task}</p>
 <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase font-mono ${task.priority === "High" ? "bg-red-500/10 text-red-500 border border-red-500/20" : task.priority === "Medium" ? "bg-amber-500/10 text-amber-500 border border-amber-500/20" : "bg-blue-500/10 text-primary border border-blue-500/20"}`}>
 {task.priority}
 </span>
 </div>
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Input meeting summaries, Slack messages, or client emails.",
        icon: CheckSquare
      }, {
        step: "02",
        title: "Automated Extraction",
        description: "AI parses explicit and implicit action items into discrete tasks.",
        icon: Sliders
      }, {
        step: "03",
        title: "Export to Tracker",
        description: "Copy task lists pre-formatted for Linear, Trello, or Jira ticket creation.",
        icon: CheckCircle2
      }]} badges={["100% Free", "Task Prioritization", "Linear & Jira Ready"]} />

 <ToolFeatureGuides features={[{
        icon: CheckSquare,
        title: "Implicit Task Recognition",
        description: "Identifies tasks even when phrased casually ('we should look into X')."
      }, {
        icon: Tag,
        title: "Category & Priority Tagging",
        description: "Assigns High/Medium/Low priority rankings based on task urgency."
      }, {
        icon: CheckCircle2,
        title: "Zero Data Persistence",
        description: "Processes project details strictly inside client-side browser memory."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Can I export tasks to Linear or Notion?",
        answer: "Yes! Use the 'Copy All' button to paste structured task lists directly into Linear or Notion database tables."
      }, {
        question: "How are task priorities calculated?",
        answer: "Tasks containing strict deadlines or blocking language are automatically categorized as High priority."
      }]} />
