...
const DEFAULT_COLUMNS: Column[] = [
  ...
  { id: "done", title: "Done", color: "border-l-4 border-l-emerald-500" },
];

const DEFAULT_TASKS: Task[] = [
  { id: "t1", title: "Design Landing Page Hero Section", description: "Incorporate glassmorphism cards and CTA", columnId: "todo" },
  { id: "t2", title: "Audit SEO Structured Data", description: "Verify Schema.org JSON-LD tags", columnId: "inprogress" },
  { id: "t3", title: "Deploy Toolzium V1 Release", description: "Vercel production build deployment", columnId: "done" },
];
...
export function KanbanClient() {
  ...
  return (
    <div className="relative max-w-6xl mx-auto space-y-8">
      ...
      <ToolPageHeader
        icon={LayoutGrid}
        title="Customizable Visual Kanban Board"
        description="Organize tasks into workflow columns (To Do, In Progress, Done), create custom columns, and move task cards across swimlanes."
        actions={
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <Input
                  ...
                  placeholder="Column Title (e.g. Code Review)..."
                  className="h-10 text-xs font-medium"
                />
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 pt-2">
        {columns.map((col, colIdx) => {
          ...
          return (
            <div key={col.id} className="min-w-[280px] w-[300px] shrink-0 bg-muted/20 border border-border/60 rounded-2xl p-4 flex flex-col max-h-[70vh]">
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                {columnTasks.length === 0 ? (
                  <div className="text-xs text-muted-foreground text-center py-8 italic border border-dashed border-border/60 rounded-xl">
                    No tasks in {col.title}
                  </div>
                ) : (
                  columnTasks.map((task) => (
                    <div key={task.id} className="bg-background rounded-xl p-3.5 shadow-sm border border-border/60 space-y-2.5">
