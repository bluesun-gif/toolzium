...
export function HabitTrackerClient() {
  ...
  const toggleHabit = (habitId: string, date: string) => {
    setHabits(habits.map(habit => {
      if (habit.id === habitId) {
        ...
        return {
          ...habit,
          completedDates: isCompleted ? habit.completedDates.filter(d => d !== date) : [...habit.completedDates, date].sort()
        };
      }
      return habit;
    }));
  ...
  const getRecentDays = () => {
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      const displayStr = d.toLocaleDateString(undefined, {
        weekday: "short",
        day: "numeric"
      });
      days.push({
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <form onSubmit={addHabit} className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input className="w-12 h-11 px-0 text-center rounded-xl font-bold" value={newHabitEmoji} onChange={e => setNewHabitEmoji(e.target.value)} maxLength={2} placeholder="✨" />
                  <Input id="h-name" className="flex-1 h-11 font-medium text-foreground" value={newHabitName} onChange={e => setNewHabitName(e.target.value)} placeholder="e.g. Morning Walk..." required />
                </div>
              </div>
            ...
            <div className="flex flex-col items-center p-4 bg-muted/20 border border-border/60 rounded-2xl">
              ...
              <div className="relative w-24 h-24 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle cx="48" cy="48" r="40" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray={251.2} strokeDashoffset={251.2 - 251.2 * overallProgress / 100} className="text-primary transition-all duration-500 ease-in-out" />
                </svg>
        ...
        <GlassCard className="md:col-span-2 space-y-4">
          ...
          <CardContent className="p-0 sm:p-6 overflow-x-auto">
            {habits.length === 0 ? <div className="text-center py-12 text-muted-foreground text-xs italic">
                No habits added yet. Start by creating a habit on the left!
              </div> : <div className="min-w-[650px]">
                <div className="grid grid-cols-[200px_1fr] gap-4 mb-3 px-2">
                  ...
                  <div className="grid grid-cols-14 gap-1">
                    {recentDays.map(day => <div key={day.dateStr} className={cn("text-[9px] font-bold text-center uppercase tracking-tighter leading-tight", day.isToday ? "text-primary font-black scale-110" : "text-muted-foreground")}>
                ...
                return <div key={habit.id} className="grid grid-cols-[200px_1fr] gap-4 py-3 border-t border-border/60 group items-center px-2">
                      <div className="flex flex-col gap-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <Button onClick={() => deleteHabit(habit.id)} className="opacity-0 group-hover:opacity-100 transition-opacity ml-auto text-muted-foreground hover:text-destructive p-1" title="Delete habit">
