...
export function AiMeetingSummarizerClient() {
  ...
  const handleSummarize = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setSummary({
        ...
        keyDecisions: ["Approved migration to zero-dependency client engines for peak performance.", "Confirmed deployment freeze scheduled for end-of-week release validation."],
        actionItems: [{
          owner: "Lead Eng",
          task: "Complete performance audit and verify clean build pipeline",
          deadline: "Friday"
        }, {
          owner: "Product",
          task: "Update client documentation and release notes",
          deadline: "Monday"
        }],
 ...
 <textarea className="w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50 min-h-[140px]" placeholder="e.g. Alex: We need to finalize the roadmap by Friday. Sarah agreed to handle the client presentation while Mark updates the API schemas..." value={transcript} onChange={e => setTranscript(e.target.value)} />
 </div>

 <div>
 <Label className="text-xs mb-1 block">Meeting Type Focus</Label>
 <select className="w-full rounded-md border border-input bg-background px-3 py-2 text-xs" value={meetingType} onChange={e => setMeetingType(e.target.value as any)}>
 ...
 {summary.keyDecisions.map((dec, i) => <li key={i}>{dec}</li>)}
 </ul>
 </GlassCard>
 ...
 {summary.actionItems.map((item, i) => <div key={i} className="flex items-center justify-between text-xs bg-muted/30 p-2 rounded border border-border/40">
 ...
 <ToolHowItWorks steps={[{
        step: "01",
        title: "Paste Meeting Text",
        description: "Paste raw transcript text or informal bullet points from your meeting.",
        icon: FileText
      }, {
        step: "02",
        title: "Select Focus",
        description: "Filter priorities based on Strategy, Agile Standups, or Client Sales calls.",
        icon: Sliders
      }, {
        step: "03",
        title: "Copy & Share",
        description: "Copy executive summaries and action items directly into Slack or Notion.",
        icon: CheckCircle2
      }]} badges={["100% Free", "Action Item Extractor", "Private & Offline"]} />

 <ToolFeatureGuides features={[{
        icon: FileText,
        title: "Executive Decision Extraction",
        description: "Filters conversational filler to highlight binding decisions and commitments."
      }, {
        icon: Users,
        title: "Owner & Deadline Tagging",
        description: "Automatically maps tasks to specific team members with assigned deadlines."
      }, {
        icon: CheckCircle2,
        title: "Complete Privacy",
        description: "Your confidential team transcripts are processed 100% locally in your browser memory."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Can I paste raw Otter.ai or Teams transcripts?",
        answer: "Yes! The summarizer parses speaker tags and raw timestamps cleanly."
      }, {
