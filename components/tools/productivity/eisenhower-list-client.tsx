...
const QUADRANTS = [
 ...
 { id:"q2", title:"Schedule", desc:"Not Urgent & Important"},
...
export function EisenhowerListClient() {
 ...
 const [newTaskTexts, setNewTaskTexts] = useState<Record<string, string>>({ q1:"", q2:"", q3:"", q4:""});
 ...
 if (!text) return;
 const newTask: Task = { id: Date.now().toString(), text, completed: false, quadrant };
 setTasks([...tasks, newTask]);
 setNewTaskTexts({ ...newTaskTexts, [quadrant]:""});
 ...
 description="Prioritize tasks using the Do, Schedule, Delegate, and Eliminate method."
 icon={CheckSquare}
 actions={
 <>
 ...
 {QUADRANTS.map(q => (
 <Button key={q.id} variant={filter === q.id ?"default":"outline"} onClick={() => setFilter(q.id)} size="sm"className="hidden md:inline-flex">
 {q.title}
 </Button>
 ))}
 </div>
 ...
 value={newTaskTexts[q.id]} 
 onChange={e => setNewTaskTexts({...newTaskTexts, [q.id]: e.target.value})}
 ...
 {qTasks.map(task => (
 <div key={task.id} className="flex items-center gap-2 p-2 border rounded-md group hover:bg-secondary/50">
