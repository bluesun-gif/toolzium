...
export function AiRiskMatrixClient() {
 ...
 <textarea className="w-full rounded-lg border border-border/70 bg-background/80 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/50 min-h-[140px]" placeholder="e.g. Building a high-throughput microservices architecture with real-time WebSocket communication and database sharding..." value={projectDescription} onChange={e => setProjectDescription(e.target.value)} />
 </div>
 ...
 {riskItems.length > 0 ? <motion.div initial={{
            opacity: 0,
            y: 15
          }} animate={{
            opacity: 1,
 ...
 <Button variant="ghost" size="sm" onClick={() => handleCopy(riskItems.map(r => `Risk: ${r.title}\nScore: ${r.score}/10\nMitigation: ${r.mitigation}`).join("\n\n"), "Risk matrix")} className="h-7 text-xs gap-1">
 <Copy className="w-3.5 h-3.5" /> Copy Matrix
 </Button>
 </div>

 {riskItems.map((risk, idx) => <GlassCard key={idx} className="p-4 space-y-3 border-l-4 border-l-red-500">
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Input technical specs, stack decisions, or deployment plans.",
        icon: ShieldAlert
      }, {
        step: "02",
        title: "Matrix Evaluation",
        description: "Calculates probability scores and severity impact rankings.",
        icon: Sliders
      }, {
        step: "03",
        title: "Copy Mitigations",
        description: "Export actionable risk mitigation steps directly into architecture review docs.",
        icon: CheckCircle2
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Quantifies technical risk exposure on a 1-to-10 severity scale."
      }, {
        icon: ShieldCheck,
        title: "Proactive Defense Strategies",
        description: "Provides concrete engineering remedies for every identified threat."
      }, {
        icon: CheckCircle2,
        title: "Confidentiality Assured",
        description: "Processes system diagrams and specs strictly in local client memory."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "Risk scores combine Likelihood (Low=2, Medium=5, High=8) and Impact (Minor=2, Major=5, Critical=8) multipliers."
      }, {
