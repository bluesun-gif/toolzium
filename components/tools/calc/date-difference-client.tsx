...
export default function DateDifferenceClient() {
  ...
  const summary = useMemo(() => {
    if (!parsed) return "";
    return [`Date difference (${parsed.aFmt} → ${parsed.bFmt})${inclusive ? "[inclusive]" : ""}`, `Calendar diff: ${parsed.years}y ${parsed.months}m ${parsed.days}d`, `Total days: ${parsed.totalDays}`, `Total weeks: ${parsed.totalWeeks}`, `Total hours: ${parsed.totalHours}`, `Total minutes: ${parsed.totalMinutes}`, `Total seconds: ${parsed.totalSeconds}`, `Business days (Mon–Fri): ${parsed.bizDays}`].join("\n");
  }, [parsed, inclusive]);
  ...
  const statItems = useMemo<StatItem[]>(() => {
    ...
    return [{
      ...
      key: "days",
      label: "Total days",
      value: parsed ? String(parsed.totalDays) : dash
    }, {
      key: "weeks",
      label: "Total weeks",
      value: parsed ? String(parsed.totalWeeks) : dash
    }, {
      key: "hours",
      label: "Total hours",
      value: parsed ? String(parsed.totalHours) : dash
    }, {
      key: "minutes",
      label: "Total minutes",
 ...
 {/* Header */}
 <ToolPageHeader icon={CalendarRange} title="Date Difference" description="Uses your local timezone. Toggle inclusive to count both start and end dates." actions={<>
 <ResetButton onClick={resetAll} />
 <ActionButton variant="outline" icon={RefreshCcw} label="Swap" onClick={swap} />
 <CopyButton variant="default" label="Copy Summary" getText={() => summary} disabled={!summary} />
 </>} />
 ...
 {QUICK_PRESETS.map(p => <ActionButton key={p.value} variant="secondary" size="sm" label={p.label} onClick={() => preset(p.value)} />)}
 </div>

 <div className="grid gap-6 md:grid-cols-[1fr_auto_1fr] md:items-end">
 <div className="grid gap-2">
 <InputField label="Start date" type="date" value={start} onChange={e => setStart(e.target.value)} />
 <ActionButton variant="outline" size="sm" icon={CalendarDays} label="Today" onClick={setTodayStart} />
 </div>
 ...
 <div className="grid gap-2">
 <InputField label="End date" type="date" value={end} onChange={e => setEnd(e.target.value)} />
 <ActionButton variant="outline" size="sm" icon={CalendarDays} label="Today" onClick={setTodayEnd} />
 </div>
 </div>

 {/* Inclusive toggle */}
 <div className="mt-4">
 <SwitchRow label="Count both start and end dates (inclusive)" checked={inclusive} onCheckedChange={setInclusive} />
 </div>
 ...
 {statItems.map(s => <Stat key={s.key} label={s.label} value={s.value} className={s.className} />)}

 <GlassCard className="p-4">
 <div className="space-y-1">
 <div className="text-sm text-muted-foreground">Range</div>
 <div className="text-sm font-medium">
 {parsed ? `${parsed.aFmt} → ${parsed.bFmt}${inclusive ? "(inclusive)" : ""}` : "—"}
 </div>
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: BookOpen
      }]} badges={["Years, months & days", "Instant calculation", "Works offline"]} />
 ...
 <tbody>
 {[["Age calculation", "Date of birth", "Today", "Years + months + days"], ["Project duration", "Project start", "Project end", "Days or weeks"], ["Loan tenure", "Loan disbursement", "Final payment", "Months"], ["Event countdown", "Today", "Event date", "Days"], ["Contract duration", "Signing date", "Expiry date", "Months or years"], ["Subscription renewal", "Start date", "Renewal date", "Days"]].map(([useCase, start, end, unit]) => <tr key={useCase} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["January", "31", "31"], ["February", "28", "29"], ["March", "31", "31"], ["April", "30", "30"], ["May", "31", "31"], ["June", "30", "30"], ["July", "31", "31"], ["August", "31", "31"], ["September", "30", "30"], ["October", "31", "31"], ["November", "30", "30"], ["December", "31", "31"]].map(([month, normal, leap]) => <tr key={month} className="odd:bg-muted/20">
 <td className="border p-2 font-medium text-xs">{month}</td>
 <td className="border p-2 text-center text-xs">{normal}</td>
 <td className="border p-2 text-center text-xs font-medium" style={{
                    color: month === 'February' ? 'var(--primary)' : undefined
                  }}>{leap}</td>
 </tr>)}
 </tbody>
 </table>
