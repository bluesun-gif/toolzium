...
export default function CurrencyConverterClient() {
  ...
  const fetchRates = useCallback(async (baseCode: string) => {
    ...
    try {
      ...
      localStorage.setItem(`cc_rates_${baseCode}`, JSON.stringify({
        updatedAt,
        rates: data.rates,
  ...
  const inlineInverse = inverseRate && Number.isFinite(inverseRate) ? `1 ${to} = ${formatNumber(inverseRate, decimals)} ${from}` : "—";
  ...
  function toggleFavorite() {
    ...
    setFavorites(favs => {
      ...
      return exists ? favs.filter(f => pairKey(f.from, f.to) !== key) : [{
        from,
        to
      }, ...favs].slice(0, 15);
    });
  ...
  const csvRows = useMemo<(string | number)[][]>(() => {
    ...
    return [["Time", "From", "To", "Amount", "Rate", "Result"], ...history.map(h => [h.ts, h.from, h.to, h.amount, h.rate, h.result])];
  }, [history]);
 ...
 {/* Header */}
 <ToolPageHeader icon={Globe} title="Currency Converter" description={`Convert currencies with live rates (${PROVIDER_LABEL}). Cached for ${CACHE_HOURS}h to load fast.`} actions={<>
 ...
 <InputField id="amount" label="Amount" inputMode="decimal" placeholder="100" value={amount} onChange={e => setAmount(e.target.value)} />
 ...
 {!!lastUpdated && <div className="text-xs text-muted-foreground">
 Updated: {new Date(lastUpdated).toLocaleString()} {isStale ? "(stale)" : ""}
 </div>}
 </div>

 {/* From */}
 <SelectField label="From" value={from} onValueChange={v => setFrom(v as string)} options={currencyOptions} />

 {/* To */}
 <SelectField label="To" value={to} onValueChange={v => setTo(v as string)} options={currencyOptions} />
 ...
 <ActionButton variant={isFavorite ? "default" : "outline"} icon={Sparkles} label={isFavorite ? "Favorited" : "Add Favorite"} onClick={toggleFavorite} />
 <CopyButton variant="outline" label="Copy Share Link" getText={() => typeof window !== "undefined" ? window.location.href : ""} disabled={typeof window === "undefined"} />
 <div className="ml-auto flex items-center gap-3">
 <InputField id="decimals" label="Decimals" type="number" min={0} max={10} value={decimals} onChange={e => setDecimals(Math.min(10, Math.max(0, Number(e.target.value) || 0)))} className="w-28" />
 <Badge variant={status === "ok" ? "default" : status === "cached" ? "outline" : status === "loading" ? "outline" : "destructive"}>
 {status === "loading" ? "Fetching…" : status === "ok" ? "Live" : status === "cached" ? isStale ? "Cached (stale)" : "Cached" : "Error"}
 </Badge>
 </div>
 </div>
 </CardContent>
 ...
 <div className="mt-2">
 <CopyButton size="sm" label="Copy rate" getText={() => rate ? `1 ${from} = ${rate} ${to}` : ""} disabled={!rate} />
 </div>
 </div>
 ...
 <div className="mt-2">
 <CopyButton size="sm" label="Copy amount" getText={() => rate ? String(result) : ""} disabled={!rate} />
 </div>
 </div>
 </CardContent>
 </GlassCard>
 ...
 {/* Favorites & Recent */}
 {(favorites.length > 0 || history.length > 0) && <div className="grid gap-4 lg:grid-cols-2">
 {favorites.length > 0 && <GlassCard>
 <CardHeader>
 ...
 {favorites.map((f, i) => <ActionButton key={`${f.from}-${f.to}-${i as number}`} variant="outline" size="sm" label={`${f.from} → ${f.to}`} onClick={() => {
              setFrom(f.from);
              setTo(f.to);
            }} />)}
 </CardContent>
 ...
 <ExportCSVButton variant="outline" icon={TrendingUp} label="Export CSV" disabled={!history.length} filename="currency-history.csv" getRows={() => csvRows} />
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: BookOpen
      }]} badges={["Privacy-first", "150+ world currencies", "Live rates, 3-min cache"]} />
 ...
 <tbody>
 {[["Spot Rate", "Current market price for immediate exchange", "Online converters, interbank market"], ["Bid/Ask Rate", "Buy/sell price offered by banks and brokers", "Forex brokers, money changers"], ["Forward Rate", "Agreed rate for exchange at a future date", "Importers, exporters, hedge funds"], ["Cross Rate", "Rate between two non-USD currencies (via USD)", "International wire transfers"], ["Official Rate", "Central bank–set rate (some countries)", "Government, regulated transactions"]].map(([type, desc, usedBy]) => <tr key={type} className="odd:bg-muted/20">
 ...
 {[["EUR/USD", "Euro / US Dollar", "Most traded pair — ~23% of global FX volume"], ["USD/JPY", "US Dollar / Japanese Yen", "Key carry trade pair; influenced by BOJ policy"], ["GBP/USD", "British Pound / USD (Cable)", "Highly volatile; affected by UK economic data"], ["USD/BDT", "US Dollar / Bangladeshi Taka", "Pegged/managed float; relevant for Bangladesh"], ["USD/INR", "US Dollar / Indian Rupee", "Emerging market pair; tracked for South Asia trade"], ["USD/CNY", "US Dollar / Chinese Yuan", "Managed by PBOC; critical for global trade pricing"]].map(([pair, name, note]) => <div key={pair} className="flex flex-col gap-1 rounded-md border bg-muted/30 p-2">
