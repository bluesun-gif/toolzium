...
export default function ScientificCalculatorClient() {
  ...
  const handleCalculate = () => {
    try {
      let evalExpr = expression.replace(/sin\(/g, "Math.sin(").replace(/cos\(/g, "Math.cos(").replace(/tan\(/g, "Math.tan(").replace(/sqrt\(/g, "Math.sqrt(").replace(/log\(/g, "Math.log10(").replace(/ln\(/g, "Math.log(").replace(/pi/g, "Math.PI").replace(/e/g, "Math.E").replace(/\^/g, "**");
  ...
  const buttons = ["sin(", "cos(", "tan(", "sqrt(", "C", "DEL", "log(", "ln(", "pi", "e", "(", ")", "7", "8", "9", "/", "^", "%", "4", "5", "6", "*", "1", "2", "3", "-", "0", ".", "=", "+"];
 ...
 <div className="text-primary-foreground text-4xl font-bold overflow-x-auto whitespace-nowrap">{result || "0"}</div>
 </div>
 
 <div className="grid grid-cols-6 gap-2">
 {buttons.map(btn => {
              let className = "h-12 rounded-lg font-semibold text-sm transition-colors";
              if (btn === "=") className += "bg-primary text-primary-foreground hover:bg-primary/90 col-span-2";else if (btn === "C" || btn === "DEL") className += "bg-destructive/20 text-destructive hover:bg-destructive/30";else if (["+", "-", "*", "/", "^", "%", "(", ")"].includes(btn)) className += "bg-muted hover:bg-muted/80 text-foreground";else if (["sin(", "cos(", "tan(", "sqrt(", "log(", "ln(", "pi", "e"].includes(btn)) className += "bg-primary/10 text-primary hover:bg-primary/20 text-xs";else className += "bg-card border border-border/50 hover:bg-muted/50 text-foreground";
              return <Button key={btn} variant="ghost" className={className} onClick={() => {
                if (btn === "C") handleClear();else if (btn === "DEL") handleBackspace();else if (btn === "=") handleCalculate();else handleButtonClick(btn);
              }}>
 {btn}
 </Button>;
            })}
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Use the keypad to type your mathematical expression, including scientific functions.",
        icon: Calculator
      }, {
        step: "02",
        title: "Review Formula",
        description: "Check the top line of the display to ensure your formula is entered correctly.",
        icon: Calculator
      }, {
        step: "03",
        title: "Calculate",
        description: "Press the equals button to instantly compute the final result.",
        icon: Calculator
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Includes sin, cos, tan, log, ln, square root, and constants like pi and e."
      }, {
        icon: Calculator,
        title: "Expression Editing",
        description: "Use the backspace button to correct typos without clearing the entire formula."
      }, {
        icon: Calculator,
        title: "Operator Support",
        description: "Supports standard arithmetic, exponents (^), and modulo (%) operations."
      }, {
        icon: Calculator,
        title: "Error Handling",
        description: "Displays a clear error toast if the mathematical expression is invalid."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        ...
        answer: "This calculator uses radians for sin, cos, and tan functions, which is the standard in higher mathematics and programming."
      }, {
        question: "How do I calculate exponents?",
        answer: "Use the ^ symbol. For example, to calculate 2 to the power of 8, type 2^8."
      }, {
        question: "What does the 'ln' button do?",
        answer: "The 'ln' button calculates the natural logarithm (base e), while 'log' calculates the common logarithm (base 10)."
      }]} />
