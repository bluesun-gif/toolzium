...
export default function TipSplitterClient() {
 ...
 <span className="ml-2 text-muted-foreground">{c.symbol}</span>
  ...
  const [tipBase, setTipBase] = React.useState<TipBaseMode>(qs("tb", "pre-tax") as TipBaseMode || "pre-tax");
  const [people, setPeople] = React.useState<string>(qs("p", "2") || "2");
  const [roundScope, setRoundScope] = React.useState<RoundScope>(qs("rs", "none") as RoundScope || "none");
  const [roundMode, setRoundMode] = React.useState<RoundMode>(qs("rm", "nearest") as RoundMode || "nearest");
  ...
  const currentRows = React.useMemo<(string | number)[][]>(() => [["Currency", currency], ["Bill (pre-tax)", fmt(subtotal, currency)], ["Tax %", taxPctN], ["Tax amount", fmt(tax, currency)], ["Tip base", tipBase === "pre-tax" ? "Pre-tax" : "Post-tax"], ["Tip %", tipPctN], ["Tip amount", fmt(tipAmount, currency)], ["People", peopleN], ["Rounding", `${roundScope} / ${roundMode}`], ["Rounding adjustment", fmt(roundingAdjustment, currency)], ["Total", fmt(totalRounded, currency)], ["Per-person (rounded)", fmt(perPersonRounded, currency)], ["Per-person (raw)", fmt(perPersonRaw, currency)]], [currency, subtotal, taxPctN, tax, tipBase, tipPctN, tipAmount, peopleN, roundScope, roundMode, roundingAdjustment, totalRounded, perPersonRounded, perPersonRaw]);
 ...
 {/* Header */}
 <ToolPageHeader icon={UtensilsCrossed} title="Tip Splitter" description="Split a bill across people with tax, tip, and smart rounding." actions={<>
 <ResetButton onClick={resetAll} />
 <ActionButton variant="default" icon={Calculator} label="Save Result" onClick={saveHistory} />
 </>} />
 ...
 {/* Currency */}
 <SelectField label="Currency" value={currency} onValueChange={v => setCurrency(v as Currency)} options={currencyOptions} />

 {/* Bill */}
 <InputField label="Bill Amount (pre-tax)" id="bill" inputMode="decimal" placeholder="e.g. 1200" value={bill} onChange={e => setBill(e.target.value)} />

 {/* People */}
 <div className="space-y-2">
 <Label htmlFor="people">People</Label>
 <div className="flex items-center gap-2">
 <ActionButton variant="outline" size="icon" icon={Minus} aria-label="decrease people" onClick={() => setPeople(String(Math.max(1, parseNum(people) - 1)))} />
 <InputField id="people" inputMode="numeric" value={people} onChange={e => setPeople(e.target.value)} />
 <ActionButton variant="outline" size="icon" icon={Plus} aria-label="increase people" onClick={() => setPeople(String(Math.max(1, parseNum(people) + 1)))} />
 </div>
 <div className="flex flex-wrap gap-1">
 {peoplePresets.map(pp => <Badge key={pp} variant={parseNum(people) === pp ? "default" : "outline"} className="cursor-pointer" onClick={() => setPeople(String(pp))}>
 {pp}
 </Badge>)}
 </div>
 </div>
 ...
 {/* Tax */}
 <InputField label="Tax (%)" id="tax" inputMode="decimal" value={taxPct} onChange={e => setTaxPct(e.target.value)} placeholder="e.g. 0 or 7.5" />

 {/* Tip */}
 <div className="space-y-2">
 <InputField label="Tip (%)" id="tip" inputMode="decimal" value={tipPct} onChange={e => setTipPct(e.target.value)} />
 <div className="flex flex-wrap gap-1">
 {tipPresets.map(p => <Badge key={p} variant={parseNum(tipPct) === p ? "default" : "outline"} className="cursor-pointer" onClick={() => setTipPct(String(p))}>
 {p}%
 </Badge>)}
 </div>
 </div>
 ...
 <Badge variant={tipBase === "pre-tax" ? "default" : "outline"} className="cursor-pointer" onClick={() => setTipBase("pre-tax")}>
 Pre-tax
 </Badge>
 <Badge variant={tipBase === "post-tax" ? "default" : "outline"} className="cursor-pointer" onClick={() => setTipBase("post-tax")}>
 Post-tax
 </Badge>
 ...
 {roundScope !== "none" && <div className="mt-1 text-xs text-muted-foreground">
 ...
 <CopyButton label="Copy Summary" icon={copied === "values" ? Check : Copy} getText={() => summaryText} onCopied={() => setCopied("values")} />
 <CopyButton label="Copy Share Link" icon={copied === "link" ? Check : Copy} getText={() => shareUrl ? shareUrl : ""} onCopied={() => setCopied("link")} />
 <ExportCSVButton filename="tip-split.csv" label="Export This Split" icon={Download} getRows={async () => currentRows} variant="outline" />
 {history.length > 0 && <ExportCSVButton filename="tip-split-history.csv" label="Export History" icon={Download} getRows={async () => historyRows} variant="outline" />}
 </div>

 {/* History */}
 {history.length > 0 && <GlassCard className="mt-4">
 <CardHeader>
 ...
 <tbody>
 {history.map((h, idx) => <tr key={idx as number} className="[&>td]:border-b [&>td]:px-3 [&>td]:py-2">
 ...
 <ToolHowItWorks steps={[{
        ...
        icon: Calculator
      }]} badges={["Instant calculation", "Custom tip %", "Split any party size"]} />
 ...
 <tbody>
 {[["Restaurant (sit-down)", "10%", "15–18%", "20–25%"], ["Bar / Bartender", "—", "$1–2/drink or 15%", "20%+"], ["Food delivery", "10%", "15–18%", "20%+"], ["Taxi / Rideshare", "10%", "15%", "20%"], ["Hotel housekeeping", "—", "$2–5/night", "$5–10/night"], ["Hair salon", "10%", "15–20%", "25%+"], ["Spa / Massage", "10%", "15–20%", "25%+"], ["Pizza delivery", "—", "$2–5 or 15%", "20%+"]].map(([service, poor, standard, excellent]) => <tr key={service} className="odd:bg-muted/20">
 ...
 <tbody>
 {[["USA / Canada", "Expected, part of income", "15–20% at restaurants"], ["UK / Australia", "Appreciated, not required", "10–15% for good service"], ["Continental Europe", "Optional, round up", "Round up or 5–10%"], ["Japan / South Korea", "Not customary; can offend", "No tip expected"], ["Middle East", "Appreciated", "10–15%"], ["Bangladesh/India", "Appreciated in restaurants", "10% or round up"]].map(([country, culture, amount]) => <tr key={country} className="odd:bg-muted/20">
