...
export default function DiscountFinderClient() {
 ...
 <Input type="number" value={originalPrice} onChange={e => setOriginalPrice(Number(e.target.value))} min={0} step={0.01} />
 </div>
 <div>
 <label className="text-xs font-medium text-muted-foreground mb-2 block">
 Discount (%)
 ...
 <CopyButton getText={() => calculations.discountAmount.toFixed(2)} label="Copy" />
 </div>
 ...
 <CopyButton getText={() => calculations.priceAfterDiscount.toFixed(2)} label="Copy" />
 </div>

 {taxPercent > 0 && <div className="p-4 bg-muted/40 rounded-lg text-center">
 <div className="text-xs text-muted-foreground mb-1">Tax</div>
 <div className="text-xl font-bold text-orange-500">
 ${calculations.taxAmount.toFixed(2)}
 </div>
 <CopyButton getText={() => calculations.taxAmount.toFixed(2)} label="Copy" />
 </div>}
 ...
 <CopyButton getText={() => calculations.finalPrice.toFixed(2)} label="Copy" />
 </div>
 </div>
 ...
 <CopyButton getText={() => calculations.totalSavings.toFixed(2)} label="Copy Savings" />
 </div>
 </CardContent>
 </GlassCard>
 ...
 <ToolHowItWorks steps={[{
        ...
        description: "Input the full price before any discount is applied.",
        icon: Tag
      }, {
        step: "02",
        title: "Set Discount",
        description: "Enter the discount percentage to see your savings.",
        icon: Tag
      }, {
        step: "03",
        title: "Add Tax (Optional)",
        description: "Include sales tax to calculate the final total price.",
        icon: Copy
 ...
 <ToolFeatureGuides features={[{
        ...
        description: "Instantly see how much you save with any percentage discount."
      }, {
        icon: Copy,
        title: "Tax Support",
        description: "Optionally add sales tax to calculate the true final price."
      }, {
        icon: Tag,
        title: "Savings Display",
        description: "Clearly shows your total savings in a highlighted card."
      }, {
        icon: Copy,
        title: "Copy Values",
        description: "Copy any calculated value with one click for use elsewhere."
      }]}>
 ...
 <ToolFaqAccordion faqs={[{
        question: "Is tax calculated on the original or discounted price?",
        answer: "Sales tax is typically calculated on the price after the discount is applied, which is how this calculator works."
      }, {
        question: "Can I use this for bulk discounts?",
        answer: "Yes, enter the total original price for all items and the discount percentage to see your total savings."
      }, {
