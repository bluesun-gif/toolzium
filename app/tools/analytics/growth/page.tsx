...
• Daily High Target: ${data.dailyHigh.visitors} visitors peak!`;
  ...
  const maxHistoryVal = Math.max(...data.history.map((h) => Math.max(h.uniqueVisitors, h.toolRuns, 1)), 10);
  ...
  return (
    <div className="container max-w-6xl mx-auto py-8 px-4 sm:px-6 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            <Badge className="rounded-full bg-primary/10 text-primary border-primary/20 px-3 py-0.5 text-xs font-bold">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Button
            ...
            variant="outline"
            className="rounded-2xl font-bold text-xs gap-1.5 h-10 border-primary/20 hover:bg-primary/5"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Visitors */}
        <Card className="rounded-3xl border-2 border-primary/20 bg-gradient-to-br from-card to-background shadow-lg">
          <CardContent className="p-5 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-muted-foreground uppercase">Unique Visitors</span>
            ...
            <div className="flex items-center justify-between text-[11px] text-muted-foreground pt-1 border-t border-border/40">
              ...
              <span className="font-bold text-foreground">{data.dailyHigh.visitors}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-3xl border-2 border-primary/20 bg-gradient-to-br from-card to-background shadow-lg">
          <CardContent className="p-5 space-y-3">
            ...
            <div className="flex items-center justify-between text-[11px] text-muted-foreground pt-1 border-t border-border/40">
              ...
              <span className="font-bold text-foreground">{data.dailyHigh.toolRuns} runs</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-3xl border-2 border-primary/20 bg-gradient-to-br from-card to-background shadow-lg">
          <CardContent className="p-5 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-muted-foreground uppercase">Clicks & Actions</span>
        ...
        {/* #1 Winner Tool */}
        <Card className="rounded-3xl border-2 border-primary/30 bg-gradient-to-br from-primary/10 via-card to-background shadow-lg">
          <CardContent className="p-5 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-muted-foreground uppercase">#1 Ranked Tool</span>
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground mt-0.5">
                <span className="font-bold text-primary">{data.topTool.runs} runs</span>
                <span>•</span>
            ...
            <div className="flex items-center justify-between text-[11px] text-muted-foreground pt-1 border-t border-border/40">
              ...
              <span className="font-bold text-emerald-500">🏆 Top Pull Driver</span>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="rounded-3xl border-2 border-primary/20 bg-card shadow-xl overflow-hidden">
        ...
        <CardContent className="p-6">
          ...
          <div className="h-64 sm:h-72 w-full flex items-end justify-between gap-1 sm:gap-2 pt-6">
            {data.history.map((day, idx) => {
              const visitorHeight = Math.max(8, (day.uniqueVisitors / maxHistoryVal) * 100);
              const runsHeight = Math.max(8, (day.toolRuns / maxHistoryVal) * 100);
              const label = day.date.slice(5); // MM-DD

              return (
                <div key={idx} className="flex-1 flex flex-col items-center gap-2 group relative">
                  {/* Tooltip on hover */}
                  <div className="absolute -top-14 bg-popover text-popover-foreground border border-border shadow-lg rounded-xl px-2.5 py-1 text-[10px] font-mono whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                  ...
                  <div className="w-full flex items-end justify-center gap-0.5 sm:gap-1 h-48">
                    ...
                    <div
                      ...
                      className="w-full max-w-[12px] bg-primary rounded-t-md hover:bg-primary/80 transition-all duration-300"
                    />
                    {/* Tool Runs Bar */}
                    <div
                      style={{ height: `${runsHeight}%` }}
                      className="w-full max-w-[12px] bg-emerald-500 rounded-t-md hover:bg-emerald-400 transition-all duration-300"
                    />
                  </div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-muted-foreground">High-RPM Pull Tool</span>
                  </div>
                </div>

                <div className="text-right">
                  <span className="text-base font-black text-foreground">{tool.runs}</span>
